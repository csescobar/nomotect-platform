...
- The application image is built from the repository `Dockerfile` production stage.
...
- `KAMAL_IMAGE`: optional image repository name; defaults to `rails-hotwire-platform`.
...
4. Run `bash bin/kamal-validate`.
5. Run `bundle exec kamal setup`.
6. Open the installation URL and complete the protected first-run wizard with the bootstrap token.
