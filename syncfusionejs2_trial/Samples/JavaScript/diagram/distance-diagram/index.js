 ...
 */
    ...
    // --- Global collections and state variables ---
    var nodes = [];
    var connectors = [];
    var diagram;
    var selectedUnit = 'in';
    ...
    // --- EJ2 and Diagram specific constants ---
    var ConnectorConstraints = ej.diagrams.ConnectorConstraints;
    ...
    // --- Constants for hardcoded values ("Magic Numbers") ---
    var WALL_COLOR = '#333333';
    var OUTER_WALL_THICKNESS = 3;
    var SEPARATOR_THICKNESS = 5;
    var DIMENSION_LINE_COLOR = '#0066CC';
    var DIMENSION_LINE_THICKNESS = 2;
    var EXTENSION_LINE_THICKNESS = 1;

    // Measurement offsets
    var TOTAL_WIDTH_Y_OFFSET = -50;
    var TOTAL_HEIGHT_X_OFFSET = -70;
    var SEPARATOR1_Y_OFFSET = 40;
    var SEPARATOR3_Y_OFFSET = 20;
    var HALL_SEPARATOR_X_OFFSET = 270;
    var SEPARATOR4_Y_OFFSET = -20;
    var SEPARATOR2_X_OFFSET = 30;
    var RESTROOM_SEPARATOR_X_OFFSET = 60;
    var EXTENSION_LINE_OFFSET = 10;
    
    function getConnectorsByIds(ids) {
        var foundConnectors = {};
        ids.forEach(function (id) {
            foundConnectors[id] = connectors.find((c) => c.id === id);
        });
        return foundConnectors;
    }

    // Helper function to create a wall connector
    function createWallConnector(id, startX, startY, endX, endY, color, thickness) {
    ...
    // Helper function to create various items as nodes
    function createItem(id, x, y, width, height, pathData, options) {
        var item = {
            id: id,
            width: width,
            height: height,
    ...
    // Functions to create specific items, mapping to the 'createItem' helper
    function createBedroomItem(id, x, y, width, height, pathData) {
        var options = {
            fill: '#E8F4FD',
            strokeColor: '#2E86AB',
        };
    ...
    function createKitchenItem(id, x, y, width, height, pathData) {
        var options = {};
        if (id === 'KitchenSink') {
            options.fill = '#FFF2E8';
    ...
    function createLivingRoomItem(id, x, y, width, height, pathData) {
        var options = {
            fill: '#FFF8DC',
            strokeColor: '#B8860B',
        };
    ...
    // Function to set up the connections between wall ports
    function createPortConnections() {
        var c = getConnectorsByIds([
            'WallOuterTop', 'WallOuterLeft', 'WallOuterBottom', 'WallOuterRight',
            'Separator1', 'Separator2', 'Separator3', 'Separator4'
        ]);

        if (c.WallOuterTop && c.WallOuterLeft) c.WallOuterTop.sourceID = 'WallOuterLeft';
        if (c.WallOuterTop && c.WallOuterRight) c.WallOuterTop.targetID = 'WallOuterRight';
        if (c.WallOuterBottom && c.WallOuterLeft) c.WallOuterBottom.sourceID = 'WallOuterLeft';
        if (c.WallOuterBottom && c.WallOuterRight) c.WallOuterBottom.targetID = 'WallOuterRight';
        if (c.Separator3 && c.WallOuterLeft) {
            c.Separator3.sourceID = 'WallOuterLeft';
            c.Separator3.sourcePortID = 'WallOuterLeftMiddlePort';
        }
        if (c.Separator2 && c.WallOuterTop) {
            c.Separator2.sourceID = 'WallOuterTop';
            c.Separator2.sourcePortID = 'WallOuterTopMiddlePort';
        }
        if (c.Separator1 && c.WallOuterRight) {
            c.Separator1.sourceID = 'WallOuterRight';
            c.Separator1.sourcePortID = 'WallOuterRightMiddlePort2';
        }
        if (c.Separator4 && c.WallOuterRight) {
            c.Separator4.sourceID = 'WallOuterRight';
            c.Separator4.sourcePortID = 'WallOuterRightMiddlePort';
        }
    }

    function createRestroomSeparator() {
        createWallConnector('RestroomSeparator', 780, 610, 780, 480, WALL_COLOR, SEPARATOR_THICKNESS);
        var c = getConnectorsByIds(['RestroomSeparator', 'WallOuterBottom']);
        if (c.RestroomSeparator && c.WallOuterBottom) {
            c.RestroomSeparator.sourceID = 'WallOuterBottom';
            c.RestroomSeparator.sourcePortID = 'WallOuterBottomMiddlePort2';
        }
    }

    function createHallSeparator() {
        createWallConnector('HallSeparator', 540, 610, 540, 410, WALL_COLOR, SEPARATOR_THICKNESS);
        var c = getConnectorsByIds(['HallSeparator', 'WallOuterBottom']);
        if (c.HallSeparator && c.WallOuterBottom) {
            c.HallSeparator.sourceID = 'WallOuterBottom';
            c.HallSeparator.sourcePortID = 'WallOuterBottomMiddlePort';
        }
    }
    
    // Creates the entire diagram layout
    function createInitialLayout() {
        // Create all walls
        createWallConnector('WallOuterTop', 299, 150, 901, 150, WALL_COLOR, OUTER_WALL_THICKNESS);
        createWallConnector('WallOuterLeft', 300, 150, 300, 610, WALL_COLOR, OUTER_WALL_THICKNESS);
        createWallConnector('WallOuterBottom', 299, 610, 901, 610, WALL_COLOR, OUTER_WALL_THICKNESS);
        createWallConnector('WallOuterRight', 900, 150, 900, 610, WALL_COLOR, OUTER_WALL_THICKNESS);
        createWallConnector('Separator1', 900, 430, 670, 430, WALL_COLOR, SEPARATOR_THICKNESS);
        createWallConnector('Separator2', 628, 150, 628, 313, WALL_COLOR, SEPARATOR_THICKNESS);
        createWallConnector('Separator3', 300, 310, 560, 310, WALL_COLOR, SEPARATOR_THICKNESS);
        createWallConnector('Separator4', 900, 310, 700, 310, WALL_COLOR, SEPARATOR_THICKNESS);
        createPortConnections();

        // --- Create furniture and door nodes ---
        // Represents a bed with decorative pillows
        createBedroomItem(
        ...
        // Represents a small bedroom bench
        createBedroomItem(
        ...
        // Represents a wardrobe or cabinet
        createBedroomItem(
        ...
        // Represents a second swinging door
        createBedroomItem(
            'bedroomDoor2', 664, 285, 60, 50,
            'M 0 26.92 L 0 30 L 28 30 L 28 26.92 M 0 26.92 L 28 26.92 M 0 26.92 C 0 12.05 12.36 0 27.61 0 L 28 0 L 28 26.92'
        );
        // Represents a kitchen sink with two basins
        createKitchenItem(
        ...
        // Represents a TV stand
        createLivingRoomItem(
        ...
        // Represents a full bathroom with toilet and sink
        createLivingRoomItem(
        ...
        // Represents the main entrance double door
        createLivingRoomItem(
        ...
        // Add dimensions after all elements are created
        addMeasurementConnectorsRuntime();
        addOverallMeasurementConnectors();
    }

    // Measurement and Annotation Logic
    function createDimensionConnector(id, startX, startY, endX, endY, dimensionText, isHorizontal) {
        var dimensionLine = {
            id: id,
            sourcePoint: { x: startX, y: startY },
            targetPoint: { x: endX, y: endY },
            ...
            style: { strokeColor: DIMENSION_LINE_COLOR, strokeWidth: DIMENSION_LINE_THICKNESS },
            sourceDecorator: { shape: 'Arrow', style: { fill: DIMENSION_LINE_COLOR, strokeColor: DIMENSION_LINE_COLOR } },
            targetDecorator: { shape: 'Arrow', style: { fill: DIMENSION_LINE_COLOR, strokeColor: DIMENSION_LINE_COLOR } },
            annotations: [
                {
                    content: dimensionText,
                    style: { color: 'black', fontSize: 12, fill: 'white', fontFamily: 'Arial', bold: true },
                    offset: 0.5,
                    textAlign: 'Center',
                },
            ],
        ...
        var extOffset = EXTENSION_LINE_OFFSET;
        if (isHorizontal) {
            createExtensionLine(id + '_ExtStart', startX, startY - extOffset, startX, startY + extOffset);
            createExtensionLine(id + '_ExtEnd', endX, endY - extOffset, endX, endY + extOffset);
        } else {
            createExtensionLine(id + '_ExtStart', startX - extOffset, startY, startX + extOffset, startY);
            createExtensionLine(id + '_ExtEnd', endX - extOffset, endY, endX + extOffset, endY);
        }
    }

    function createBathroomDoor(id, x, y, width, height, pathData) {
        var options = {
            fill: '#E6E6FA',
            strokeColor: '#9370DB',
            rotateAngle: 270,
    ...
    function createExtensionLine(id, startX, startY, endX, endY) {
        var extensionLine = {
            id: id,
            sourcePoint: { x: startX, y: startY },
            targetPoint: { x: endX, y: endY },
            ...
            sourceDecorator: { shape: 'None' },
            targetDecorator: { shape: 'None' },
            style: { strokeColor: DIMENSION_LINE_COLOR, strokeWidth: EXTENSION_LINE_THICKNESS },
        };
        connectors.push(extensionLine);
    }

    function addMeasurementConnectorsRuntime() {
        var allSeparators = connectors.filter(
            (c) => c.id.startsWith('Separator') || c.id === 'RestroomSeparator' || c.id === 'HallSeparator'
        );
        var commonWalls = getConnectorsByIds(['WallOuterTop', 'WallOuterBottom', 'WallOuterRight']);

        allSeparators.forEach(function (separator) {
            addSpecificSeparatorMeasurement(separator, commonWalls);
        });
    }

    function addOverallMeasurementConnectors() {
        var c = getConnectorsByIds(['WallOuterTop', 'WallOuterLeft', 'WallOuterBottom', 'WallOuterRight']);

        if (c.WallOuterTop && c.WallOuterLeft && c.WallOuterBottom && c.WallOuterRight) {
            // Total Width Measurement
            var totalWidth = Math.abs(c.WallOuterRight.targetPoint.x - c.WallOuterLeft.sourcePoint.x);
            var widthText = formatMeasurement(convertPixelsToSelectedUnit(totalWidth));
            createDimensionConnector(
                'TotalWidthMeasurement',
                c.WallOuterLeft.sourcePoint.x,
                c.WallOuterTop.sourcePoint.y + TOTAL_WIDTH_Y_OFFSET,
                c.WallOuterRight.targetPoint.x,
                c.WallOuterTop.sourcePoint.y + TOTAL_WIDTH_Y_OFFSET,
                widthText,
                true
            );
            ...
            var totalHeight = Math.abs(c.WallOuterBottom.targetPoint.y - c.WallOuterTop.sourcePoint.y);
            var heightText = formatMeasurement(convertPixelsToSelectedUnit(totalHeight));
            createDimensionConnector(
                'TotalHeightMeasurement',
                c.WallOuterLeft.sourcePoint.x + TOTAL_HEIGHT_X_OFFSET,
                c.WallOuterTop.sourcePoint.y,
                c.WallOuterLeft.sourcePoint.x + TOTAL_HEIGHT_X_OFFSET,
                c.WallOuterBottom.targetPoint.y,
                heightText,
                false
            );
        }
    ...
    function addSpecificSeparatorMeasurement(separator, commonWalls) {
        var { WallOuterTop: wallOuterTop, WallOuterBottom: wallOuterBottom, WallOuterRight: wallOuterRight } = commonWalls;
        var yOffset, xOffset, distance;

        switch (separator.id) {
            case 'Separator1':
                distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x);
                yOffset = wallOuterBottom.sourcePoint.y + SEPARATOR1_Y_OFFSET;
                createDimensionConnector(separator.id + 'Measurement', separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset, formatMeasurement(convertPixelsToSelectedUnit(distance)), true);
                break;
            case 'Separator3':
                distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x);
                yOffset = wallOuterBottom.sourcePoint.y + SEPARATOR3_Y_OFFSET;
                createDimensionConnector(separator.id + 'Measurement', separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset, formatMeasurement(convertPixelsToSelectedUnit(distance)), true);
                break;
            case 'HallSeparator':
                distance = Math.abs(separator.targetPoint.y - separator.sourcePoint.y);
                xOffset = HALL_SEPARATOR_X_OFFSET;
                createDimensionConnector(separator.id + 'Measurement', xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y, formatMeasurement(convertPixelsToSelectedUnit(distance)), false);
                break;
            case 'Separator4':
                distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x);
                yOffset = wallOuterTop.sourcePoint.y + SEPARATOR4_Y_OFFSET;
                createDimensionConnector(separator.id + 'Measurement', separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset, formatMeasurement(convertPixelsToSelectedUnit(distance)), true);
                break;
            case 'Separator2':
                distance = Math.abs(separator.targetPoint.y - separator.sourcePoint.y);
                xOffset = wallOuterRight.sourcePoint.x + SEPARATOR2_X_OFFSET;
                createDimensionConnector(separator.id + 'Measurement', xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y, formatMeasurement(convertPixelsToSelectedUnit(distance)), false);
                break;
            case 'RestroomSeparator':
                distance = Math.abs(separator.targetPoint.y - separator.sourcePoint.y);
                xOffset = wallOuterRight.sourcePoint.x + RESTROOM_SEPARATOR_X_OFFSET;
                createDimensionConnector(separator.id + 'Measurement', xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y, formatMeasurement(convertPixelsToSelectedUnit(distance)), false);
                break;
        }
    }
    ...
    function convertPixelsToSelectedUnit(pixelValue) {
        var PIXELS_PER_FOOT = 96.0;
        var PIXELS_PER_INCH = 8.0;
        var PIXELS_PER_CM = 3.15;
        switch (selectedUnit) {
            case 'ft': return pixelValue / PIXELS_PER_FOOT;
            case 'in': return pixelValue / PIXELS_PER_INCH;
            case 'cm': return pixelValue / PIXELS_PER_CM;
            default: return pixelValue / PIXELS_PER_FOOT;
        }
    }
        ...
        if (id.startsWith('Separator') || id === 'RestroomSeparator' || id === 'HallSeparator') {
            ...
            switch (id) {
                case 'Separator1':
                    ...
                    startY = endY = wallOuterBottom.sourcePoint.y + SEPARATOR1_Y_OFFSET;
                    isHorizontal = true;
                    break;
                case 'Separator3':
                    distance = Math.abs(movedConnector.targetPoint.x - movedConnector.sourcePoint.x);
                    startX = movedConnector.sourcePoint.x; endX = movedConnector.targetPoint.x;
                    startY = endY = wallOuterBottom.sourcePoint.y + SEPARATOR3_Y_OFFSET;
                    isHorizontal = true;
                    break;
                case 'HallSeparator':
                    ...
                    startX = endX = HALL_SEPARATOR_X_OFFSET;
                    isHorizontal = false;
                    break;
                case 'Separator4':
                    distance = Math.abs(movedConnector.targetPoint.x - movedConnector.sourcePoint.x);
                    startX = movedConnector.sourcePoint.x; endX = movedConnector.targetPoint.x;
                    startY = endY = wallOuterTop.sourcePoint.y + SEPARATOR4_Y_OFFSET;
                    isHorizontal = true;
                    break;
                case 'Separator2':
                    distance = Math.abs(movedConnector.targetPoint.y - movedConnector.sourcePoint.y);
                    startY = movedConnector.sourcePoint.y; endY = movedConnector.targetPoint.y;
                    startX = endX = wallOuterRight.sourcePoint.x + SEPARATOR2_X_OFFSET;
                    isHorizontal = false;
                    break;
                case 'RestroomSeparator':
                    distance = Math.abs(movedConnector.targetPoint.y - movedConnector.sourcePoint.y);
                    startY = movedConnector.sourcePoint.y; endY = movedConnector.targetPoint.y;
                    startX = endX = wallOuterRight.sourcePoint.x + RESTROOM_SEPARATOR_X_OFFSET;
                    isHorizontal = false;
                    break;
            }
            ...
            var extOffset = EXTENSION_LINE_OFFSET;
            if (isHorizontal) {
                extStartConnector.sourcePoint = { x: startX, y: startY - extOffset };
                extStartConnector.targetPoint = { x: startX, y: startY + extOffset };
                extEndConnector.sourcePoint = { x: endX, y: endY - extOffset };
                extEndConnector.targetPoint = { x: endX, y: endY + extOffset };
            } else {
                extStartConnector.sourcePoint = { x: startX - extOffset, y: startY };
                extStartConnector.targetPoint = { x: startX + extOffset, y: startY };
                extEndConnector.sourcePoint = { x: endX - extOffset, y: endY };
                extEndConnector.targetPoint = { x: endX + extOffset, y: endY };
            }
            diagram.dataBind();
        }
    }

    // Handler for the unit dropdown change event
    ...
    dropDownList.appendTo('#unit-dropdown');
    ...
    diagram = new ej.diagrams.Diagram({
        ...
        targetPointChange: onSourcePointChanged,
    ...
    diagram.appendTo('#distanceDiagram');
