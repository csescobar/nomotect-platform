...
/**
 * Angle Diagram sample - Updated to match TS/HTML changes
 */
    ...
    var suppressAngleChange = false; // prevents recursive change when we set value programmatically
    var angleNumeric;
    ...
    function initializeDiagram() {
        var nodes = [
            ...
            createImageNode('supportPost', 215, 185, 465, 565, 'https://ej2.syncfusion.com/javascript/demos/src/diagram/Images/angle/panelSupport.png'),
            createImageNode('solarPanelFrame', 260, 50, 478.25, 485, 'https://ej2.syncfusion.com/javascript/demos/src/diagram/Images/angle/solarPanel.png', solarData.currentAngle, (ej.diagrams.NodeConstraints.Default | ej.diagrams.NodeConstraints.ReadOnly) & ~ej.diagrams.NodeConstraints.Drag, 0.5, 0.8),
        ...
        diagram = new ej.diagrams.Diagram({
            ...
            scrollChange: () => {
                if (locationDropdown && locationDropdown.isPopupOpen) {
            ...
            pageSettings: {
                width: 1300,
                height: 820,
                background: { source: 'https://ej2.syncfusion.com/javascript/demos/src/diagram/Images/angle/background.png', scale: 'Meet' }
            }
        });
        diagram.appendTo('#diagram');
    }

    // Enforce angle constraints and keep UI in sync during rotation
    ...
    function initializeLocationDropdown() {
        var locationOptions = locationData.map(function (location) {
            return { text: location.name, value: location.name };
        });
        if (locationDropdown) {
            locationDropdown.destroy();
        }
        locationDropdown = new ej.dropdowns.DropDownList({
    ...
    function onLocationChanged(args) {
        ...
        if (location) {
            ...
            if (locationAngle <= 57) {
                solarData.currentAngle = 303 + locationAngle;  // 0..57 -> 303..360
            } else {
                solarData.currentAngle = locationAngle - 57;   // 58..100 -> 1..43
            }
    ...
    function initializeEfficiencyGauge() {
        ...
        efficiencyGauge = new ej.circulargauge.CircularGauge({
            width: '200px',
            height: '200px',
    ...
    function initializePerformanceMessage() {
        ...
        performanceMessage = new ej.notifications.Message({
    ...
    function initializeAngleControls() {
        ...
        if (inputEl) {
            ...
            inputEl.addEventListener('keydown', function (e) {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    inputEl.blur(); // triggers strictMode clamp if needed
        ...
        document.addEventListener('keydown', function onDocKeydown(e) {
            ...
            if (active && (active === inputEl || (inputEl && active === inputEl.closest('.e-input-group')))) return;
            var key = e.key;
    ...
    function angleCalculation() {
        ...
        if (normalizedAngle >= 303 && normalizedAngle <= 360) {
            solarData.intPanelAngleDeg = normalizedAngle - 303;       // -> 0..57
        } else if (normalizedAngle >= 0 && normalizedAngle <= 43) {
            solarData.intPanelAngleDeg = 57 + normalizedAngle;        // -> 57..100
        } else {
    ...
    <div style="text-align: center;">
        ...
        // Ensure the rotator is visible and selected
        setTimeout(function () {
    ...
    function getAngleDescription() {
        var relativeAngle = Math.abs(solarData.intPanelAngleDeg - 57);
        if (relativeAngle <= 5) return 'Horizontal'; // Tolerant horizontal
