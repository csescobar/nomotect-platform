this.default = function() {
    var prompts = [
        {
            ...
            suggestionData: ["I like the AI health assistant idea!", "How can we ensure privacy with this Assistant?"]
        },
        {
    ...
    var templateAiAssistView = new ej.interactivechat.AIAssistView({
        bannerViewTemplate: '#ai-bannerViewTemplate',
        promptItemTemplate: '#ai-promptItemTemplate',
        responseItemTemplate: '#ai-responseItemTemplate',
        promptSuggestionItemTemplate: '#ai-promptSuggestionItemTemplate',
        footerTemplate: '#ai-footerTemplate',
        ...
        promptRequest: function (args) {
            setTimeout(function () {
                ...
                if (foundPrompt) {
                    templateAiAssistView.addPromptResponse(foundPrompt.response);
                    templateAiAssistView.promptSuggestions = foundPrompt.suggestionData;
                }
                else {
                    templateAiAssistView.addPromptResponse("<p>For real-time prompt processing, connect the AIAssistView component to your preferred AI service, such as OpenAI or Azure Cognitive Services. Ensure you obtain the necessary API credentials to authenticate and enable seamless integration.</p>"); 
                    templateAiAssistView.promptSuggestions = suggestion;
                }
                var responseTemplate = document.querySelectorAll('.ai-response-template');
                var copyButton = responseTemplate[responseTemplate.length - 1].querySelector('.response-toolbar-icon');
    ...
    templateAiAssistView.appendTo('#template_AiAssistView');
    ...
    function GenerateContent() {
        if (!(textareaObj.value === null || textareaObj.value === '')) {
            ...
            textareaObj.value = '';
            templateAiAssistView.executePrompt(promptValue);   
        }
    }

    function copyClick(copyButtonEle) {
        const textToCopy = copyButtonEle.parentElement.parentElement.innerHTML;
