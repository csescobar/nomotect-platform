...
searchPopup = new ej.dropdowns.AutoComplete({
    ...
    filtering: function (e) {
        ...
        let value = [];
        for (let file of val) {
            if (file.doc.ignoreOnBuild !== true && !(ej.base.Browser.isDevice && file.doc.hideOnDevice === true)) {
...
function onsearchInputChange(e) {
    ...
    for (var j = 0; j < val.length; j++) {
        if (val[j].doc.ignoreOnBuild !== true && !(ej.base.Browser.isDevice && val[j].doc.hideOnDevice === true)) {
...
function getSampleList() {
    ...
    for (var i = 0; i < tempList.length; i++) {
        ...
        if (ej.base.Browser.isDevice && temp.hideOnDevice === true) {
            continue;
        }
        var data = new ej.data.DataManager(temp.samples);
        var query = new ej.data.Query().where('ignoreOnBuild', 'notEqual', true);
