...
import { getAzureOpenAIAssist} from './service.js';
...
export default {
  ...
  methods: {
    ...
    initializeApp() {
      ...
      this.$refs.aiToast.ej2Instances.show({
    ...
    async execute(prompt) {
      try {
        ...
        setTimeout(async () => {
          ...
          ai.addPromptResponse(marked.parse(text));
        ...
        const ai = this.$refs.aiAssistView?.ej2Instances;
        const fallback =
          "⚠️ Something went wrong while connecting to the Gemini service. Please check your API key.";
