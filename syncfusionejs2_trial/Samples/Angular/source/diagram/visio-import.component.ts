import { Component, ViewEncapsulation, ViewChild, Inject, ElementRef } from '@angular/core';
import {
...
@Component({
  ...
  templateUrl: 'visio-import.html',
  styleUrls: ['visio-import.css'],
...
export class VisioImportDiagramComponent {
  ...
  @ViewChild('vsdxInput', { static: false })
  public vsdxInput!: ElementRef<HTMLInputElement>;
  ...
  ngAfterViewInit(): void {
    // Wire up the native change event once the view is ready
    this.vsdxInput.nativeElement.addEventListener(
      'change',
      this.onVsdxInputChange
    );
  }

  private onVsdxInputChange = async (event: Event): Promise<void> => {
    const input = event.target as HTMLInputElement;
    const file = input.files?.[0];
    if (!file) return;

    await this.diagram.importFromVisio(file);
    this.diagram.width = '100%';
    this.diagram.height = '700px';

    // Clear so the same file selection will fire `change` again
    this.vsdxInput.nativeElement.value = '';
  };

  // Call this when user clicks "Import Visio (.vsdx)" in the toolbar
  private openVsdxPicker(): void {
    // Clear BEFORE opening the picker to ensure `change` fires for same file
    this.vsdxInput.nativeElement.value = '';
    this.vsdxInput.nativeElement.click();
  }
