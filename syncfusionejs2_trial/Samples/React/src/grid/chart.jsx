...
import { GridComponent, ColumnsDirective, ColumnDirective, Inject, ContextMenu, Filter, Sort, Freeze } from '@syncfusion/ej2-react-grids';
...
export class GridChartIntegration extends SampleBase {
    ...
    handleQueryCellInfo = (args) => {
        if (args.column && args.column.field === 'ProfitLoss') {
            const profit = args.data.ProfitLoss;
            if (profit < 0) {
    ...
    categoryTemplate = (props) => {
        return (<div className="e-category-info">
        <div dangerouslySetInnerHTML={{ __html: props.CategoryIcon }}/>
        <span>{props.Category}</span>
      </div>);
    };
    productTemplate = (props) => {
        const src = 'src/grid/images/product/' + props.Image + '.png';
        return (<div className="e-product-info">
        <img src={src} alt={props.Product}/>
    ...
    render() {
        ...
        <div className="control-section">
          <GridComponent id="GridChart" ref={grid => this.gridInstance = grid} dataSource={sales} rowHeight={60} height={500} allowFiltering={true} filterSettings={{ type: 'Menu' }} allowSorting={true} allowMultiSorting={true} allowSelection={true} gridLines="Both" selectionSettings={{ type: 'Multiple' }} contextMenuItems={this.contextMenuItems} contextMenuClick={this.handleContextMenuClick.bind(this)} queryCellInfo={this.handleQueryCellInfo.bind(this)} created={this.handleCreated.bind(this)}>
            <ColumnsDirective>
