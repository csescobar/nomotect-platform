...
var BatchEdit = /** @class */ (function (_super) {
    ...
    function BatchEdit() {
        ...
        _this.gridRef = null;
        _this.toolbarOptions = ['Add', 'Delete', 'Update', 'Cancel', 'Undo', 'Redo'];
        _this.filterSettings = { type: 'CheckBox' };
        _this.editSettings = { allowEditing: true, allowAdding: true, allowDeleting: true, mode: 'Batch', enableUndoRedo: true };
        _this.pageSettings = { pageCount: 5 };
        _this.selectionSettings = { mode: 'Cell', type: 'Multiple' };
        _this.getCategoryFromProduct = function (productName) {
            var item = data_1.inventoryStoreData.find(function (data) { return data.Product === productName; });
            return item ? item.Category : '';
        };
        _this.beforeBatchSave = function (args) {
            var changes = _this.gridRef.getBatchChanges();
            changes.addedRecords.forEach(function (row) {
                row.Category = _this.getCategoryFromProduct(row.Product);
            });
        };
        _this.cellEdit = function (args) {
            if (args.type === 'edit' && args.columnName === 'Product') {
                args.cancel = true;
            }
        };
        _this.categoryTemplate = function (props) {
            var category = (props.Category || _this.getCategoryFromProduct(props.Product) || '').toString();
    ...
    BatchEdit.prototype.render = function () {
        ...
        return (React.createElement("div", { className: 'control-pane' },
            React.createElement("div", { className: 'control-section' },
                React.createElement(ej2_react_grids_1.GridComponent, { id: "BatchEdit", dataSource: data_1.inventoryStoreData, pageSettings: this.pageSettings, allowSorting: true, toolbar: this.toolbarOptions, allowPaging: true, editSettings: this.editSettings, allowFiltering: true, filterSettings: this.filterSettings, selectionSettings: this.selectionSettings, height: 400, clipMode: 'EllipsisWithTooltip', cellEdit: this.cellEdit, beforeBatchSave: this.beforeBatchSave, ref: function (grid) { return _this.gridRef = grid; } },
                    React.createElement(ej2_react_grids_1.ColumnsDirective, null,
                        ...
                        React.createElement(ej2_react_grids_1.ColumnDirective, { field: 'Product', headerText: 'Product Name', width: '160', template: this.categoryTemplate, editType: 'dropdownedit', defaultValue: 'MacBook Pro' }),
