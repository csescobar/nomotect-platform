...
var multiselectComp;
var rootMap = new Map();
var cardRendering = function (cards) {
    var errorContent = document.querySelector('.tile_layout .row.error');
    if (cards.length > 0) {
        errorContent.style.display = 'none';
        cards.forEach(function (data, index) {
            var cardId = "card_sample_".concat(index + 1);
            var cardEle = document.getElementById(cardId);
            if (cardEle) {
                var root = rootMap.get(cardId);
                if (!root) {
...
var Tile = function () {
    ...
    var multiSelectFun = function (e) {
        ...
        keywords.forEach(function (key) {
            var dublicate = new ej2_data_1.DataManager(cardDa).executeLocal(new ej2_data_1.Query().where('cardImage.tag', 'Contains', key, true));
