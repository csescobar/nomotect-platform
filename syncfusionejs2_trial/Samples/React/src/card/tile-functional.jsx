...
const cardRendering = (cards) => {
    const errorContent = document.querySelector('.tile_layout .row.error');
    if (cards.length > 0) {
...
const Tile = () => {
    ...
    const multiSelectFun = (e) => {
        ...
        keywords.forEach((key) => {
            const dublicate = new DataManager(cardDa).executeLocal(new Query().where('cardImage.tag', 'Contains', key, true));
