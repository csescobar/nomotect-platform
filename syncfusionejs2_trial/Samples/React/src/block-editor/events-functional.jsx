...
const API = () => {
    ...
    const blockChangeEvent = (args) => {
        ...
        if (actionCounts.Insertion)
            messages.push(`${plural(actionCounts.Insertion, 'block')} inserted`);
        if (actionCounts.Deletion)
            messages.push(`${plural(actionCounts.Deletion, 'block')} deleted`);
        if (actionCounts.Moved)
            messages.push(`${plural(actionCounts.Moved, 'block')} moved`);
        if (actionCounts.Update)
            messages.push(`${plural(actionCounts.Update, 'block')} updated`);
