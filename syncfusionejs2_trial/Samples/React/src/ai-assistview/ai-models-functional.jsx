...
const AIAssistModels = () => {
    ...
    const [sidebarType, setSidebarType] = useState('Push');
    ...
    const updateConversationName = (prompt, convId) => {
    ...
    const checkAndUpdateLocalStorage = () => {
        ...
        if (!appData[selectedConvId])
            return;
        appData[selectedConvId].prompts = prompts.map((p) => ({
            prompt: p.prompt || '',
            response: p.response || ''
        }));
        localStorage.setItem('aiassist-model', JSON.stringify(appData));
    ...
    const streamAIResponse = async (fullResponse) => {
        let streamed = '';
    ...
    const listItemTemplate = (data) => (<div className="e-text-content" onClick={() => onItemSelect({ id: data.id, text: data.text })} title={data.text}>
      <span className="e-list-text">{data.text}</span>
      <span className="delete-icon e-icons e-trash" title="Delete Conversation" onClick={(e) => deleteConversation(data.id, e)}/>
    </div>);
    const models = [
        { id: 'gemini', name: 'Gemini 2.5 Flash' },
        { id: 'deepseek', name: 'DeepSeek-R1' },
    ...
    return (<div className="control-pane">
      <div className="control-section">
        <div className="ai-models">
          <AIAssistViewComponent id="aiAssistView" ref={(inst) => (aiAssistRef.current = inst)} bannerTemplate={bannerTemplate} enableAttachments={true} attachmentSettings={attachmentSettings} promptSuggestions={suggestions} promptRequest={promptRequest} showHeader={showHeader} enableStreaming={true} stopRespondingClick={stopRespondingClick} width="auto" footerToolbarSettings={footerToolbarSettings}>
            <div className="ai-assist-header">
              <ButtonComponent id="close" iconCss="e-icons e-menu" onClick={toggleSidebar}/>
              <DropDownListComponent id="ai-model-dropdown" dataSource={models} fields={{ text: 'name', value: 'id' }} value={selectedModel} change={onModelChange} popupHeight="200px" width="200px"/>
            </div>
          </AIAssistViewComponent>
        </div>

        <SidebarComponent id="assistantSidebar" ref={(inst) => (sidebarRef.current = inst)} target=".ai-models" width="250px" position="Left" enableDock={enableDock} dockSize={dockSize} enableGestures={enableGestures} type={sidebarType} showBackdrop={showBackdrop} closeOnDocumentClick={closeOnDocumentClick}>
          ...
          <div className="assistant-sidebar-content">
            ...
            <ListViewComponent id="conversation-list" ref={(inst) => (listRef.current = inst)} dataSource={listData} template={listItemTemplate}/>
          </div>
        </SidebarComponent>

        <ToastComponent ref={(inst) => (toastRef.current = inst)} position={{ X: 'right', Y: 'Top' }} target=".e-views" timeOut={1500} showCloseButton={true}/>
      </div>
