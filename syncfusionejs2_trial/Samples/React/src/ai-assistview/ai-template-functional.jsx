...
const Template = () => {
    ...
    const promptRequest = async (args) => {
        ...
        let response = 'For real-time prompt processing, connect the AI AssistView control to your preferred AI service, such as OpenAI or Azure Cognitive Services. Ensure you obtain the necessary API credentials to authenticate and enable seamless integration.';
        if (foundPrompt) {
            response = foundPrompt.response;
        }
        else {
            try {
                const aiResponse = await getAIResponse(args, abortControllerRef.current);
