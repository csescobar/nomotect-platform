...
export class NotionAIClone extends SampleBase {
    ...
    webIconCheckedState = true;
    editIconCheckedState = true;
    currentMode = 'Sidebar';
    lastActiveMode = 'Sidebar'; // tracks mode before hide, used by FAB to restore
    abortController;
    componentDidMount() {
    ...
    onSettingsDropdownCreated = () => {
        ...
        setTimeout(() => {
            settingsItems.forEach((item) => {
                ...
                new Switch({
                    checked: isChecked,
                    change: (args) => this.toggleSwitch(args, item.text)
                }).appendTo(switchElem);
            });
        }, 0);
    };
    onPromptRequest = async (args) => {
        ...
        if (this.assistInstance) {
            this.assistInstance.addPromptResponse(await getAIResponse(args, this.abortController));
    ...
    toolbarItemClicked = (args) => {
        if (args.item.iconCss === 'e-icons e-edit-notes') {
            ...
            this.currentMode = '';
            if (this.fabInstance)
                this.fabInstance.element.style.display = '';
        }
        ...
        switch (mode) {
            case 'Sidebar':
                if (this.sidebarInstance)
                    this.sidebarInstance.show();
                this.toggleBackgroundState(true);
                if (this.sidebarInstance && wrapper)
                    this.sidebarInstance.element.appendChild(wrapper);
                this.toggleIconClass('e-horizontal-line', 'e-chevron-right-double');
                break;
            case 'Floating':
    ...
    toggleIconClass = (selectorIconClass, replaceIconClass) => {
        const icon = this.assistInstance.toolbarHeader?.querySelector(`.${selectorIconClass}`);
        if (icon) {
    ...
    toggleSwitch = (args, text) => {
        const visibility = !args.checked;
        if (text === 'Can make changes') {
            this.editIconCheckedState = !visibility;
            const editIcon = this.assistInstance.footerToolbarEle?.element?.querySelector('.e-edit')?.closest('.e-toolbar-item');
            if (editIcon)
            ...
            const webIcon = this.assistInstance.footerToolbarEle?.element?.querySelector('.e-time-zone')?.closest('.e-toolbar-item');
            if (webIcon)
    ...
    persistActiveSession = () => {
        if (!this.activeSessionId || !this.assistInstance)
            return;
        const session = this.sessionChats.find((s) => s.id === this.activeSessionId);
        if (session)
            session.prompts = this.assistInstance.prompts;
    };
    ...
    loadSession = (sessionId) => {
        ...
        if (!session || !this.assistInstance)
            return;
