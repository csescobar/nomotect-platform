...
const NotionAIClone = () => {
    ...
    const modelIcons = data["modelIcons"];
    ...
    const webIconCheckedState = useRef(true);
    const editIconCheckedState = useRef(true);
    const currentMode = useRef('Sidebar');
    const lastActiveMode = useRef('Sidebar'); // tracks mode before hide, used by FAB to restore
    ...
    const onPromptRequest = useCallback(async (args) => {
        if (assistInstance.current)
            assistInstance.current.promptSuggestions = [];
        abortController.current = new AbortController();
        args.prompt = 'Based on the following notes content:\n\n' + document.querySelector('.notes-content')?.textContent + '\n\n---\n\nUser prompt: ' + args.prompt;
        if (assistInstance.current) {
            assistInstance.current.addPromptResponse(await getAIResponse(args, abortController.current));
    ...
    const toggleSwitch = useCallback((args, text) => {
        const visibility = !args.checked;
        if (text === 'Can make changes') {
            editIconCheckedState.current = !visibility;
            const editIcon = assistInstance.current.footerToolbarEle?.element?.querySelector('.e-edit')?.closest('.e-toolbar-item');
            if (editIcon)
            ...
            const webIcon = assistInstance.current.footerToolbarEle?.element?.querySelector('.e-time-zone')?.closest('.e-toolbar-item');
            if (webIcon)
                visibility ? webIcon.classList.remove('e-hidden') : webIcon.classList.add('e-hidden');
        }
    }, []);
    const onSettingsDropdownCreated = useCallback(() => {
        const settingsItems = [
        ...
        setTimeout(() => {
            settingsItems.forEach((item) => {
                ...
                new Switch({
                    checked: isChecked,
                    change: (args) => toggleSwitch(args, item.text)
                }).appendTo(switchElem);
            });
        }, 50);
    }, [toggleSwitch]);
    const updateModelIcon = useCallback((modelName) => {
    ...
    const onAssistViewCreated = useCallback(() => {
        ...
        screenDropdownRef.current = new DropDownButton({
            ...
            select: (args) => moveAssistview(args.item.text)
        });
    ...
    const footerToolbarItemClicked = useCallback((args) => {
    ...
    return (<div className='control-section'>
      <div className="notion-like-ui e-card">
        ...
        <DialogComponent id="dialogElem" target=".notes-page" position={{ X: 'right', Y: 0 }} animationSettings={{ effect: 'FadeZoom' }} width="500px" visible={false} cssClass="custom-dialog" ref={dialogInstance}>
        ...
        <SidebarComponent id="notionSidebar" target=".notes-page" width="400px" position="Right" animate={false} isOpen={false} ref={sidebarInstance}/>

        {/* Fullscreen container: wrapper DOM-moved here on Full screen mode */}
        <div id="fullscreenContainer"></div>

        <FabComponent id="fabElem" iconCss="e-icons e-magic-wand" target=".notes-page" ref={(fab) => { fabInstance.current = fab; }}/>
        <div id="toastTarget"></div>

        <ToastComponent content="Share chat option is clicked !" position={{ X: 'Right', Y: 'Top' }} showCloseButton={true} ref={toastInstance}/>
      </div>
