...
const AIAssistant = () => {
    ...
    const suggestion = data["assistantSuggestions"];
    ...
    const checkAndUpdateLocalStorage = (prompt) => {
        ...
        if (selectedConvId) {
            ...
            if (convData && convData.name === convData.name) {
                const dataSource = listViewRef.current?.dataSource;
                if (dataSource) {
                ...
                <div className="control-section">
                    <div>
                        <div className="ai-assistant">
                            <AIAssistViewComponent id="aiAssistView" ref={aiAssistViewRef} promptSuggestions={suggestion} toolbarSettings={toolbarSettings} enableAttachments={true} attachmentSettings={attachmentSettings} promptRequest={(args) => execute(args.prompt)} bannerTemplate={bannerTemplate} created={created}/>
                        </div>
                    </div>
                </div>
                <SidebarComponent id="assistantSidebar" ref={sidebarRef} width="260px" target=".ai-assistant" position="Left" enableDock={true} dockSize="75px" enableGestures={false}>
                    ...
                    <div className="assistant-sidebar-content">
                        <div className="assistant-listview-option" tabIndex={1}>
                            ...
                            <span className="e-icons e-refresh" title="Reset" id="resetButton" onClick={handleReset}></span>
                        </div>
                        <ListViewComponent id="assistant-listview-grp" ref={listViewRef} dataSource={listData} fields={listViewFields} template={listTemplate} select={onListViewSelect}/>
                    </div>
                </SidebarComponent>
