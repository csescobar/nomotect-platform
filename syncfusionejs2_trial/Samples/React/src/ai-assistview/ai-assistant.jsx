...
export class AIAssistant extends SampleBase {
    ...
    suggestion = data["assistantSuggestions"];
    ...
    // Templates
    listTemplate = (data) => {
        return (<div className="chat-item"><div className="chat-title">{data.text}</div></div>);
    };
    ...
    checkAndUpdateLocalStorage = (prompt) => {
        ...
        if (this.selectedConvId) {
            ...
            if (convData && convData.name === convData.name) {
                ...
                const dataSource = this.listViewRef.dataSource;
                if (dataSource) {
              ...
              <div className="control-section">
                  <div>
                      <div className="ai-assistant">
                          <AIAssistViewComponent id="aiAssistView" ref={aiassistview => (this.aiAssistViewRef = aiassistview)} promptSuggestions={this.suggestion} toolbarSettings={this.toolbarSettings} enableAttachments={true} attachmentSettings={this.attachmentSettings} promptRequest={(args) => this.execute(args.prompt)} bannerTemplate={this.bannerTemplate} created={this.created}/>
                      </div>
                  </div>
              </div>
              <SidebarComponent id="assistantSidebar" ref={sidebar => (this.sidebarRef = sidebar)} width="260px" target=".ai-assistant" position="Left" enableDock={true} dockSize="75px" enableGestures={false}>
                  ...
                  <div className="assistant-sidebar-content">
                      <div className="assistant-listview-option" tabIndex={1}>
                          ...
                          <span className="e-icons e-refresh" title="Reset" id="resetButton" onClick={this.handleReset}></span>
                      </div>
                      <ListViewComponent id="assistant-listview-grp" ref={listView => (this.listViewRef = listView)} dataSource={this.listDatas} fields={this.listViewFields} template={this.listTemplate} select={this.onListViewSelect}/>
                  </div>
              </SidebarComponent>
