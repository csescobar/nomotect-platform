...
export class GeminiClone extends SampleBase {
    prompts = data["geminiPromptResponseData"];
    assistInstance;
    geminiContainer;
    modelDropdown;
    isFirstPrompt = true;
    ...
    promptRequest = async (args) => {
        ...
        const response = foundPrompt ? foundPrompt.response : await getAIResponse(args, this.abortController);
    ...
    toggleButtons = () => {
        const sendBtn = this.assistInstance.element.querySelector('.e-assist-send')?.parentElement;
        const audioBtn = this.assistInstance.element.querySelector('.e-assistview-speech-to-text');
        const hasPrompt = this.assistInstance.prompt &&
