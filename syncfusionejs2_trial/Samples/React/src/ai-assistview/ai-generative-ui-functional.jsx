...
const GenerativeUI = () => {
    ...
    const generativeSuggestions = data["generativeSuggestions"];
    const toolSystemPrompt = data["toolSystemPrompt"];
    const promptsData = data["promptsData"];
    const currentChartConfig = useRef({});
    const currentGridConfig = useRef({});
    ...
    const ChartTemplate = (args) => (<ChartComponent id="generativeChart" primaryXAxis={{
      ...
      <SeriesCollectionDirective>
        <SeriesDirective dataSource={currentChartConfig.current.dataSource} xName={currentChartConfig.current.xField} yName={currentChartConfig.current.yField} type={currentChartConfig.current.chartType} marker={{ visible: true }}/>
      </SeriesCollectionDirective>
    </ChartComponent>);
    const GridTemplate = (args) => (<GridComponent dataSource={currentGridConfig.current.data} allowPaging={false} pageSettings={{ pageSize: 8 }} width="fit-content">
      <ColumnsDirective>
        {currentGridConfig.current.columns?.map((column, index) => (<ColumnDirective key={index} field={column.field} headerText={column.headerText} width={column.width} textAlign={column.textAlign}/>))}
      </ColumnsDirective>
