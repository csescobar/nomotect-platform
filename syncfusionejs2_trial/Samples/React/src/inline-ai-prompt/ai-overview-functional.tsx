...
const Overview = () => {
  ...
  const inlinePromptRef = React.useRef<InlineAIPromptComponent | null>(null);
  ...
  const currentHoveredParagraphRef = React.useRef<HTMLParagraphElement | null>(null);
  ...
  const currentHoveredParagraphSyncRef = React.useRef<HTMLParagraphElement | null>(null);

  const attachHoverEvent = React.useCallback((paragraph: HTMLParagraphElement): void => {
  ...
  const updateSparkleButtonPosition = React.useCallback((paragraph: HTMLParagraphElement): void => {
  ...
  const handleResponseItemSelect = React.useCallback((args: any): void => {
    if (args.command.label === 'Accept') {
      if (isGlobalRequest && emailContentRef.current) {
        const lastPrompt = inlinePromptRef.current?.prompts[
          (inlinePromptRef.current?.prompts?.length || 1) - 1
        ] as any;
        if (lastPrompt) {
