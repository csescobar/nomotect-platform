...
const Overview = () => {
    ...
    const inlinePromptRef = React.useRef(null);
    ...
    const attachHoverEvent = React.useCallback((paragraph) => {
    ...
    const updateSparkleButtonPosition = React.useCallback((paragraph) => {
    ...
    const handleResponseItemSelect = React.useCallback((args) => {
        if (args.command.label === 'Accept') {
            if (isGlobalRequest && emailContentRef.current) {
                const lastPrompt = inlinePromptRef.current?.prompts[(inlinePromptRef.current?.prompts?.length || 1) - 1];
                if (lastPrompt) {
    ...
    return (<div className="control-pane">
      <div className="control-section">
        <div className="overview-inlineAIPrompt">
            ...
            <div className="email-composer e-card">
              ...
              <div className="email-field-vertical e-card-content">
                <div className="message-header">
                  <ButtonComponent ref={aiAssistantBtnRef} id="aiAssistantBtn" iconCss="e-icons e-ai-chat" isPrimary={true} onClick={handleAIAssistantClick}>
                      AI Assistant
                    </ButtonComponent>
                </div>
                ...
                <ButtonComponent ref={sparkleButtonRef} id="sparkleBtn" iconCss="e-icons e-ai-chat" isPrimary={true} style={{ display: 'none' }} onClick={handleSparkleClick}/>
              </div>

              <div className="email-actions e-card-content">
                <ButtonComponent id="sendEmailBtn" cssClass="e-primary" onClick={handleSendEmail}>
                  Send Email
                </ButtonComponent>
              </div>
            </div>
          ...
          <InlineAIPromptComponent ref={inlinePromptRef} id="inlinePrompt" commandSettings={commandSettings} relateTo="#emailContent" promptRequest={handlePromptRequest} responseSettings={responseSettings} open={() => setIsPopupOpen(true)} close={() => {
