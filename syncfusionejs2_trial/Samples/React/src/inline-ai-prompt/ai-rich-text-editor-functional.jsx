 ...
 */
const RichTextEditorFunctional = () => {
    useEffect(() => {
        updateSampleSection();
    }, []);
    const promptRef = React.useRef(null);
    const rteEditorRef = React.useRef(null);
    const rteInstanceRef = React.useRef(null);
    const abortControllerRef = React.useRef();
    const rteSelectionRef = React.useRef(null);
    const markdownConverterRef = React.useRef(null);
    const selectedSpanRef = React.useRef(null);
    ...
    const handleToolbarClick = (args) => {
        if (args.item.prefixIcon === 'e-icons e-ai-chat') {
            if (!rteInstanceRef.current || !rteSelectionRef.current)
                return;
            const range = rteSelectionRef.current.getRange(document);
            const relateToEl = range.endContainer && range.endContainer.parentElement;
            const selectedText = rteInstanceRef.current.getSelection();
    ...
    return (<div className="control-pane">
      <div className="control-section">
        <div className="rte-integration-container">
          ...
          <InlineAIPromptComponent ref={promptRef} id="inline-ai-prompt" commandSettings={commandSettings} inlineToolbarSettings={inlineToolbarSettings} responseMode="Inline" promptRequest={handlePromptRequest} close={handlePromptClose} responseSettings={responseSettings}/>
        </div>
      </div>

      <div id="action-description">
        <p>
          This sample demonstrates the integration of the Inline AI Prompt component with the Syncfusion Rich Text
      ...
      <div id="description">
        <p>
          In this example, the Inline AI Prompt component is seamlessly integrated with the Rich Text Editor,
