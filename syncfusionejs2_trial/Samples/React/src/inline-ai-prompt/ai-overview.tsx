...
import { InlineAIPromptComponent, PromptRequestEventArgs } from '@syncfusion/ej2-react-interactive-chat';
...
export class Overview extends SampleBase<{}, InlinePromptState> {
  private inlinePromptRef: React.RefObject<InlineAIPromptComponent> = React.createRef();
  ...
  private currentHoveredParagraph: HTMLParagraphElement | null = null;
  // Synchronous ref to reliably track hovered paragraph without state delays
  private currentHoveredParagraphSyncRef: HTMLParagraphElement | null = null;
  ...
  private attachHoverEventsToChildren = (container: HTMLElement): void => {
    ...
    paragraphs.forEach((para) => {
      const paragraph = para as HTMLParagraphElement;
      // Remove old listener first to avoid duplicates
      const existingListener = (paragraph as any).__hoverListener;
      if (existingListener) {
      ...
      (paragraph as any).__hoverListener = handleMouseEnter;
      paragraph.addEventListener('mouseenter', handleMouseEnter);
    });
  };

  private attachHoverEvent = (paragraph: HTMLParagraphElement): void => {
    // Remove old listener first to avoid duplicates
    const existingListener = (paragraph as any).__hoverListener;
    if (existingListener) {
      paragraph.removeEventListener('mouseenter', existingListener);
  ...
  private updateSparkleButtonPosition = (paragraph: HTMLParagraphElement): void => {
  ...
  private handleAIAssistantClick = (): void => {
    if (this.inlinePromptRef.current && this.aiAssistantBtnRef.current) {
      ...
      this.inlinePromptRef.current.relateTo = (this.aiAssistantBtnRef.current as any).element as HTMLElement;
      this.inlinePromptRef.current.dataBind();
      this.inlinePromptRef.current.showPopup();
    }
  };
  ...
  private handleResponseItemSelect = (args: any): void => {
    if (args.command.label === 'Accept') {
      if (this.state.isGlobalRequest && this.emailContentRef.current) {
        const lastPrompt = this.inlinePromptRef.current?.prompts[
          (this.inlinePromptRef.current?.prompts?.length || 1) - 1
        ] as any;
        if (lastPrompt) {
