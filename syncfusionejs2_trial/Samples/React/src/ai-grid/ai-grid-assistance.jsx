...
function GridAIAssistance() {
    ...
    const toolbarClick = (args) => {
        if (args.item.id === 'ai-assist-btn') {
            const gridRect = gridInstance.current.element.getBoundingClientRect();
    ...
    function clicked(text) {
        ...
        showSpinner(document.getElementById('ai-grid'));
        ...
        aiOutput.then((result) => {
            if (!result) {
                ...
                hideSpinner(document.getElementById('ai-grid'));
            ...
            try {
                ...
                if (queryOps.where) {
                    if (Array.isArray(queryOps.where)) {
                        queryOps.where.forEach(where => {
                            query.where(where.field, where.operator || 'equal', where.value, typeof where.value === 'string' ? true : where.ignoreCase);
                        });
                    }
                    else {
                        query.where(queryOps.where.field, queryOps.where.operator || 'equal', queryOps.where.value, typeof queryOps.where.value === 'string' ? true : queryOps.where.ignoreCase);
                    }
                }
                ...
                if (queryOps.search && queryOps.search && Array.isArray(queryOps.search)) {
                    queryOps.search.forEach(search => {
                        query = query.search(search.key, search.fields, search.operator || 'contains', true, search.ignoreAccent ?? false);
                    });
                }
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                ...
                <GridComponent ref={gridInstance} id="ai-grid" height={650} dataSource={gadgetsPurchaseData} toolbar={toolbarOptions} toolbarClick={toolbarClick}>
                    <ColumnsDirective>
                        <ColumnDirective field="transactionId" headerText="Transaction ID" width="130" template={(data) => (<a>{data.transactionId}</a>)}/>
                        <ColumnDirective field="customerDetails" headerText="Customer Name" width="220" textAlign="Center" template={(data) => (<div>
                                    <p>{data.customerDetails.name}</p>
                                    <p className="email">{data.customerDetails.email}</p>
                                </div>)}/>
                        <ColumnDirective field="product" headerText="Product" width="208" textAlign="Left" template={(data) => (<div className='product-items'>
                        ...
                        <ColumnDirective field="status" headerText="Status" width="93" textAlign='Right' template={(data) => (<div>
