...
export class CompareDocuments extends SampleBase<{}, {
  ...
  private editorRef1: DocumentEditorContainerComponent;
  private editorRef2: DocumentEditorContainerComponent;
  private serviceUrl: string = 'https://services.syncfusion.com/react/production/api/documenteditor/';
  ...
  public rendereComplete(): void {
    ...
    if (this.editorRef1 && this.editorRef2) {
      this.editorRef1.documentEditor.viewChange = (): void => {
      ...
      this.editorRef2.documentEditor.viewChange = (): void => {
  ...
  private async openFileInEditor(
    ...
    if (formatType === '.sfdt') {
      ...
      reader.onload = (e): void => {
        const docData = e.target?.result as string | undefined;
  ...
  private async loadComparedDocumentAndOpen(
    editorRef: DocumentEditorContainerComponent,
    originalFile: File,
    revisedFile: File
  ): Promise<void> {
  ...
  private onCompare = async (): Promise<void> => {
    this.setState({ compareClicked: false });
    setTimeout(async (): Promise<void> => {
      this.setState({ compareClicked: true });
      if (this.state.showResult) {
        if (this.state.originalFile) await this.openFileInEditor(this.state.originalFile, this.editorRef1);
        ...
        if (this.state.originalFile) await this.openFileInEditor(this.state.originalFile, this.editorRef1);
        if (this.state.revisedFile) await this.openFileInEditor(this.state.revisedFile, this.editorRef2);
      }
    }, 0);
  }
  ...
  private handleOriginalFileChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    this.setState({
  ...
  private handleRevisedFileChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    this.setState({
  ...
  private handleShowResultChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'row', gap: 60 }}>
            <label>
              ...
              <input
                ...
                onChange={this.handleOriginalFileChange}
              />
            </label>
            <label>
              Revised Document :
              <input
                ...
                type="file"
                accept=".docx,.dotx,.docm,.dotm,.doc,.dot,.rtf,.txt,.xml,.html,.sfdt"
                onChange={this.handleRevisedFileChange}
              />
            </label>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 20 }}>
            ...
            <ButtonComponent
              ...
              disabled={!this.state.originalFile || !this.state.revisedFile}
            ...
            <ButtonComponent
              ...
              disabled={!this.state.compareClicked}
            >
              Download Result
            </ButtonComponent>
          </div>
        ...
        >
          <div style={{ width: '50%', minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <DocumentEditorContainerComponent
              id="editor1"
              ref={(scope) => { this.editorRef1 = scope; }}
          ...
          <div style={{ width: '50%', minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <DocumentEditorContainerComponent
              id="editor2"
              ref={(scope) => { this.editorRef2 = scope; }}
              serviceUrl={this.serviceUrl}
              height={'100%'}
              width={'100%'}
              enableToolbar={false}
