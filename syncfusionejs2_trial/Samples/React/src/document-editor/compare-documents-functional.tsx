...
const CompareDocumentsFunctional = () => {
  const editorRef1 = useRef<DocumentEditorContainerComponent | null>(null);
  const editorRef2 = useRef<DocumentEditorContainerComponent | null>(null);
  ...
  let serviceUrl: string = 'https://services.syncfusion.com/react/production/api/documenteditor/';
  ...
  const isSupportedFormatType = (formatType: string): boolean => {
    switch (formatType) {
      case '.docx':
      case '.dotx':
      case '.docm':
  ...
  async function loadComparedDocumentAndOpen(
    editorRef2: React.RefObject<DocumentEditorContainerComponent | null>,
    originalFile: File,
    revisedFile: File
  ): Promise<void> {
    const formData = new FormData();
  ...
  const onCompare = async (): Promise<void> => {
    ...
    setTimeout(async (): Promise<void> => {
      ...
      if (showResult) {
        if (originalFile) await openFileInEditor(originalFile, editorRef1);
        if (originalFile && revisedFile) {
          await loadComparedDocumentAndOpen(editorRef2, originalFile, revisedFile);
        }
      } else {
        if (originalFile) await openFileInEditor(originalFile, editorRef1);
        if (revisedFile) await openFileInEditor(revisedFile, editorRef2);
      }
    }, 0);
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 60 }}>
          <label>
            ...
            <input
              ...
              onChange={e => setOriginalFile(e.target.files ? e.target.files[0] : null)}
            />
          </label>
          <label>
            Revised Document :
            <input
              ...
              type="file"
              accept=".docx,.dotx,.docm,.dotm,.doc,.dot,.rtf,.txt,.xml,.html,.sfdt"
              onChange={e => setRevisedFile(e.target.files ? e.target.files[0] : null)}
            />
          </label>
        </div>
