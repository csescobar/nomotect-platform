...
import { GanttComponent, TaskFieldsModel, ColumnsDirective, ColumnDirective, Selection, Inject, LabelSettingsModel, TimelineSettingsModel, SplitterSettingsModel, TooltipSettingsModel } from '@syncfusion/ej2-react-gantt';
...
const Baseline = () => {
  ...
  const labelSettings: LabelSettingsModel = {
    rightLabel: rightLabelTemplate as any
  };
  ...
  const projectStartDate: Date = new Date('07/02/2025');
  const projectEndDate: Date = new Date('09/15/2025');
  ...
  return (
    <div className='control-pane'>
      <div className='control-section'>
        <GanttComponent id='Baseline' ref={gantt => ganttInstance = gantt} dataSource={baselineData} renderBaseline={true} labelSettings={labelSettings}
          treeColumnIndex={0} allowSelection={true} projectStartDate={projectStartDate} projectEndDate={projectEndDate} parentTaskbarTemplate={ParentTemplate}
        ...
        <div style={{ float: 'right', margin: '10px' }}>Source:
          <a href="https://en.wikipedia.org/wiki/Service_(motor_vehicle)"
            target='_blank'>https://en.wikipedia.org/wiki/Service_(motor_vehicle)</a>
        </div>
      </div>
      ...
      <div id="description">
        <p>In this demo sample, the baseline feature in the React Gantt Chart helps visualize the variance between planned and actual task schedules. Baselines provide a clear reference for tracking project deviations and are rendered for all task types including child tasks, parent tasks, and milestones.</p>
