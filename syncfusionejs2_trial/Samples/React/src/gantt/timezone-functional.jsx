...
import { GanttComponent, Inject, Selection, Toolbar, ColumnDirective, ColumnsDirective } from '@syncfusion/ej2-react-gantt';
...
const Timezone = () => {
    ...
    const dayWorkingTime = [{ from: 0, to: 23 }];
    const getTimeZonesWithOffsets = () => {
        ...
        return uniqueZones.map(function (tz) {
            ...
            let offsetPart = parts.filter(function (part) { return part.type === 'timeZoneName'; })[0];
    ...
    const timelineData = ['Day', 'Week', 'Month'];
    const timelineUnit = (args) => {
    ...
    const toolbar = [
        ...
        {
            align: 'Left',
            template: () => (<ComboBoxComponent id="timezonelist" value="UTC (UTC+00:00)" placeholder="Select Time Zone" change={timeZoneList} allowFiltering={true} filterType="Contains" dataSource={getTimeZonesWithOffsets()} width="300px" popupWidth="350px" popupHeight="350px" fields={{ value: 'id', text: 'text' }}/>),
        },
        {
            align: 'Right',
            template: () => (<ButtonComponent id="previous-timespan" cssClass="previous-timespan" onClick={getPreviousTimeSpan} iconCss='e-icons e-chevron-left-fill'/>)
        },
        {
            type: 'Separator',
            align: 'Right'
        ...
        {
            align: 'Right',
            template: () => (<DropDownListComponent id="timeline" value={timelineSettings.topTier.unit} placeholder="Select timeline" change={timelineUnit} dataSource={timelineData} width='100px'/>)
        },
        {
            type: 'Separator',
            align: 'Right'
        ...
        {
            align: 'Right',
            template: () => (<ButtonComponent id="next-timespan" cssClass="next-timespan" onClick={getNextTimeSpan} iconCss='e-icons e-chevron-right-fill'/>)
        }
    ];
