...
import { GanttComponent, Inject, Edit, Selection, DayMarkers, ColumnMenu, Toolbar, Filter, Reorder, Sort, Resize, ColumnsDirective, ColumnDirective, EditDialogFieldsDirective, EditDialogFieldDirective, AddDialogFieldsDirective, AddDialogFieldDirective } from '@syncfusion/ej2-react-gantt';
...
const GanttDialogEditing = () => {
    ...
    const actionComplete = (args) => {
        if (args.requestType === 'openAddDialog' || args.requestType === 'openEditDialog') {
            const gantt = document.getElementById('Dialog')?.ej2_instances[0];
            const tabObj = document.getElementById(`${gantt.element.id}_Tab`)?.ej2_instances[0];
            const selectedTab = tabObj.selected;
            if (tabObj) {
                tabObj.selected = function (args) {
                    if (args.selectedIndex === 1) {
                        const gridObj = document.getElementById(`${gantt.element.id}DependencyTabContainer`)?.ej2_instances[0];
                        if (gridObj) {
    ...
    return (<div className='control-pane'>
      <div className='control-section'>
        <GanttComponent id="Dialog" ref={ganttInstance} dataSource={dialogData} taskFields={taskFields} resourceFields={resourceFields} resources={dataResources} editSettings={editSettings} toolbar={toolbar} renderBaseline={true} treeColumnIndex={1} taskMode="Custom" allowSelection={true} showColumnMenu={true} splitterSettings={splitterSettings} gridLines="Both" highlightWeekends={true} timelineSettings={timelineSettings} labelSettings={labelSettings} allowResizing={true} taskbarHeight={25} rowHeight={46} height="650px" projectStartDate={new Date('03/30/2025')} projectEndDate={new Date('07/19/2025')} actionComplete={actionComplete}>
          <ColumnsDirective>
          ...
          <AddDialogFieldsDirective>
            <AddDialogFieldDirective type="General" fields={['TaskID', 'TaskName', 'StartDate', 'Duration', 'EndDate', 'Progress']}/>
            <AddDialogFieldDirective type="Dependency"/>
            <AddDialogFieldDirective type="Resources" additionalParams={{
            allowFiltering: true,
            allowSorting: true,
            allowResizing: true,
            showColumnMenu: true,
            ...
            <AddDialogFieldDirective type="Advanced" fields={['ConstraintType', 'ConstraintDate', 'isManual', 'Work']}/>
          </AddDialogFieldsDirective>
          <EditDialogFieldsDirective>
            <EditDialogFieldDirective type="General" fields={['TaskID', 'TaskName', 'StartDate', 'Duration', 'EndDate', 'Progress']}/>
            <EditDialogFieldDirective type="Dependency" additionalParams={{
            allowSorting: true,
            toolbar: ['Add', 'Edit', 'Delete', 'Search'],
            editSettings: { allowEditing: true, allowAdding: true, allowDeleting: true },
            searchSettings: { fields: ['name'], ignoreCase: true },
        }}/>
            <EditDialogFieldDirective type="Resources" additionalParams={{
            allowFiltering: true,
            allowSorting: true,
            allowResizing: true,
            showColumnMenu: true,
            ...
            <EditDialogFieldDirective type="Segments" additionalParams={{
            allowFiltering: true,
            allowSorting: true,
            ...
            <EditDialogFieldDirective type="Advanced" fields={['ConstraintType', 'ConstraintDate', 'isManual', 'Work']}/>
          </EditDialogFieldsDirective>
