...
import { MultiSelectComponent, CheckBoxSelection, DropDownListComponent } from '@syncfusion/ej2-react-dropdowns';
import { AccumulationChartComponent, AccumulationSeriesCollectionDirective, AccumulationSeriesDirective, AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel, ChartComponent, SeriesCollectionDirective, SeriesDirective, Category, StackingColumnSeries, Legend, Tooltip, Inject, ColumnSeries, BarSeries, DataLabel, SparklineComponent, SparklineTooltip, SplineAreaSeries, Crosshair } from '@syncfusion/ej2-react-charts';
import { GridComponent, ColumnsDirective, ColumnDirective, Sort as GridSort, Page as GridPage, Filter as GridFilter, Inject as GridInject, Resize, Group, RowDD, Toolbar, ExcelExport, PdfExport } from '@syncfusion/ej2-react-grids';
import { CircularGaugeComponent, AxesDirective as CircularGaugeAxesDirective, AxisDirective as CircularGaugeAxisDirective, PointersDirective, PointerDirective, RangesDirective, RangeDirective, Annotations, AnnotationDirective, AnnotationsDirective, GaugeTooltip, Legend as CircularGaugeLegend } from '@syncfusion/ej2-react-circulargauge';
import { MapsComponent, LayersDirective, LayerDirective, Legend as MapsLegend, Inject as MapsInject, MapsTooltip, MarkersDirective, MarkerDirective, Zoom as MapsZoom, Selection as MapsSelection, Highlight as MapsHighlight } from '@syncfusion/ej2-react-maps';
import { KanbanComponent, ColumnsDirective as KanbanColumnsDirective, ColumnDirective as KanbanColumnDirective } from '@syncfusion/ej2-react-kanban';
...
const InsuranceData = InsuranceDatasource;
MultiSelectComponent.Inject(CheckBoxSelection);
setCulture('en');
setCurrencyCode('USD');
const POLICY_TYPES = ['Travel', 'Home', 'Life', 'Health', 'Disability'];
const ALLOWED_YEARS = [2025, 2024, 2023, 2022];
...
const DeltaBadge = ({ delta, goodWhenUp = true }) => {
    ...
    return (<span style={getBadgeStyle(good ? 'good' : 'bad')}>
...
const fmtCurrency = (n) => currencyCompact.format(Number(n || 0));
const Dashboard1 = ({ selectedYears, onChangeYears }) => {
    const OverviewRef = React.useRef(null);
    const ProfitvsExpenseRef = React.useRef(null);
    const AssuredAmountRef = React.useRef(null);
    const APERef = React.useRef(null);
    const PolicyRatioRef = React.useRef(null);
    const PolicyHolderRef = React.useRef(null);
    const ReferralRef = React.useRef(null);
    const PoliciesSoldRef = React.useRef(null);
    const yearOptions = React.useMemo(() => ALLOWED_YEARS.map(y => ({ id: String(y), year: String(y) })), []);
    const currentYearOverview = React.useMemo(() => (selectedYears.length ? Math.max(...selectedYears) : ALLOWED_YEARS[0]), [selectedYears]);
    ...
    const kpis = React.useMemo(() => {
        const data = InsuranceData.overview; // source: default-datasource.json
    ...
    const onCurrencyAxisLabel = (args) => {
        if (args?.axis?.orientation === 'Vertical') {
            args.text = fmtCurrency(Number(args.value || 0));
        }
    };
    const onNumericAxisLabel = React.useCallback((args) => {
    ...
    const PoliciesSold = () => {
        const data = React.useMemo(() => {
            const overviewdata = InsuranceData.overview ?? [];
        ...
        return (<div className="kpi-totalcard">
        ...
        <div className="spark-content" style={{ width: '50%', height: '50%' }}>
          <div style={{ marginTop: 8, width: '100%' }}>
            <SparklineComponent id="policies-sold-sparkline" ref={PoliciesSoldRef} type="Line" dataSource={data} xName="x" yName="y" valueType="Category" width="100%" height="55px" lineWidth={2} fill="#05B3DA" markerSettings={{ visible: ['All'], size: 2, fill: '#05B3DA' }} tooltipSettings={{ visible: true, format: 'Year: ${x}<br/>Policies: ${y}' }}>
    ...
    const { assuredDelta, totalAmountDelta, avgPremiumDelta } = React.useMemo(() => {
        ...
        const sumFor = (yr, key) => overviewdata.reduce((s, r) => (Number(r.year ?? r.Year) === yr ? s + (Number(r[key]) || 0) : s), 0);
        const countFor = (yr) => overviewdata.reduce((s, r) => (Number(r.year ?? r.Year) === yr ? s + 1 : s), 0);
    ...
    const AssuredAmount = () => (<div className="insurance-card">
    ...
    const TotalAmount = () => (<div className="insurance-card">
    ...
    const AverageAmount = () => (<div className="insurance-card">
    ...
    const policyRateData = React.useMemo(() => {
        const policydata = InsuranceData.policydata ?? [];
    ...
    const onPercentageTextRender = (args) => {
        ...
        if (pct != null)
            args.text = `${pct.toFixed(2)}%`; // percentage only
    };
    const PolicyRatePieChart = () => (<div style={{ height: '100%', width: '100%' }}>
      {policyRateData.length === 0 ? (<div style={{ padding: 12 }}>No policy rate data for the selected year.</div>) : (<AccumulationChartComponent id='policyratio-chart' ref={PolicyRatioRef} legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} tooltip={{ enable: true, format: '<b>${point.x}</b><br>Percentage: <b>${point.percentage}%</b>', header: '', enableHighlight: true }} load={onAccumulationLoad} textRender={onPercentageTextRender} enableSmartLabels={true}>
          <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective dataSource={policyRateData} xName='x' yName='y' radius='70%' border={{ color: '#FFFFFF', width: 1 }} dataLabel={{ visible: true, position: 'Outside', name: 'text' }} explode explodeIndex={0} explodeOffset='10%' palettes={pieThemePalette} animation={{ enable: false }}/>
          </AccumulationSeriesCollectionDirective>
        </AccumulationChartComponent>)}
    </div>);
    const profitExpenseData = React.useMemo(() => {
        const policydata = InsuranceData.policydata ?? [];
        if (!policydata.length)
            return [];
        const currentYear = selectedYears.length ? Math.max(...selectedYears) : undefined;
        ...
        const addFromColumns = (acc, t, row) => {
    ...
    const ProfitVsExpenseChart = () => (<div style={{ width: '100%', height: '100%', padding: 8 }}>
      {profitExpenseData.length === 0 ? (<div style={{ padding: 12 }}>No profit/expense data for the selected year.</div>) : (<ChartComponent id="profit-expense-chart" ref={ProfitvsExpenseRef} primaryXAxis={{
                valueType: 'Category',
          ...
          <SeriesCollectionDirective>
            <SeriesDirective type="Column" name="Profit" dataSource={profitExpenseData} xName="policy" yName="profit" columnSpacing={0.2} marker={{ visible: true, dataLabel: { visible: true } }} fill='#FF7000' animation={{ enable: false }}/>
            <SeriesDirective type="Column" name="Expense" dataSource={profitExpenseData} xName="policy" yName="expense" columnSpacing={0.2} marker={{ visible: true, dataLabel: { visible: true } }} fill='#FFB200' animation={{ enable: false }}/>
          </SeriesCollectionDirective>
        </ChartComponent>)}
    </div>);
    const AssuredAmountByPolicyTypeChart = () => {
        // Drilldown state for Gender donut
        const [genderDrill, setGenderDrill] = React.useState({ open: false });
        ...
        const genderDonutData = React.useMemo(() => {
            ...
            for (const r of overview) {
                ...
                if (pt !== wanted)
                    continue;
                const gRaw = r.HolderGender ?? r.holderGender ??
        ...
        const onAssuredChartClick = React.useCallback((args) => {
            ...
            if (policy)
                setGenderDrill({ open: true, policy });
        }, [assuredByPolicyTypeData]);
        return (<div style={{ width: '100%', height: '100%', padding: 8 }}>
        {genderDrill.open && (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: 10 }}>
        ...
        {!genderDrill.open ? (assuredByPolicyTypeData.length === 0 ? (<div style={{ padding: 12 }}>No assured amount data for the selected year(s).</div>) : (<ChartComponent id="assured-by-type-chart" ref={AssuredAmountRef} primaryXAxis={{
                    valueType: 'Category',
                    majorGridLines: { width: 0 },
                    labelIntersectAction: 'Trim'
                }} primaryYAxis={{
                    ...
                    lineStyle: { width: 0 },
                    majorTickLines: { width: 0 }
                }} chartArea={{ border: { width: 0 } }} legendSettings={{ visible: false }} tooltip={{ enable: true }} axisLabelRender={onCurrencyAxisLabel} tooltipRender={onCurrencyTooltip} textRender={onCurrencyDataLabel} load={onChartLoad} pointClick={(args) => {
              ...
              <SeriesCollectionDirective>
                <SeriesDirective type="Column" name="Assured Amount" dataSource={assuredByPolicyTypeData} xName="policy" yName="amount" columnSpacing={0.2} cornerRadius={{ topLeft: 4, topRight: 4 }} fill="#f6b67a" marker={{ visible: true, dataLabel: { visible: true } }} animation={{ enable: false }}/>
              </SeriesCollectionDirective>
            </ChartComponent>)) : (
            // Drilldown view: donut (Male/Female only)
            <>
            {genderDonutData.length === 0 ? (<div style={{ padding: 12 }}>No gender data found.</div>) : (<div style={{ width: '100%', height: '90%' }}>
                <AccumulationChartComponent id="gender-donut-inline" legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} load={onAccumulationLoad} tooltip={{ enable: true, format: '<b>${point.x}</b><br/>Count: <b>${point.y}</b>', header: '' }}>
                  <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
                  <AccumulationSeriesCollectionDirective>
                    <AccumulationSeriesDirective dataSource={genderDonutData} xName='x' yName='y' innerRadius='60%' radius='70%' dataLabel={{ visible: true, position: 'Outside', name: 'text' }} borderRadius={10} border={{ width: 4, color: '#ffffff' }} palettes={donutPaletteColors} animation={{ enable: false }}/>
                  </AccumulationSeriesCollectionDirective>
                </AccumulationChartComponent>
              </div>)}
          </>)}
    ...
    const PolicyHolderAgePie = () => (<div style={{ height: '100%', width: '100%' }}>
      {ageDistributionData.length === 0 ? (<div style={{ padding: 12 }}>No age data available.</div>) : (<AccumulationChartComponent id='policyholder-age-pie' ref={PolicyHolderRef} legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} load={onAccumulationLoad} tooltip={{ enable: true, format: '<b>${point.x}</b><br>Count: <b>${point.y}</b>', header: '' }}>
          <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective dataSource={ageDistributionData} xName='x' yName='y' radius='70%' dataLabel={{ visible: true, position: 'Outside', name: 'text' }} palettes={pieThemePalette} animation={{ enable: false }}/>
          </AccumulationSeriesCollectionDirective>
        </AccumulationChartComponent>)}
    </div>);
    ...
    const AnnualPremiumEquivalentChart = () => {
        return (<div style={{ width: '100%', height: '100%', padding: 8 }}>
        {apeChannelData.length === 0 ? (<div style={{ padding: 12 }}>No channel data for the selected year.</div>) : (<ChartComponent id="ape-channel-chart" ref={APERef} primaryXAxis={{
                    valueType: 'Category',
                    majorGridLines: { width: 0 },
                    labelIntersectAction: 'Trim'
                }} primaryYAxis={{
            ...
            <SeriesCollectionDirective>
              <SeriesDirective type="Bar" name="APE" dataSource={apeChannelData} xName="channel" yName="value" columnSpacing={0.2} cornerRadius={{ topLeft: 4, topRight: 4 }} marker={{ visible: true, dataLabel: { visible: true } }} fill='#FF4949' animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>)}
      </div>);
    };
    ...
    const ReferralResourceDonut = () => (<div style={{ height: '100%', width: '100%' }}>
      {referralData.length === 0 ? (<div style={{ padding: 12 }}>No referral data for the selected year(s).</div>) : (<AccumulationChartComponent id='referral-donut' ref={ReferralRef} legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} load={onAccumulationLoad} tooltip={{ enable: true, format: '<b>${point.x}</b><br>Count: <b>${point.y}</b>', header: '' }}>
          <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective dataSource={referralData} xName='x' yName='y' innerRadius='60%' radius='70%' dataLabel={{ visible: true, position: 'Outside', name: 'text' }} borderRadius={10} border={{ width: 4, color: '#ffffff' }} explode explodeIndex={0} explodeOffset='8%' palettes={donutPaletteColors} animation={{ enable: false }}/>
          </AccumulationSeriesCollectionDirective>
        </AccumulationChartComponent>)}
    </div>);
    return (<div className="Container">
      <div className="e-card insurance-toolbar">
        ...
        <div className="insurance-toolbar-right" style={{ display: 'flex', gap: 10 }}>
          <DropDownListComponent id="yearDropOverview" placeholder="Select year" dataSource={yearOptions} fields={{ text: 'year', value: 'id' }} value={currentYearOverview != null ? String(currentYearOverview) : undefined} change={onYearChange} width={160}/>
        </div>
      </div>
      <DashboardLayoutComponent id="dashboard_performance" ref={OverviewRef} style={{ height: '85vh', width: '100%', zIndex: 1 }} columns={8} cellAspectRatio={90 / 100} cellSpacing={[10, 10]} allowResizing={false} allowDragging={false} mediaQuery="(max-width:950px)" created={onCreated}>
        <PanelsDirective>
...
const Dashboard2 = ({ selectedYears, onChangeYears, selectedPolicyTypes, onChangePolicyTypes }) => {
    const ClaimAnalysisRef = React.useRef(null);
    const DeniedRef = React.useRef(null);
    const ClaimAmountComparisonRef = React.useRef(null);
    const KanbanRef = React.useRef(null);
    const RecordsMapRef = React.useRef(null);
    const ClaimSeverityRef = React.useRef(null);
    const gridRef = React.useRef(null);
    const TotalclaimRef = React.useRef(null);
    const POLICY_TYPES_FILTER = ['Health', 'Life', 'Home', 'Travel', 'Disability'];
    // Build available years from claimdata dynamically
    const yearOptions = React.useMemo(() => {
        return ALLOWED_YEARS.map(y => ({ id: String(y), label: String(y) }));
    }, []);
    const currentYearClaim = React.useMemo(() => (selectedYears.length ? Math.max(...selectedYears) : Number(yearOptions[0]?.id)), [selectedYears, yearOptions]);
    const policyTypeOptions = React.useMemo(() => [{ id: 'All', name: 'All Policy Types' }, ...POLICY_TYPES_FILTER.map(t => ({ id: t, name: t }))], []);
    ...
    const kpis = React.useMemo(() => {
        ...
        const totalClaimAmount = rows.reduce((sum, r) => sum + (Number(r.ClaimAmount) || 0), 0);
    ...
    const totalClaimsSparkByYear = React.useMemo(() => {
        ...
        const counts = new Map(ALLOWED_YEARS.map(y => [y, 0]));
    ...
    const TotalClaims = () => (<div className="kpi-totalcard">
      <div className="spark-header">
      ...
      <div className="spark-content" style={{ width: '50%', height: '50%' }}>
        <div style={{ marginTop: 8, width: '100%' }}>
          <SparklineComponent id="total-claims-spark" ref={TotalclaimRef} type="Pie" dataSource={totalClaimsSparkByYear} xName="x" yName="y" valueType="Category" width="100%" height="55px" lineWidth={2} palette={sparklinePalette} tooltipSettings={{ visible: true, format: 'Year: ${x}<br/>Claims: ${y}' }}>
            <Inject services={[SparklineTooltip]}/>
          </SparklineComponent>
        </div>
      </div>
    ...
    const approvalStats = React.useMemo(() => {
        ...
        for (const r of claims) {
            ...
            if (useType && !types.has(pt))
                continue;
            total += 1;
            if (isApproved(r.ClaimStatus ?? r.claimStatus ?? r.Status ?? r.status))
                approved += 1;
        }
    ...
    const { totalClaimDelta, avgClaimDelta, approvalRateDelta } = React.useMemo(() => {
        ...
        const rowsFor = (yr) => claims.filter(r => {
            const y = Number(r.year ?? r.Year);
            if (y !== yr)
                return false;
            if (!useType)
    ...
    const TotalClaimAmount = () => (<div className="insurance-card">
    ...
    const AverageClaimAmount = () => (<div className="insurance-card">
    ...
    const ClaimApprovalRate = () => (<div className="insurance-card">
    ...
    const { paidOutSeries, deniedSeries, totalClaimsSeries, yAmountMax, yCountMax } = React.useMemo(() => {
        const claims = InsuranceData.claimdata ?? [];
        if (!claims.length || !selectedYears.length) {
        ...
        const statusGroup = (v) => {
            const s = String(v ?? '').trim().toLowerCase();
            if (['approved', 'accept', 'accepted', 'paid', 'paid out', 'payout', 'settled'].some(k => s.includes(k)))
                return 'paid';
            if (['rejected', 'denied', 'declined'].some(k => s.includes(k)))
                return 'denied';
            return undefined;
        };
        const typesLower = new Set(selectedPolicyTypes.map(t => t.toLowerCase()));
    ...
    const DeniedChart = () => (<div style={{ height: '100%', width: '100%' }}>
      {paidDeniedPieData.length === 0 ? (<div style={{ padding: 12 }}>No paid/denied data for the selected filters.</div>) : (<AccumulationChartComponent id="paid-denied-pie" ref={DeniedRef} legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} tooltip={{ enable: true, header: '' }} load={onAccumulationLoad} tooltipRender={(args) => {
                const x = String(args?.point?.x ?? '');
                const y = Number(args?.point?.y ?? 0);
          ...
          <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective dataSource={paidDeniedPieData} xName="x" yName="y" radius="70%" dataLabel={{ visible: true, position: 'Outside', name: 'text' }} border={{ color: '#FFFFFF', width: 1 }} palettes={pieThemePalette} animation={{ enable: false }}/>
          </AccumulationSeriesCollectionDirective>
        </AccumulationChartComponent>)}
    </div>);
    React.useEffect(() => {
    ...
    const toKanbanStatus = (v, reason) => {
        const s = String(v ?? '').toLowerCase();
        if (['approved', 'accept', 'accepted', 'paid', 'paid out', 'payout', 'settled'].some(k => s.includes(k)))
            return 'Approved';
        if (['rejected', 'denied', 'declined'].some(k => s.includes(k)))
            return 'Rejected';
        if (['in progress', 'inprogress', 'pending', 'under review', 'processing'].some(k => s.includes(k)))
            return 'Inprogress';
    ...
    const kanbanCardTemplate = (props) => {
        const policyClass = (name) => {
    ...
    const onKanbanDialogOpen = (args) => {
        // Block the edit/add dialog
        if (args.requestType === 'Edit' || args.requestType === 'Add') {
            args.cancel = true;
        }
    };
    const onCardDoubleClick = (args) => { args.cancel = true; };
    const ClaimStatusKanban = () => (<div style={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', minHeight: 0, marginTop: '10px' }}>
      {kanbanData.length === 0 ? (<div style={{ padding: 12 }}>No claims for the selected filters.</div>) : (<div className='claimstatus-kanban' style={{ flex: 1, overflow: 'hidden', minHeight: 0 }}>
          <KanbanComponent ref={KanbanRef} id="claim-status-kanban" key={`kanban-${selectedYears.join(',')}-${selectedPolicyTypes.join(',')}`} keyField="Status" dataSource={kanbanData} allowDragAndDrop={false} height="100%" width="100%" cardSettings={{
                showHeader: false,
    ...
    const ClaimAmountComparisonChart = () => {
        ...
        const toNum = (value) => typeof value === 'number' && Number.isFinite(value) ? value :
        ...
        return (<div style={{ width: '100%', height: '100%', padding: 8, boxSizing: 'border-box' }}>
        <ChartComponent id="claim-amount-monthly" ref={ClaimAmountComparisonRef} enableAnimation={false} primaryXAxis={{
                valueType: 'Category',
                majorGridLines: { width: 0 },
                labelIntersectAction: 'Trim',
          ...
          <SeriesCollectionDirective>
            {hasPrevData && (<SeriesDirective type="SplineArea" dataSource={prevSeries} xName="x" yName="y" name={`${prevYear} Claim Amount`} opacity={0.5} border={{ width: 2, color: '#CDA310' }} fill="#CDA310" marker={{ visible: true, width: 6, height: 6, shape: 'Circle' }} animation={{ enable: false }}/>)}
            {hasCurrData && (<SeriesDirective type="SplineArea" dataSource={currSeries} xName="x" yName="y" name={`${currentYear} Claim Amount`} opacity={0.5} border={{ width: 2, color: '#FF7000' }} fill="#FF7000" marker={{ visible: true, width: 6, height: 6, shape: 'Circle' }} animation={{ enable: false }}/>)}
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    };
    ...
    const claimGridData = React.useMemo(() => {
        const all = InsuranceData.claimdata ?? [];
        const years = new Set(selectedYears);
        const types = new Set(selectedPolicyTypes.map(t => t.toLowerCase()));
        const useType = types.size > 0;
        return all
    ...
    const queryCellInfo = (args) => {
        if (args.column?.field === 'ClaimStatus') {
            ...
            const variant = key === 'approved' ? 'approved' :
    ...
    const REGIONS = ['United States', 'Brazil', 'United Kingdom', 'Russia', 'China', 'India'];
    ...
    const { amountByRegion, countByRegion } = React.useMemo(() => {
        ...
        const types = new Set(selectedPolicyTypes.map(t => t.toLowerCase()));
        const useType = types.size > 0;
        const amountMap = new Map(REGIONS.map(r => [r, 0]));
        const countMap = new Map(REGIONS.map(r => [r, 0]));
        for (const r of claims) {
            const y = Number(r.year ?? r.Year);
            if (!years.has(y))
                continue;
    ...
    const TotalRecordsByRegionMap = () => {
        ...
        const allowedTooltipCountries = React.useMemo(() => new Set(['United States', 'Brazil', 'United Kingdom', 'Russia', 'China', 'India']), []);
        const onMapTooltipRender = React.useCallback((args) => {
        ...
        if (!regions.length)
            return <div style={{ padding: 12 }}>No records for the selected filters.</div>;
        ...
        return (<div className="insurance-map-panel">
        <MapsComponent ref={RecordsMapRef} id="records-region-map" height="100%" width="100%" background="transparent" tooltipDisplayMode="MouseMove" legendSettings={{ visible: true, position: 'Bottom', mode: 'Default' }} tooltipRender={onMapTooltipRender} load={Mapload}>
          <MapsInject services={[MapsTooltip, MapsZoom, MapsLegend, MapsSelection, MapsHighlight]}/>
          <LayersDirective>
            <LayerDirective dataSource={mapData} shapeData={worldData} shapeDataPath="name" shapePropertyPath="name" shapeSettings={{
                fill: '#E5E5E5',
              ...
              <MarkersDirective>
                <MarkerDirective visible={true} dataSource={markers} animationDuration={0} template={'<div class="map-marker">${label}</div>'}/>
              </MarkersDirective>
            </LayerDirective>
          </LayersDirective>
        </MapsComponent>
    ...
    const { severityByRegion, severityMax } = React.useMemo(() => {
        const amt = new Map(amountByRegion.map(d => [d.x, Number(d.y || 0)]));
        const cnt = new Map(countByRegion.map(d => [d.x, Number(d.y || 0)]));
    ...
    const ClaimSeverityByRegionChart = () => {
        const pieData = React.useMemo(() => severityByRegion.map(d => ({ ...d, text: fmtCurrency(Number(d.y || 0)) })), [severityByRegion]);
        return (<div style={{ width: '100%', height: '100%' }}>
        {pieData.length === 0 ? (<div style={{ padding: 12 }}>No severity data for the selected filters.</div>) : (<AccumulationChartComponent id="claim-severity-region-pie" ref={ClaimSeverityRef} legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} load={onAccumulationLoad} tooltip={{ enable: true, header: '' }} tooltipRender={(args) => {
                    const x = String(args?.point?.x ?? '');
                    const y = Number(args?.point?.y ?? 0);
            ...
            <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
            <AccumulationSeriesCollectionDirective>
              <AccumulationSeriesDirective dataSource={pieData} xName="x" yName="y" radius="70%" dataLabel={{ visible: true, position: 'Outside', name: 'text' }} border={{ color: '#FFFFFF', width: 1 }} palettes={pieThemePalette} animation={{ enable: false }}/>
            </AccumulationSeriesCollectionDirective>
          </AccumulationChartComponent>)}
      </div>);
    };
    return (<div className="Container">
      <div className="e-card insurance-toolbar">
        ...
        <div className="insurance-toolbar-right" style={{ display: 'flex', gap: 10 }}>
          <DropDownListComponent id="policyTypeToolbar" dataSource={policyTypeOptions} fields={{ text: 'name', value: 'id' }} placeholder="Policy type" value={selectedPolicyTypeValue} change={onPolicyTypeChange} width={200}/>
          <DropDownListComponent id="yearToolbarClaim" dataSource={yearOptions} fields={{ text: 'label', value: 'id' }} placeholder="Select year" value={currentYearClaim != null ? String(currentYearClaim) : undefined} change={onYearChange} width={160}/>
        </div>
      </div>
      <DashboardLayoutComponent id="dashboard_performance" ref={ClaimAnalysisRef} style={{ width: '100%', height: '100%', zIndex: 1 }} columns={8} cellAspectRatio={90 / 100} cellSpacing={[10, 10]} allowResizing={false} allowDragging={false} mediaQuery="(max-width:950px)" created={onCreated}>
        <PanelsDirective>
...
const Dashboard3 = ({ selectedYears, onChangeYears }) => {
    const CustomerFeedbackRef = React.useRef(null);
    const NpsRef = React.useRef(null);
    const CrrRef = React.useRef(null);
    const ChannelFeedbackRef = React.useRef(null);
    const CustomerRatingRef = React.useRef(null);
    const gridInstance = React.useRef(null);
    const totalResponseRef = React.useRef(null);
    const YEAR_RANGE_CUSTOMER = [2025, 2024, 2023, 2022];
    const [selectedYearsCustomer, setSelectedYearsCustomer] = React.useState(selectedYears.length ? selectedYears : (YEAR_RANGE_CUSTOMER.length ? [YEAR_RANGE_CUSTOMER[0]] : []));
    const assignYearFromComment = React.useCallback((text, index) => {
    ...
    const yearOptionsCustomer = React.useMemo(() => YEAR_RANGE_CUSTOMER.map(y => ({ id: String(y), label: String(y) })), []);
    const currentYearCustomer = React.useMemo(() => (selectedYears.length ? Math.max(...selectedYears) : YEAR_RANGE_CUSTOMER[0]), [selectedYears]);
    React.useEffect(() => {
        setSelectedYearsCustomer(selectedYears);
    }, [selectedYears]);
    const onYearChangeToolbar = (e) => {
        const v = Number(e?.value);
        if (Number.isFinite(v)) {
            onChangeYears([v]);
    ...
    const totalResponses = React.useMemo(() => {
        const customerdata = InsuranceData.customerdata ?? {};
        const rows = Array.isArray(customerdata?.SentimentAnalysis) ? customerdata.SentimentAnalysis : [];
        ...
        if (Array.isArray(rows) && rows.length) {
            ...
            for (const r of rows) {
                ...
                if (!Number.isFinite(y) || !years.has(y))
                    continue;
                const hasBuckets = r.Positive != null || r.positive != null ||
    ...
    const totalResponsesSparkByYear = React.useMemo(() => {
        const cd = InsuranceData.customerdata ?? {};
        const rows = Array.isArray(cd?.SentimentAnalysis) ? cd.SentimentAnalysis
            : (Array.isArray(cd) ? cd : []);
        const getYear = (r, idx) => {
            const y = Number(r?.Year ?? r?.year);
            if (Number.isFinite(y))
    ...
    const TotalResponsesCard = () => (<div className="kpi-totalcard">
      <div className="spark-header">
      ...
      <div className="spark-content" style={{ width: '50%', height: '50%' }}>
        <div>
          <SparklineComponent id="total-responses-spark" ref={totalResponseRef} type="Column" dataSource={totalResponsesSparkByYear} xName="x" yName="y" valueType="Category" width="100%" height="55px" fill="#05B3DA" axisSettings={{ minY: 130 }} tooltipSettings={{ visible: true, format: 'Year: ${x}<br/>Responses: ${y}' }}>
            <Inject services={[SparklineTooltip]}/>
          </SparklineComponent>
        </div>
      </div>
    ...
    const avgSatisfaction = React.useMemo(() => {
        const rows = Array.isArray(InsuranceData.customerdata) ? InsuranceData.customerdata : [];
        if (!rows.length)
            return 0;
        const years = new Set(selectedYearsCustomer);
        const toNum = (v) => typeof v === 'number' && Number.isFinite(v) ? v :
            (typeof v === 'string' && v.trim() && Number.isFinite(+v) ? +v : NaN);
        const resolveYear = (row, idx) => {
            const y = Number(row?.Year ?? row?.year);
            if (Number.isFinite(y))
                return y;
    ...
    const SatisfactionScore = () => (<div className="insurance-card">
    ...
    const SentimentOverview = () => {
        ...
        const card = (title, count, total, imgUrl) => (<div className="sentiment-card">
        ...
        return (<div className="sentiment-panel-center">
    ...
    const gaugeRows = React.useMemo(() => (Array.isArray(InsuranceData.gaugedata) ? InsuranceData.gaugedata : []), []);
    const currentCustomerYear = React.useMemo(() => (selectedYearsCustomer.length ? Math.max(...selectedYearsCustomer) : undefined), [selectedYearsCustomer]);
    const getGaugeValue = React.useCallback((keys) => {
        ...
        <CircularGaugeComponent id="nps-gauge" ref={NpsRef} background="transparent" height="100%" width="100%" centerX="50%" centerY="70%" allowMargin={false} animationDuration={2000} title='Net Promoter Score (NPS)' titleStyle={{ size: '16px', fontWeight: '600' }} tooltip={{ enable: true, template: tooltipTpl }} load={onGaugeLoad} legendSettings={{ visible: false }}>
          ...
          <CircularGaugeAxesDirective>
            <CircularGaugeAxisDirective startAngle={270} endAngle={90} radius="100%" minimum={0} maximum={100} majorTicks={{ width: 1.5, height: 12, interval: 20, offset: 35 }} lineStyle={{ width: 0 }} minorTicks={{ width: 0 }} labelStyle={{ font: { size: '14px', fontFamily: 'inherit' }, position: 'Outside', offset: -40 }}>
              ...
              <RangesDirective>
                <RangeDirective start={0} end={value} radius="80%" color="#FF7000" startWidth={40} endWidth={40}/>
              </RangesDirective>
    ...
    const CrrGauge = () => {
        ...
        return (<div className="gauge-center" style={{ height: '100%', width: '100%' }}>
        <CircularGaugeComponent id="crr-gauge" ref={CrrRef} background="transparent" height="100%" width="100%" centerX="50%" centerY="70%" allowMargin={false} animationDuration={2000} title='Customer Retention Rate' titleStyle={{ size: '16px', fontWeight: '600' }} tooltip={{ enable: true, template: tooltipTpl }} load={onGaugeLoad} legendSettings={{ visible: false }}>
          <Inject services={[Annotations, GaugeTooltip, CircularGaugeLegend]}/>
          <CircularGaugeAxesDirective>
            <CircularGaugeAxisDirective startAngle={270} endAngle={90} radius="100%" minimum={0} maximum={100} majorTicks={{ width: 1.5, height: 12, interval: 20, offset: 35 }} lineStyle={{ width: 0 }} minorTicks={{ width: 0 }} labelStyle={{ font: { size: '14px', fontFamily: 'inherit' }, position: 'Outside', offset: -40 }}>
              <AnnotationsDirective>
              ...
              <RangesDirective>
                <RangeDirective start={0} end={v} radius="80%" color="#CE9461" startWidth={40} endWidth={40}/>
              </RangesDirective>
            </CircularGaugeAxisDirective>
          </CircularGaugeAxesDirective>
        </CircularGaugeComponent>
    ...
    const channelFeedbackData = React.useMemo(() => {
        ...
        const CHANNELS = ['Email', 'Phone', 'Social Media', 'Surveys', 'Agent', 'SMS'];
        const counts = new Map(CHANNELS.map(c => [c, 0]));
        const normChannel = (v) => {
            const s = String(v ?? '').trim().toLowerCase();
            if (!s)
                return undefined;
    ...
    const ChannelFeedbackChart = () => {
        return (<div style={{ width: '100%', height: '100%', padding: 8 }}>
        {channelFeedbackData.length === 0 ? (<div style={{ padding: 12 }}>No channel feedback for the selected year(s).</div>) : (<ChartComponent id="channel-feedback-chart" ref={ChannelFeedbackRef} primaryXAxis={{
                    valueType: 'Category',
                    majorGridLines: { width: 0 },
                    labelIntersectAction: 'Trim'
                }} primaryYAxis={{
            ...
            <Inject services={[ColumnSeries, Category, Legend, Tooltip, DataLabel]}/>
            <SeriesCollectionDirective>
              <SeriesDirective type="Column" dataSource={channelFeedbackData} xName="x" yName="y" columnSpacing={0.2} marker={{ visible: true, dataLabel: { visible: true } }} cornerRadius={{ topLeft: 4, topRight: 4 }} fill="#F8CA7E" animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>)}
      </div>);
    };
    ...
    const CustomerRatingPie = () => (<div style={{ height: '100%', width: '100%' }}>
      {ratingPieData.every(d => d.y === 0) ? (<div style={{ padding: 12 }}>No rating data for the selected year(s).</div>) : (<AccumulationChartComponent id='customer-rating-pie' ref={CustomerRatingRef} legendSettings={{ visible: true, position: 'Bottom' }} enableAnimation={true} load={onAccumulationLoad} tooltip={{ enable: true, format: '<b>Rating ${point.x}</b><br>Count: <b>${point.y}</b>', header: '' }}>
          <Inject services={[AccumulationLegend, PieSeries, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective dataSource={ratingPieData} xName='x' yName='y' innerRadius='60%' radius='70%' dataLabel={{ visible: true, position: 'Outside', name: 'text' }} palettes={donutPaletteColors} animation={{ enable: false }} borderRadius={10} border={{ width: 4, color: '#ffffff' }}/>
          </AccumulationSeriesCollectionDirective>
        </AccumulationChartComponent>)}
    </div>);
    // --- Feedback Details Grid ---
    const classifySentiment = React.useCallback((text) => {
    ...
    const normalizeSentiment = React.useCallback((v) => {
        const s = String(v ?? '').trim().toLowerCase();
        if (!s)
            return undefined;
    ...
    const feedbackGridData = React.useMemo(() => {
        ...
        rows.forEach((r, idx) => {
            ...
            const sRaw = r['Sentiment Analysis'] ?? r?.SentimentAnalysis ?? r?.sentimentAnalysis ?? r?.Sentiment ?? r?.sentiment;
    ...
    const onFeedbackCellStyle = (args) => {
    ...
    const FeedbackDetailsGrid = () => (<div style={{ width: '100%', height: '100%', padding: 8 }}>
      <GridComponent ref={gridInstance} id='feedbackDetailsGrid' dataSource={feedbackGridData} allowPaging={false} enableVirtualMaskRow={true} allowSorting={true} allowResizing={true} allowFiltering={true} allowMultiSorting={true} width={'100%'} height={'100%'} allowGrouping={true} allowExcelExport={true} allowPdfExport={true} toolbar={toolBarOptions} filterSettings={{ type: 'Menu' }} queryCellInfo={onFeedbackCellStyle} toolbarClick={toolbarClick}>
        <ColumnsDirective>
          ...
          <ColumnDirective field="Rating" headerText="Rating" headerTemplate={headerWithTooltip("Rating")} width="130" textAlign="Center" template={ratingTemplate}/>
    ...
    return (<div className="Container">
      <div className="e-card insurance-toolbar">
        ...
        <div className="insurance-toolbar-right" style={{ display: 'flex', gap: 10 }}>
          <DropDownListComponent id="yearToolbarCustomer" dataSource={yearOptionsCustomer} fields={{ text: 'label', value: 'id' }} placeholder="Select year" value={currentYearCustomer != null ? String(currentYearCustomer) : undefined} change={onYearChangeToolbar} width={160}/>
        </div>
      </div>
      <DashboardLayoutComponent id="dashboard_performance" ref={CustomerFeedbackRef} style={{ width: '100%', zIndex: 1 }} columns={8} cellAspectRatio={90 / 100} cellSpacing={[10, 10]} allowResizing={false} allowDragging={false} mediaQuery="(max-width:950px)">
        <PanelsDirective>
...
export class InsuranceDashboard extends SampleBase {
    sidebarRef;
    ...
    constructor(props) {
        ...
        const overviewdata = InsuranceData.overview;
        const policydata = InsuranceData.policydata;
        const fromOverview = Array.from(new Set((overviewdata ?? []).map(r => Number(r.year ?? r.Year)).filter(Number.isFinite))).sort();
        const allYears = (fromOverview.length
            ? fromOverview
            : Array.from(new Set((policydata ?? []).map(r => Number(r.year ?? r.Year)).filter(Number.isFinite))).sort());
        const availAllowed = allYears.filter(y => ALLOWED_YEARS.includes(y));
    ...
    DOCK_SIZE = 60;
    OPEN_WIDTH = 240;
    titleTemplate = '<div class="dashboard-title">Insurance Dashboard</div>';
    ...
    renderDashboard = () => {
        ...
        const onChangeYears = (ys) => this.setState({ sharedYears: ys });
        switch (selectedId) {
            case 'overview':
                return <Dashboard1 selectedYears={years} onChangeYears={onChangeYears}/>;
            case 'cliam-analysis':
                return (<Dashboard2 selectedYears={years} onChangeYears={onChangeYears} selectedPolicyTypes={this.state.sharedPolicyTypes} onChangePolicyTypes={(vals) => this.setState({ sharedPolicyTypes: vals })}/>);
            case 'customer':
    ...
    render() {
        ...
        <div className='control-section'>
          <div className="insurance-root">
            <div>
              <ToolbarComponent cssClass="insurance-dockToolbar" id="insurance-dockToolbar" height={`${this.TOOLBAR_HEIGHT}px`} clicked={this.onToolbarClicked}>
                <ItemsDirective>
            ...
            <div className="insurance-workarea">
              <SidebarComponent id="dockInsurancesidebar" ref={this.sidebarRef} enableDock={true} width={`${this.OPEN_WIDTH}px`} dockSize={`${this.DOCK_SIZE}px`} closeOnDocumentClick={false} enableGestures={false} className="insurance-sidebar" type="Push" target=".insurance-content" open={this.onSidebarOpen} close={this.onSidebarClose} created={this.onSidebarCreated}>
                {this.withTooltip('Overview', <div className={`insurance-nav-item ${isActive('overview')}`} onClick={() => this.setState({ selectedId: 'overview' })}>
                    <span className="e-icons e-home" aria-hidden="true"/>
                    <span className="insurance-nav-text">Overview</span>
                  </div>)}
                {this.withTooltip('Claim Analysis', <div className={`insurance-nav-item ${isActive('cliam-analysis')}`} onClick={() => this.setState({ selectedId: 'cliam-analysis' })}>
                    <span className={this.icon('claim-analysis')} aria-hidden="true"/>
                    <span className="insurance-nav-text">Claim Analysis</span>
                  </div>)}
                {this.withTooltip('Customer Feedback', <div className={`insurance-nav-item ${isActive('customer')}`} onClick={() => this.setState({ selectedId: 'customer' })}>
