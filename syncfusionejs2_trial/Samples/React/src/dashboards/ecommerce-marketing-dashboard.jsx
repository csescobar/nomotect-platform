...
import { MapsComponent, LayersDirective, LayerDirective, Legend as MapsLegend, MapsTooltip } from '@syncfusion/ej2-react-maps';
import './ecommerce-marketing-dashboard.css';
import { AccumulationChartComponent, AccumulationSeriesCollectionDirective, AccumulationSeriesDirective, AccumulationLegend, Legend, PieSeries, AccumulationTooltip, SeriesCollectionDirective, AccumulationDataLabel, ChartComponent, ColumnSeries, Category, SeriesDirective, Tooltip, DataLabel, Highlight, SplineAreaSeries, Inject, BulletChartComponent, BulletRangeCollectionDirective, BulletRangeDirective, BulletTooltip, StackingColumnSeries, StackingBarSeries, SplineSeries, LineSeries, SplineRangeAreaSeries, Tooltip as ChartTooltip, WaterfallSeries, PyramidSeries, SparklineComponent, SparklineTooltip, Inject as SparkInject, BarSeries } from '@syncfusion/ej2-react-charts';
import { CircularGaugeComponent, AxesDirective as GaugeAxesDirective, AxisDirective as GaugeAxisDirective, PointersDirective, PointerDirective, RangesDirective, RangeDirective, Annotations as GaugeAnnotations, AnnotationsDirective as GaugeAnnotationsDirective, AnnotationDirective as GaugeAnnotationDirective, Inject as GaugeInject, GaugeTooltip } from '@syncfusion/ej2-react-circulargauge';
import { DropDownListComponent } from '@syncfusion/ej2-react-dropdowns';
import { GridComponent, ColumnsDirective, ColumnDirective, DetailRow, ExcelExport, Inject as GridInject, PdfExport, Toolbar, Page, Sort } from '@syncfusion/ej2-react-grids';
import { HeatMapComponent, Inject as HeatmapInject, Adaptor as HeatmapAdaptor, Legend as HeatmapLegend, Tooltip as HeatmapTooltip } from '@syncfusion/ej2-react-heatmap';
...
const pickPalette = (arr, count) => arr.slice(0, Math.max(0, Math.min(arr.length, count)));
const monthlyRev2022 = [
...
const scenarioOptions = ['Baseline', 'Optimistic', 'Conservative'];
function buildYearProductSales(year, upToMonthIndex) {
    ...
    const scaled = Object.fromEntries(base.map(b => [b.name, Array(12).fill(0)]));
    for (let m = 0; m < 12; m++) {
...
const marketChannelSplitByRegion = {
...
const channelCR_2022 = shiftCR(channelCR_2024, -0.20);
const channelCR_2023 = shiftCR(channelCR_2024, -0.10);
const channelCR_2025 = shiftCR(channelCR_2024, +0.10);
const channelCRByYear = {
    2022: channelCR_2022,
    2023: channelCR_2023,
    2024: channelCR_2024,
    2025: channelCR_2025,
...
const supplyCoverByMonth = [1.05, 0.85, 1.10, 0.95, 0.80, 0.75, 0.90, 0.70, 1.00, 0.95, 0.85, 1.05];
const computeStockStatus = (units, monthIdx) => {
...
class TargetVsActualBulletPanel extends React.PureComponent {
    static defaultProps = {
        year: 2025,
    ...
    TargetActualBulletchartRef = React.createRef();
    state = { metric: 'revenue' };
    refreshTargetActualBulletPanelChart = () => {
    ...
    componentDidUpdate(prevProps, prevState) {
        const propsChanged = prevProps.year !== this.props.year ||
    ...
    computeCategoryShareForMonth(year, monthIdx) {
    ...
    computeIncludedOrdersForMonth(year, monthIdx) {
    ...
    computeRevenueSliceForMonth(year, monthIdx) {
        const master = (monthlyRevenueByYear[year] ?? [])[monthIdx];
        const masterActual = master?.actual ?? 0;
        const masterTarget = master?.target ?? 0;
        const denomOrders = (() => {
            const allRegions = ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica'];
            let sum = 0;
    ...
    computeOrdersTargetForMonth(year, monthIdx, actualOrders) {
        const master = (monthlyRevenueByYear[year] ?? [])[monthIdx];
    ...
    onBulletTooltipRender = (args) => {
        ...
        const v = args?.value;
        const achievedText = typeof v === 'number' ? formatCurrency(v) : String(v ?? '');
        const t = Array.isArray(args?.target) ? args.target[0] : args?.target;
        const targetText = typeof t === 'number' ? formatCurrency(t) : String(t ?? '');
    ...
    render() {
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box' }}>
...
class SalesMixGrowthPanel extends React.PureComponent {
    SalesMixGrowthChartRef = React.createRef();
    state = { drill: null, activeTab: 'mix' };
    refreshSalesMixGrowthChart = () => {
        this.SalesMixGrowthChartRef.current?.refresh();
    };
    componentDidMount() {
        this.props.onReady?.({ refreshSalesMixGrowthChart: this.refreshSalesMixGrowthChart });
    }
    onChannelChartMouseClick = (args) => {
        ...
        if (target.startsWith('smg_channel_donut_datalabel_Series_0_text_') ||
            ...
            if (idx >= 0) {
                const chart = this.SalesMixGrowthChartRef.current;
                const pt = chart?.visibleSeries?.[0]?.points?.[idx] ??
    ...
    isAllMonths = () => this.props.monthIndex === ALL_MONTH;
    getIncludedRegions() {
        const r = this.props.region;
        if (r === 'ALL')
            return ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica'];
    ...
    categoryShareForMonth(year, monthIdx) {
        const products = buildYearProductSales(year);
        const totals = {};
        let totalUnits = 0;
        for (const p of products) {
    ...
    revenueByCategoryWithinChannelMonth(year, monthIdx, channel) {
    ...
    ordersForChannelMonth(year, monthIdx, ch) {
        let sum = 0;
    ...
    denomOrdersAllRegionsAllChannels(year, monthIdx) {
        let sum = 0;
        for (const region of ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica']) {
            const rows = buildChannelsByRegionYear(year, region);
            const r = rows[monthIdx];
            if (!r)
                continue;
    ...
    revenueByCategoryForMonth(year, monthIdx) {
        const master = (monthlyRevenueByYear[year] ?? [])[monthIdx];
        const masterActual = master?.actual ?? 0;
    ...
    revenueByCategoryForYear(year) {
        const cats = this.props.categories?.length ? this.props.categories : CATEGORY_OPTIONS;
        const agg = Object.fromEntries(cats.map(c => [c, 0]));
        for (let i = 0; i < 12; i++) {
            for (const row of this.revenueByCategoryForMonth(year, i)) {
                if (row && typeof row.y === 'number')
                    agg[row.x] = (agg[row.x] ?? 0) + row.y;
            }
        }
        return Object.keys(agg).map(k => ({ x: k, y: agg[k] }));
    }
    revenueByCategoryWithinChannelYear(year, channel) {
        const cats = this.props.categories?.length ? this.props.categories : CATEGORY_OPTIONS;
        const agg = Object.fromEntries(cats.map(c => [c, 0]));
        for (let i = 0; i < 12; i++) {
    ...
    onCurrencyTooltip = (args) => {
        const y = Number(args?.point?.y ?? 0);
        const x = String(args?.point?.x ?? '');
        const series = String(args?.series?.name ?? '');
        args.text = series ? `${x} : ${formatCurrency(y)}` : `${x}: ${formatCurrency(y)}`;
    };
    onBack = () => this.setState({ drill: null });
    roundSeriesToTotal(data) {
        const items = data.map(d => ({ x: d.x, y: Number(d.y || 0) }));
        const total = items.reduce((s, d) => s + d.y, 0);
        if (!isFinite(total) || items.length === 0)
            return items.map(d => ({ x: d.x, y: Math.round(d.y) }));
    ...
    renderMixView() {
        ...
        const chColors = chData.map(d => ChannelColors[d.x] ?? '#999');
        const byCategoryRaw = (isAll ? this.revenueByCategoryForYear(year) : this.revenueByCategoryForMonth(year, monthIndex)).filter(d => d.y > 0);
        ...
        const onTextRender = (args) => {
            ...
            if (!Number.isFinite(pct)) {
                ...
                const sum = pts.reduce((s, p) => s + (Number(p?.y) || 0), 0);
                pct = sum > 0 ? Math.round((y / sum) * 100) : 0;
            }
            args.text = `${formatCurrency(y)} (${pct}%)`;
        };
        return (<div style={{ height: '100%', width: '100%', minHeight: 0 }}>
        <AccumulationChartComponent id={chartId} ref={this.SalesMixGrowthChartRef} legendSettings={{ visible: true, position: "Right" }} tooltip={{ enable: true }} tooltipRender={this.onCurrencyTooltip} textRender={onTextRender} pointClick={drill ? undefined : this.onChannelDonutPointClick} chartMouseClick={!drill ? this.onChannelChartMouseClick : undefined} load={onAccumulationLoad}>
          <Inject services={[PieSeries, AccumulationLegend, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            {!drill && (<AccumulationSeriesDirective type="Pie" dataSource={chDataForChart} xName="legendText" yName="y" innerRadius="65%" palettes={chColors} dataLabel={{ visible: true, position: 'Outside', connectorStyle: { length: '10px' }, name: 'y', font: { size: '12px' } }} animation={{ enable: false }} borderRadius={10} border={{ width: 4, color: '#ffffff' }}/>)}
            {drill && drill.type === 'channel' && (<AccumulationSeriesDirective type="Pie" dataSource={drillData._forChart ?? drillData} xName="legendText" yName="y" innerRadius="65%" palettes={drillColors} dataLabel={{ visible: true, position: 'Outside', connectorStyle: { length: '10px' }, name: 'y', font: { size: '12px' } }} animation={{ enable: false }} borderRadius={10} border={{ width: 4, color: '#ffffff' }}/>)}
          </AccumulationSeriesCollectionDirective>
        </AccumulationChartComponent>
      </div>);
    }
    clampMonth = (m) => Math.max(0, Math.min(11, m));
    buildRollingWindow = (year, monthIndex, windowSize = 3) => {
        const out = [];
    ...
    pctChange = (current, base) => (base > 0 ? ((current - base) / base) * 100 : null);
    fmtPct = (v) => {
    ...
    getPrevMonth = (year, monthIndex) => {
        if (monthIndex > 0)
            return { year, monthIdx: monthIndex - 1 };
        return { year: year - 1, monthIdx: 11 };
    };
    onTrendsViewAxisLabelRender = (args) => {
    ...
    renderTrendView() {
        ...
        const growthRows = includedChannels.map((ch) => {
            const curr = currByCh[ch] ?? 0;
            const pm = prevByCh ? (prevByCh[ch] ?? 0) : 0;
            const ly = yoyByCh ? (yoyByCh[ch] ?? 0) : 0;
            return { ch, mom: this.pctChange(curr, pm), yoy: this.pctChange(curr, ly) };
        });
        return (<div style={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {/* Growth chips */}
        {!isAll && (<div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {growthRows.map((g) => (<div key={g.ch} style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 8,
                        padding: '4px 10px',
                ...
                <span style={{ width: 8, height: 8, borderRadius: 999, background: ChannelColors[g.ch] ?? '#64748B' }}/>
        ...
        <div style={{ flex: 1, minHeight: 0, height: '100%', width: '100%' }}>
          <ChartComponent id="salesMixGrowthTrendTab" height="100%" width="100%" primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 }, labelStyle: { size: '10px' } }} primaryYAxis={{ labelFormat: 'c0', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, majorGridLines: { width: 0 }, labelStyle: { size: '10px' } }} chartArea={{ border: { width: 0 } }} tooltip={{ enable: true }} legendSettings={{ visible: true }} axisLabelRender={this.onTrendsViewAxisLabelRender} load={onChartLoad} tooltipRender={this.onCurrencyTooltip}>
            <Inject services={[SplineSeries, Legend, ChartTooltip, Category]}/>
            <SeriesCollectionDirective>
              {includedChannels.map((ch) => (<SeriesDirective key={ch} type="Spline" name={ch} dataSource={trendSeriesByChannel[ch]} xName="m" yName="y" width={2} marker={{ visible: true, width: 8, height: 8 }} fill={ChannelColors[ch] ?? '#64748B'}/>))}
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    ...
    render() {
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}>
          ...
          <div className="e-btn-group">
            <ButtonComponent cssClass={this.state.activeTab === 'mix' ? 'e-primary' : 'e-outline'} onClick={() => this.setTab('mix')}>
              Mix
            </ButtonComponent>
            <ButtonComponent cssClass={this.state.activeTab === 'trend' ? 'e-primary' : 'e-outline'} onClick={() => this.setTab('trend')}>
              Trend
            </ButtonComponent>
          </div>
        </div>
...
class DemandOriginsPanel extends React.Component {
    RegionChartRef = React.createRef();
    RegionMapRef = React.createRef();
    state = { demandRegion: null, worldShape: null, channelDrill: null };
    refreshRegionChart = () => {
    ...
    componentDidUpdate(prevProps) {
        const filtersChanged = prevProps.year !== this.props.year ||
    ...
    ordersForRegionFiltered(region) {
    ...
    ordersByChannelForRegion(region) {
        const y = this.props.year;
        const monthsSel = this.monthIdxs;
    ...
    continentDemandRows() {
        ...
        const out = continents.map(c => ({ continent: c, value: 0, primaryRegion: null }));
        for (const c of continents) {
            let primary = null;
            let primaryContribution = 0;
            let totalContribution = 0;
            Object.keys(regionVals).forEach(rk => {
    ...
    mapRegionNameToKeyFromContinent(continent) {
    ...
    onMapShapeSelected = (args) => {
        const continent = args?.shapeData?.continent;
        if (!continent)
            return;
    ...
    onDemandBack = () => this.setState({ demandRegion: null, channelDrill: null });
    onChannelBack = () => this.setState({ channelDrill: null });
    onChannelPointClick = (args) => {
        const name = (args?.series?.name ?? '').trim();
        if (name === 'Paid' || name === 'Organic' || name === 'Email' || name === 'Social') {
            this.setState({ channelDrill: name });
        }
    };
    productsByChannelForRegionYear(year, region, channel) {
        const products = buildYearProductSales(year);
        const denomAllOrdersForMonth = (monthIdx) => {
            let sum = 0;
            ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica'].forEach(r => {
        ...
        const regionChannelOrdersForMonth = (monthIdx) => {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            perProduct.forEach(pp => {
                if (top.includes(pp.name))
                    row[pp.name] = (row[pp.name] ?? 0) + pp.units;
                else
                    other += pp.units;
            });
            if (other > 0)
    ...
    tooltipRender = (args) => {
        ...
        const primaryRegionLabel = primaryRegion ? `<div>Region: ${this.displayRegion(primaryRegion)}</div>` : '';
    ...
    render() {
        ...
        const chartType = 'StackingColumn';
        const stackedMax = (seriesData.length && selChannels.length)
            ? Math.max(...seriesData.map(s => selChannels.reduce((acc, ch) => acc + s[ch], 0)))
            : 100;
        const yAxisMax = Math.max(10, Math.ceil(stackedMax * 1.1));
        let productStackRows = [];
        let productKeys = [];
        if (drilledChannel && this.state.demandRegion && this.state.channelDrill) {
            ...
            productKeys = Array.from(new Set(productStackRows.flatMap(r => Object.keys(r).filter(k => k !== 'm'))));
        }
        const productColors = pickPalette(Palettes.salesMix, Math.max(3, productKeys.length));
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {/* Header */}
        ...
        {!drilled && (<MapsComponent id="demandOriginsMap" ref={this.RegionMapRef} legendSettings={{ visible: true, mode: 'Interactive', position: 'Left', orientation: 'Vertical' }} shapeSelected={this.onMapShapeSelected} tooltipRender={this.tooltipRender} load={Mapload}>
            <Inject services={[MapsTooltip, MapsLegend]}/>
            <LayersDirective>
              <LayerDirective shapeData={(this.state.worldShape ?? worldMap)} shapePropertyPath="continent" shapeDataPath="continent" dataSource={filteredDs} shapeSettings={{
        ...
        {drilledRegion && !drilledChannel && (<ChartComponent ref={this.RegionChartRef} id="demandOriginsDrillChart" primaryXAxis={{ valueType: 'Category', title: isSingleMonth ? '' : 'Month', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: '{value}', lineStyle: { width: 0 }, majorGridLines: { width: 0 }, majorTickLines: { width: 0 }, minimum: 0, maximum: yAxisMax }} tooltip={{ enable: true, shared: true }} legendSettings={{ visible: true }} pointClick={this.onChannelPointClick} load={onChartLoad}>
            <Inject services={[StackingColumnSeries, Legend, ChartTooltip, Category, DataLabel]}/>
            <SeriesCollectionDirective>
              {(selChannels.includes('Paid') ? ['Paid'] : []).map((k) => (<SeriesDirective key={k} type={chartType} name="Paid" xName="m" yName="Paid" dataSource={seriesData} columnWidth={0.5} fill={ChannelColors.Paid} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>))}
              {(selChannels.includes('Organic') ? ['Organic'] : []).map((k) => (<SeriesDirective key={k} type={chartType} name="Organic" xName="m" yName="Organic" dataSource={seriesData} columnWidth={0.5} fill={ChannelColors.Organic} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>))}
              {(selChannels.includes('Email') ? ['Email'] : []).map((k) => (<SeriesDirective key={k} type={chartType} name="Email" xName="m" yName="Email" dataSource={seriesData} columnWidth={0.5} fill={ChannelColors.Email} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>))}
              {(selChannels.includes('Social') ? ['Social'] : []).map((k) => (<SeriesDirective key={k} type={chartType} name="Social" xName="m" yName="Social" dataSource={seriesData} columnWidth={0.5} fill={ChannelColors.Social} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>))}
        ...
        {drilledChannel && (<ChartComponent id="demandProductsByChannel" primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: '{value}', lineStyle: { width: 0 }, majorGridLines: { width: 0 }, majorTickLines: { width: 0 }, minimum: 0 }} tooltip={{ enable: true, shared: true }} legendSettings={{ visible: true, position: 'Bottom' }} load={onChartLoad}>
            <Inject services={[StackingColumnSeries, Legend, ChartTooltip, Category, DataLabel]}/>
            <SeriesCollectionDirective>
              {productKeys.map((pName, i) => (<SeriesDirective key={pName} type="StackingColumn" name={pName} xName="m" yName={pName} dataSource={productStackRows} columnWidth={0.5} fill={productColors[i % productColors.length]} marker={{ dataLabel: { visible: true, font: { size: "9px", fontWeight: 'Bold' } } }} animation={{ enable: false }}/>))}
            </SeriesCollectionDirective>
          </ChartComponent>)}
      </div>);
    }
}
const asChannelKeys = (arr) => (arr ?? []).filter((x) => x === 'Paid' || x === 'Organic' || x === 'Email' || x === 'Social');
const arrayEqual = (a = [], b = []) => a.length === b.length && a.every((v, i) => v === b[i]);
const safePct = (actual, target) => (target > 0 ? (actual / target) * 100 : 0);
const isRegionKey = (r) => r === 'AsiaPacific' || r === 'Europe' || r === 'NorthAmerica' || r === 'LatinAmerica' || r === 'MiddleEastAfrica';
const ALL_CHANNELS = ['Paid', 'Organic', 'Email', 'Social'];
const PES_AD_CHANNELS = ['Paid', 'Email', 'Social'];
const BulletColors = {
    actual: '#554994',
    target: '#850E35',
    poor: '#FE8F8F',
...
function calcTrend(current, previous) {
    ...
    const dir = Math.abs(deltaPct) < 0.05 ? 'flat' : (delta > 0 ? 'up' : 'down');
...
function renderCommonKpiTile(opts) {
    const { label, valueText, badge, sparkData, sparkColor = '#10B981', sparkId = `kpi-spark-${label.replace(/\s+/g, '-').toLowerCase()}`, hideHeader = false, isMonthSelected = false } = opts;
    ...
    const badgeStyle = badge?.tone === 'good'
    ...
    return (<div className="e-card kpi-commerce-card">
      {hideHeader && (<div className="e-card-header">
      ...
      <div className="e-card-content kpi-commerce-card-content">
        ...
        {/* Optional sparkline */}
        {showSpark && (<div>
            <SparklineComponent id={sparkId} ref={getCommonSparkRef(sparkId)} height='40px' width="100%" type="Area" valueType="Category" dataSource={sparkData} xName="x" yName="y" lineWidth={2} fill={sparkColor} border={{ color: sparkColor, width: 2 }} markerSettings={{
                size: 5,
                fill: '#FFFFFF',
...
class OverviewClass extends React.Component {
    // Refs
    OverviewRef = React.createRef();
    roasSummaryChartRef = React.createRef();
    MarketShareChartRef = React.createRef();
    RegionShareGridRef = React.createRef();
    MarketShareGaugeRef = React.createRef();
    targetActualMixPanelApi;
    salesMixPanelApi;
    demandOriginsApi;
    ...
    componentDidUpdate(prevProps, prevState) {
        const propsChanged = prevProps.year !== this.props.year ||
            ...
            !arrayEqual(prevProps.channels, this.props.channels) ||
            !arrayEqual(prevProps.categories, this.props.categories);
        const stateOutOfSync = this.state.year !== this.props.year ||
    ...
    currency2 = (v) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v);
    integer = (v) => new Intl.NumberFormat('en-US', { maximumFractionDigits: 0 }).format(v);
    pct = (v) => `${v.toFixed(2)}%`;
    // Local header tooltip helper for Grid headers (tooltip = header text)
    headerWithTooltip = (label, _tip) => {
        return () => (<div title={label} style={{ display: 'inline-block', cursor: 'default' }}>{label}</div>);
    };
    get selectedMonthIdxs() {
    ...
    filteredMarketForMonth(year, monthIdx) {
        ...
        const regionChannelFactor = regions.reduce((sum, r) => {
            const rSplit = regionMarketSplit[r] ?? 0;
    ...
    singleMonthGrowthVsPrev(year, monthIdx) {
    ...
    spendForMonthByChannels(year, monthIdx, channels) {
    ...
    denomOrdersForChannels(year, monthIdx, channels) {
        const allRegions = ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica'];
        let denom = 0;
        for (const region of allRegions) {
            const chRows = buildChannelsByRegionYear(year, region);
    ...
    computeAdSpendSliceForMonth(year, monthIdx) {
    ...
    totalAdSpendFilteredForRange(year, monthIdxs) {
        return monthIdxs.reduce((sum, idx) => sum + this.computeAdSpendSliceForMonth(year, idx), 0);
    }
    computeAdAttributedRevenueSliceForMonth(year, monthIdx) {
        ...
        for (const region of regions) {
            ...
            if (!row)
                continue;
            for (const ch of adCh)
                adOrdersSelectedRegions += (row[ch] ?? 0);
        }
    ...
    totalAdAttributedRevenueFilteredForRange(year, monthIdxs) {
    ...
    renderKpiTile(opts) {
        ...
        const badgeStyle = badgeTone === 'good'
        ...
        <div className="e-card-content kpi-commerce-card-content">
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              {description && (<TooltipComponent content={description} position="TopCenter">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'left', gap: 12, marginBottom: 8 }}>
              ...
              {showTrendBadge && (<div style={{ display: 'flex', alignItems: 'center' }}>
    ...
    renderBulletChart = () => {
        return (<TargetVsActualBulletPanel year={this.props.year} monthIdxs={this.selectedMonthIdxs} region={this.props.region} channels={this.props.channels} categories={this.props.categories} onReady={(api) => (this.targetActualMixPanelApi = api)}/>);
    };
    renderSalesMixGrowth = () => {
        return (<SalesMixGrowthPanel year={this.props.year} monthIndex={this.props.monthIndex} region={this.props.region} channels={this.props.channels} categories={this.props.categories} onReady={(api) => (this.salesMixPanelApi = api)}/>);
    };
    renderDemandOrigins = () => {
        return (<DemandOriginsPanel year={this.props.year} monthIndex={this.props.monthIndex} region={this.props.region} channels={this.props.channels} categories={this.props.categories} onReady={(api) => (this.demandOriginsApi = api)}/>);
    };
    onMarketTrendsAxisLabelRender = (args) => {
        if (args.axis && args.axis.name === 'percentAxis')
            return;
        if (args?.axis?.valueType !== 'Category') {
            args.text = formatCurrency(Number(args.value || 0));
    ...
    renderMarketTrends = () => {
        ...
        if (isSingleMonth) {
          ...
          <CircularGaugeComponent id='marketShareGauge' ref={this.MarketShareGaugeRef} tooltip={{ enable: true, type: ['Pointer'] }} load={onGaugeLoad}>
            ...
            <GaugeAxesDirective>
              <GaugeAxisDirective minimum={0} maximum={100} majorTicks={{ interval: 20, height: 0 }} minorTicks={{ height: 0 }} labelStyle={{ font: { size: '12px' }, position: 'Outside', offset: 10 }}>
                <PointersDirective>
                  <PointerDirective value={+share.toFixed(2)} radius="75%" color="#7107DC" pointerWidth={10} cap={{ radius: 7, color: '#111827' }} needleEndWidth={4}/>
                </PointersDirective>
                ...
                <GaugeAnnotationsDirective>
                  <GaugeAnnotationDirective content={`<div style="text-align:center;"><div style="font-weight:800;font-size:24px;">${share.toFixed(1)}%</div></div>`} angle={0} radius="-20%" zIndex="1"/>
                </GaugeAnnotationsDirective>
              </GaugeAxisDirective>
            </GaugeAxesDirective>
        ...
        <div style={{ height: "calc(100% - 10px)", width: '100%' }}>
          <ChartComponent id="marketTrendsChart" ref={this.MarketShareChartRef} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: 'c0', lineStyle: { width: 0 }, majorTickLines: { width: 0 } }} axes={[percentAxis]} chartArea={{ border: { width: 0 } }} tooltip={{ enable: true }} legendSettings={{ visible: true }} axisLabelRender={this.onMarketTrendsAxisLabelRender} tooltipRender={this.onCurrencyTooltip} load={onChartLoad}>
            <Inject services={[SplineAreaSeries, SplineSeries, Legend, ChartTooltip, Category, DataLabel]}/>
            <SeriesCollectionDirective>
              {/* Market Sales (Area) */}
              <SeriesDirective type="SplineArea" name="Market Sales" xName="m" yName="market" dataSource={seriesData} opacity={0.6} fill="#F2B6D1" border={{ width: 2, color: '#B9005B' }} marker={{ visible: true, width: 6, height: 6 }} animation={{ enable: false }}/>
              {/* Company Sales (Line) */}
              <SeriesDirective type="Spline" name="Company Sales" xName="m" yName="your" dataSource={seriesData} width={3} marker={{ visible: true, width: 7, height: 7 }} fill="#554994" animation={{ enable: false }}/>
              {/* Market Share % (Line on percent axis) */}
              <SeriesDirective type="Spline" name="Market Share %" xName="m" yName="share" yAxisName="percentAxis" dataSource={seriesData} width={2} marker={{ visible: true, shape: 'Diamond', width: 8, height: 8, fill: '#38BE09' }} fill="#850E35" animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    };
    getIncludedChannelsForRegionGrid() {
        const sel = asChannelKeys(this.props.channels ?? []);
        return sel.length ? sel : ALL_CHANNELS;
    }
    getSelectedMonthIdxsForGrid() {
        return this.props.monthIndex === ALL_MONTH
            ? Array.from({ length: 12 }, (_, i) => i)
            : [Math.max(0, Math.min(11, this.props.monthIndex))];
    }
    categoryShareMapForMonth(year, monthIdx) {
    ...
    revenueSliceForRegionChannelMonthNoCategory(year, monthIdx, region, channel) {
        const master = (monthlyRevenueByYear[year] ?? [])[monthIdx];
        const masterActual = master?.actual ?? 0;
        if (!masterActual)
            return 0;
    ...
    buildRegionShareGridData2Level() {
        ...
        for (const rk of includedRegions) {
            ...
            const chTotalsFiltered = { Paid: 0, Organic: 0, Email: 0, Social: 0 };
            ...
            const totalRevenue = Object.keys(chTotalsFiltered)
            ...
            for (const ch of includedChannels) {
                ...
                const catAgg = Object.fromEntries(catsToShow.map(c => [c, 0]));
                for (const m of monthIdxs) {
    ...
    renderRegionShareGrid = () => {
        ...
        const totalRevenueTemplate = (props) => {
        ...
        const channelSplitColumns = [
            includedChannels.includes('Organic') && (<ColumnDirective key="online" field="online" headerText="Paid" headerTemplate={this.headerWithTooltip('Paid', '')} width="110" textAlign="Right" format="C0"/>),
            includedChannels.includes('Paid') && (<ColumnDirective key="ads" field="ads" headerText="Organic" headerTemplate={this.headerWithTooltip('Organic', '')} width="110" textAlign="Right" format="C0"/>),
            includedChannels.includes('Email') && (<ColumnDirective key="email" field="email" headerText="Email" headerTemplate={this.headerWithTooltip('Email', '')} width="110" textAlign="Right" format="C0"/>),
            includedChannels.includes('Social') && (<ColumnDirective key="social" field="social" headerText="Social" headerTemplate={this.headerWithTooltip('Social', '')} width="110" textAlign="Right" format="C0"/>),
        ].filter(Boolean);
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ height: '100%', width: '100%', minHeight: 0 }}>
          <GridComponent id="regionShareGrid" ref={this.RegionShareGridRef} dataSource={parentRows} enableHover={true} allowPaging={false} allowSorting={true} allowResizing={true} allowExcelExport={true} allowPdfExport={true} toolbar={toolbarOptions} toolbarClick={toolbarClick} childGrid={channelChildGridModel} height="100%" width="100%">
            <ColumnsDirective>
              ...
              {channelSplitColumns}
              <ColumnDirective field="totalRevenue" headerText="Total Revenue" headerTemplate={this.headerWithTooltip('Total Revenue', '')} width="130" textAlign="Right" template={totalRevenueTemplate}/>
              <ColumnDirective field="growthPct" headerText="Growth %" headerTemplate={this.headerWithTooltip('Growth %', '')} width="100" textAlign="Right" template={growthTemplate}/>
            </ColumnsDirective>
    ...
    totalRevenueContent = () => {
        ...
        const tone = deltaPct == null ? 'neutral' : deltaPct > 0 ? 'good' : 'bad';
    ...
    roasContent = () => {
        ...
        return this.renderKpiTile({
            ...
            extraContent: (<div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 0.9fr', gap: 12, alignItems: 'center' }}>
    ...
    customerGrowthContent = () => {
        ...
        const tone = currentGrowthPct == null ? 'neutral' : currentGrowthPct > 0 ? 'good' : currentGrowthPct < 0 ? 'bad' : 'neutral';
    ...
    render() {
        ...
        <div className="sidebar-content">
          <DashboardLayoutComponent ref={this.OverviewRef} id="ecommerce_marketing_overview_dashboard" columns={8} cellAspectRatio={1} cellSpacing={[12, 12]} allowResizing={false} allowDragging={false} created={this.OverviewDashboardCreated} mediaQuery="(max-width:950px)">
            <PanelsDirective>
...
const allCategories = Array.from(new Set(mixedProducts.map(p => p.category))).sort();
const CategoryColorByName = Object.fromEntries(allCategories.map((cat, i) => [cat, Palettes.categorySalesTrend[i % Palettes.categorySalesTrend.length]]));
const getCategoryColor = (name) => CategoryColorByName[name] ?? Palettes.categorySalesTrend[0];
const MarkerShapes = ['Circle', 'Diamond', 'Triangle', 'InvertedTriangle', 'Rectangle', 'Pentagon'];
const CategoryMarkerByName = Object.fromEntries(allCategories.map((cat, i) => [cat, MarkerShapes[i % MarkerShapes.length]]));
...
class MarketingClass extends React.Component {
    MarketingRef = React.createRef();
    promoVsNonPromoChartRef = React.createRef();
    discountWaterfallChartRef = React.createRef();
    promoEfficiencyBarRef = React.createRef();
    campaignPerfGridRef = React.createRef();
    campaignRevenueChartRef = React.createRef();
    funnelChartRef = React.createRef();
    state = {
        year: this.props.year ?? 2025,
        monthIndex: this.props.monthIndex ?? ALL_MONTH,
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        const anyStateChanged = prevState.year !== this.state.year ||
    ...
    getFinalPromoChannels() {
        const PROMO = ['Paid', 'Email'];
        const campaign = String(this.props.campaign || 'ALL');
        const campaignChs = (this.CAMPAIGN_PROMO_CHANNELS[campaign] ?? PROMO);
    ...
    getFinalNonPromoChannels() {
        const NONPROMO = ['Organic', 'Social'];
        const campaign = String(this.props.campaign || 'ALL');
        const campaignChs = (this.CAMPAIGN_PROMO_CHANNELS[campaign] ?? NONPROMO);
        const selected = asChannelKeys(this.props.channels ?? []);
    ...
    revenueForMonthByChannels(year, monthIdx, channels) {
    ...
    computeCategoryShareForMonth(year, monthIdx) {
        ...
        let sum = 0;
        for (const c of selectedCats)
            sum += totalUnits > 0 ? (byCat[c] ?? 0) / totalUnits : 0;
        return sum || 0;
    }
    // Denominator orders: all regions, all channels
    denomOrdersAllRegionsAllChannels(year, monthIdx) {
        let sum = 0;
        for (const r of ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica']) {
            const row = buildChannelsByRegionYear(year, r)[monthIdx];
            if (!row)
                continue;
            sum += (row.Paid ?? 0) + (row.Organic ?? 0) + (row.Email ?? 0) + (row.Social ?? 0);
    ...
    includedPromoOrdersForMonth(year, monthIdx) {
        const regions = this.getIncludedRegions();
    ...
    totalPromoRevenueForRange(year, idxs) {
        return idxs.reduce((s, i) => s + this.promoRevenueForMonth(year, i), 0);
    }
    buildPromoSparkSeries(year) {
    ...
    getFinalCampaignChannels() {
        const campaign = String(this.props.campaign || 'ALL');
        const campaignChs = (this.CAMPAIGN_PROMO_CHANNELS[campaign] ?? ALL_CHANNELS);
        const selected = asChannelKeys(this.props.channels ?? []);
    ...
    includedOrdersForMonthByChannels(year, monthIdx, channels, withCategory) {
        let sum = 0;
        for (const region of this.getIncludedRegions()) {
            const row = buildChannelsByRegionYear(year, region)[monthIdx];
            if (!row)
    ...
    computeRevenueSliceForMonthByChannels(year, monthIdx, channels) {
        const masterActual = (monthlyRevenueByYear[year] ?? [])[monthIdx]?.actual ?? 0;
        if (masterActual <= 0)
            return 0;
    ...
    computeSpendSliceForMonthByChannels(year, monthIdx, channels) {
        const rows = spendByYear[year] ?? [];
        const r = rows[monthIdx];
        if (!r)
            return 0;
        const monthSpend = channels.reduce((s, ch) => s + (r[ch] ?? 0), 0);
    ...
    metricsForMonth(year, monthIdx) {
        ...
        if (crRow) {
            ...
            for (const ch of channels) {
                ...
                const crPct = crRow[ch] ?? 0; // conversion rate %
    ...
    promoDiscountPct = 0.18; // 18% avg promo discount
    nonPromoDiscountPct = 0.04; // 4% base/returns
    // Blended COGS rate based on selected product categories for a month
    blendedCogsRateForSelectionMonth(year, monthIdx) {
        const products = buildYearProductSales(year);
    ...
    discountWaterfallForMonth(year, monthIdx) {
    ...
    discountWaterfallForRange(year, idxs) {
    ...
    buildCampaignPerformanceRows() {
        ...
        for (const c of pickCampaigns) {
            ...
            const campaignAllowed = (this.CAMPAIGN_PROMO_CHANNELS[c.key] ?? ALL_CHANNELS);
    ...
    cpcOnlyContent = () => {
        ...
        const deltaPct = (cpc != null && isFinite(cpc) && cpcPrev != null && isFinite(cpcPrev) && Math.abs(cpcPrev) > 0)
        ...
        const currency2 = (v) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v);
    ...
    renderPromoVsNonPromo = () => {
        ...
        if (isYearly) {
          ...
          <div style={{ flex: 1, minHeight: 0, height: "calc(100% - 5px)" }}>
            <ChartComponent id="promoVsNonPromoYearlyFiltered" ref={this.promoVsNonPromoChartRef} height="100%" primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: 'c0', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, minorTickLines: { width: 0 } }} legendSettings={{ visible: true }} tooltip={{ enable: true }} chartArea={{ border: { width: 0 } }} textRender={this.onPromoDataLabelRender} axisLabelRender={this.onPromoAxisLabelRender} tooltipRender={onCurrencyTooltip} load={onChartLoad}>
              <Inject services={[StackingBarSeries, Legend, ChartTooltip, Category, DataLabel]}/>
              <SeriesCollectionDirective>
                <SeriesDirective type="StackingBar" name="Non‑Promo" xName="x" yName="y" dataSource={seriesNonPromo} fill={colorNonPromo} columnWidth={0.7} marker={{ dataLabel: { visible: true, font: { fontWeight: "Bold" } } }} animation={{ enable: false }}/>
                <SeriesDirective type="StackingBar" name="Promo" xName="x" yName="y" dataSource={seriesPromo} fill={colorPromo} columnWidth={0.7} marker={{ dataLabel: { visible: true, font: { fontWeight: "Bold" } } }} animation={{ enable: false }}/>
              </SeriesCollectionDirective>
            </ChartComponent>
          </div>
        </div>);
        ...
        <div style={{ flex: 1, minHeight: 0, height: "calc(100% - 5px)" }}>
          <ChartComponent id="promoVsNonPromoMonthlyFiltered" ref={this.promoVsNonPromoChartRef} height="100%" primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: 'c0', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, minorTickLines: { width: 0 }, minimum: 0 }} legendSettings={{ visible: true }} tooltip={{ enable: true, shared: true }} chartArea={{ border: { width: 0 } }} textRender={this.onPromoDataLabelRender} axisLabelRender={this.onPromoAxisLabelRender} load={onChartLoad}>
            <Inject services={[StackingColumnSeries, Legend, ChartTooltip, Category, DataLabel]}/>
            <SeriesCollectionDirective>
              <SeriesDirective type="StackingColumn" name="Non‑Promo" xName="x" yName="nonPromo" dataSource={points} fill={colorNonPromo} columnWidth={0.5} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>
              <SeriesDirective type="StackingColumn" name="Promo" xName="x" yName="promo" dataSource={points} fill={colorPromo} columnWidth={0.5} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    };
    onDiscountWaterfallAxisLabelRender = (args) => {
        if (args.axis && args.axis.name === 'percentAxis')
            return;
        if (args?.axis?.valueType !== 'Category') {
            args.text = formatCurrency(Number(args.value || 0));
    ...
    renderDiscountWaterfall = () => {
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ flex: 1, minHeight: 0, height: '100%', width: '100%' }}>
          <ChartComponent id="discountWaterfall" ref={this.discountWaterfallChartRef} height="100%" primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: 'c0', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, minorTickLines: { width: 0 } }} tooltip={{ enable: true }} legendSettings={{ visible: false }} chartArea={{ border: { width: 0 } }} textRender={onTextRender} tooltipRender={onCurrencyTooltip} axisLabelRender={this.onDiscountWaterfallAxisLabelRender} load={onChartLoad}>
            <Inject services={[WaterfallSeries, DataLabel, Legend, ChartTooltip, Category]}/>
            <SeriesCollectionDirective>
              <SeriesDirective type="Waterfall" name="Discount Waterfall" dataSource={data} xName="x" yName="y" intermediateSumIndexes={intermediateSumIndexes} sumIndexes={sumIndexes} negativeFillColor={this.wfNegative} summaryFillColor={this.wfSummary} columnWidth={0.6} marker={{ dataLabel: { visible: true, position: 'Outer' } }} connector={{ color: '#9CA3AF', width: 1 }} fill={this.wfPositive} animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    };
    onPromoEfficiencyAxisLabelRender = (args) => {
        if (args.axis && args.axis.name === 'percentAxis')
            return;
        if (args?.axis?.valueType !== 'Category') {
            args.text = formatCurrency(Number(args.value || 0));
    ...
    onPromoEfficiencyDataLabelRender = (args) => {
        const val = args?.point?.y;
        if (typeof val === 'number' && isFinite(val)) {
            args.text = formatCurrency(Number(val));
        }
    };
    promoEfficiencyGrossVsCostsForMonth(year, monthIdx) {
    ...
    promoEfficiencyGrossVsCostsSeries(year, idxs) {
        return idxs.map(i => {
    ...
    renderPromoEfficiency = () => {
        ...
        <div style={{ minHeight: 0, flex: 1, height: "calc(100% - 10px)" }}>
          <ChartComponent id="promoEfficiencyChart" ref={this.promoEfficiencyBarRef} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: 'c0', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, minorTickLines: { width: 0 } }} tooltip={{ enable: true }} legendSettings={{ visible: true }} chartArea={{ border: { width: 0 } }} axisLabelRender={this.onPromoEfficiencyAxisLabelRender} textRender={this.onPromoEfficiencyDataLabelRender} tooltipRender={onCurrencyTooltip} load={onChartLoad}>
            <Inject services={[ColumnSeries, Legend, ChartTooltip, Category, DataLabel]}/>
            <SeriesCollectionDirective>
              {/* Column 1: Gross Sales */}
              <SeriesDirective type="Column" name="Gross Sales" dataSource={data} xName="m" yName="gross" columnWidth={0.85} fill={colorGross} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>
              {/* Column 2: Discounts + COGS */}
              <SeriesDirective type="Column" name="Discounts + COGS" dataSource={data} xName="m" yName="costs" columnWidth={0.85} fill={colorCosts} marker={{ dataLabel: { visible: true } }} animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    ...
    renderCampaignPerformanceTable = () => {
        ...
        <div style={{ flex: 1, minHeight: 0 }}>
          <GridComponent id="campaignPerfGrid" ref={this.campaignPerfGridRef} dataSource={data} allowPaging={false} toolbar={toolbarOptions} toolbarClick={toolbarClick} allowExcelExport={true} allowPdfExport={true} allowResizing={true} height="100%">
            <ColumnsDirective>
              ...
              <ColumnDirective field="roas" headerText="ROAS" headerTemplate={this.headerWithTooltip('ROAS', '')} width="110" textAlign="Right" template={roasTemplate}/>
    ...
    onRevenueCamapignAxisLabelRender = (args) => {
        if (args.axis && args.axis.name === 'percentAxis')
            return;
        if (args?.axis?.valueType !== 'Category') {
            args.text = formatCurrency(Number(args.value || 0));
    ...
    onRevenueCamapignDataLabelRender = (args) => {
        const val = args?.point?.y;
        if (typeof val === 'number' && isFinite(val)) {
            args.text = formatCurrency(Number(val));
        }
    };
    renderRevenueByCampaign = () => {
        const year = this.props.year;
        const idxs = this.selectedMonthIdxs;
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box' }}>
        <ChartComponent id="revenueByCampaignChart" ref={this.campaignRevenueChartRef} primaryXAxis={{
                valueType: 'Category',
                ...
                lineStyle: { width: 0 },
                labelStyle: { size: '11px' }
            }} chartArea={{ border: { width: 0 } }} tooltip={{ enable: true, shared: false, format: '${series.name}: ${point.y}' }} legendSettings={{ visible: false }} textRender={this.onRevenueCamapignDataLabelRender} axisLabelRender={this.onRevenueCamapignAxisLabelRender} tooltipRender={onCurrencyTooltip} load={onChartLoad}>
          <Inject services={[BarSeries, Category, ChartTooltip, DataLabel]}/>
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={dataWithColor} type="Bar" xName="x" yName="y" name="Revenue" columnWidth={0.6} pointColorMapping="color" marker={{ dataLabel: { visible: true, position: 'Outer', format: 'C0' } }} animation={{ enable: false }}/>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    };
    renderFunnelAdClickToPurchase = () => {
        const year = this.props.year;
        ...
        const VIEW_RATE_BY_CHANNEL = {
            Paid: 0.85,
            Organic: 0.65,
            Email: 0.60,
            Social: 0.55
        };
        const ATC_RATE_BY_CHANNEL = {
            Paid: 0.20,
            Organic: 0.12,
            Email: 0.10,
            Social: 0.08
        ...
        for (const mi of monthIdxs) {
            ...
            for (const ch of channels) {
                ...
                const crPct = crRow ? (crRow[ch] ?? 0) : 0;
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box' }}>
        <AccumulationChartComponent id="funnelAdClickToPurchase" ref={this.funnelChartRef} legendSettings={{ visible: true, position: 'Bottom' }} tooltip={{ enable: true }} enableBorderOnMouseMove={false} load={onAccumulationLoad} textRender={onTextRender} tooltipRender={onCurrencyTooltip}>
          <Inject services={[PyramidSeries, AccumulationLegend, AccumulationTooltip, AccumulationDataLabel]}/>
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective type="Pyramid" dataSource={rows} xName="x" yName="y" neckWidth="20%" gapRatio={0.03} palettes={funnelColors} dataLabel={{
                visible: true,
                position: 'Outside'
    ...
    onStockSharedTooltipRender = (args) => {
    ...
    handleCategoryChange = (e) => this.setState({ selectedCategory: String(e.value) });
    handleTopItemsCategoryChange = (e) => this.setState({ topItemsCategory: String(e.value) });
    get catSeriesData() {
        const byCat = this.props.monthIndex === ALL_MONTH
        ...
        const total = (s) => s.data.reduce((a, p) => a + (p.value || 0), 0);
    ...
    ProductMixCategoryColorByName = Object.fromEntries(allCategories.map((cat, i) => [cat, this.ProductMixPalette[i % this.ProductMixPalette.length]]));
    // stable markers for categories (reuse MarkerShapes)
    ProductMixCategoryMarkerByName = Object.fromEntries(allCategories.map((cat, i) => [cat, MarkerShapes[i % MarkerShapes.length]]));
    pmGetCategoryColor = (cat) => this.ProductMixCategoryColorByName[cat] ?? this.ProductMixPalette[0];
    pmGetCategoryMarker = (cat) => {
    ...
    render() {
        return (<div className="Container">
        <div className="sidebar-content">
          <DashboardLayoutComponent ref={this.MarketingRef} id="ecommerce_marketing_promo_dashboard" columns={8} cellAspectRatio={1} cellSpacing={[12, 12]} allowResizing={false} allowDragging={false} created={this.MarketingDashboardCreated} mediaQuery="(max-width:950px)">
            <PanelsDirective>
              <PanelDirective sizeX={4} sizeY={1} row={0} col={0} content={() => this.promoRevenueContent()}/>
              <PanelDirective sizeX={4} sizeY={1} row={0} col={4} content={() => this.roasContent()}/>
              <PanelDirective sizeX={2} sizeY={1} row={2} col={0} content={() => this.impressionsContent()}/>
              <PanelDirective sizeX={2} sizeY={1} row={2} col={2} content={() => this.clicksContent()}/>
              <PanelDirective sizeX={2} sizeY={1} row={2} col={4} content={() => this.ctrContent()}/>
              <PanelDirective sizeX={2} sizeY={1} row={2} col={6} content={() => this.cpcOnlyContent()}/>
              <PanelDirective sizeX={4} sizeY={3} row={4} col={0} header="<div>Promotion vs Non‑Promotion Sales Analysis</div>" content={() => this.renderPromoVsNonPromo()}/>
              <PanelDirective sizeX={4} sizeY={3} row={4} col={4} header="<div>Discount Impact Breakdown</div>" content={() => this.renderDiscountWaterfall()}/>
              <PanelDirective sizeX={8} sizeY={3} row={7} col={0} header="<div>Promotion Efficiency Analysis</div>" content={() => this.renderPromoEfficiency()}/>
              <PanelDirective sizeX={8} sizeY={4} row={10} col={0} header="<div>Campaign Performance Analysis</div>" content={() => this.renderCampaignPerformanceTable()}/>
              <PanelDirective sizeX={3} sizeY={3} row={15} col={0} header="<div>Campaign Revenue Analysis</div>" content={() => this.renderRevenueByCampaign()}/>
              <PanelDirective sizeX={5} sizeY={3} row={15} col={3} header="<div>Ad‑to‑Purchase Funnel Analysis</div>" content={() => this.renderFunnelAdClickToPurchase()}/>
            </PanelsDirective>
          </DashboardLayoutComponent>
        </div>
      </div>);
...
class ProductMixPanel extends React.PureComponent {
    donutRef = React.createRef();
    state = { drillCategory: null };
    refreshProductMixDonut = () => this.donutRef.current?.refresh();
    clearLingeringTooltip = (chartId) => {
        try {
            ...
            const host = this.donutRef.current?.element;
            if (host) {
    ...
    componentDidUpdate(prev, prevState) {
        const propsChanged = prev.year !== this.props.year ||
    ...
    regionFactor(year, monthIdx) {
        ...
        const part = regions[r] ?? 0;
        return sum > 0 ? (part / sum) : 1;
    }
    unitsForProductMonth(year, monthIdx, productName) {
    ...
    productMixDataLabelRender = (args) => {
    ...
    currency0 = (v) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })
    ...
    render() {
        ...
        if (!drilled && catData.length === 0) {
            return (<div style={{ padding: 12, background: '#fff', borderRadius: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          {drilled && (<div style={{ fontWeight: 600, marginLeft: "8px" }}>
              Top Products • {this.state.drillCategory}
            </div>)}
        </div>

        <div style={{ height: 'calc(100% - 10px)' }}>
          <AccumulationChartComponent key={chartId} id={chartId} ref={this.donutRef} tooltip={{ enable: true }} pointClick={!drilled ? this.onPointClick : undefined} legendSettings={{ visible: true, position: 'Right' }} textRender={onTextRender} tooltipRender={onCurrencyTooltip} load={onAccumulationLoad}>
            <Inject services={[PieSeries, AccumulationLegend, AccumulationTooltip, AccumulationDataLabel]}/>
            <AccumulationSeriesCollectionDirective>
              <AccumulationSeriesDirective type="Pie" dataSource={dataSource} xName="legendText" yName="y" innerRadius="60%" palettes={(drilled ? drillPalette : basePalette)} dataLabel={{ visible: true, position: 'Outside', connectorStyle: { length: '10px' }, name: 'y', font: { size: '12px' } }} animation={{ enable: false }} borderRadius={10} border={{ width: 4, color: '#ffffff' }}/>
            </AccumulationSeriesCollectionDirective>
          </AccumulationChartComponent>
        </div>
      </div>);
...
class StockCompositionPanel extends React.PureComponent {
    ...
    componentDidUpdate(prev) {
        const changed = prev.year !== this.props.year ||
            prev.monthIndex !== this.props.monthIndex ||
            prev.region !== this.props.region ||
            prev.brand !== this.props.brand ||
            prev.warehouse !== this.props.warehouse ||
    ...
    onMainChartMouseClick = (args) => {
        ...
        if (target.startsWith('currentStockComposition0_AxisLabel_')) {
            const idx = parseInt(target.split('_').pop() || '-1', 10);
            if (idx >= 0) {
                const chart = this.mainRef.current;
                const txt = chart?.primaryXAxis?.visibleLabels?.[idx]?.text ??
    ...
    refreshCurrentStockComposition() {
        if (this.state.drillCategory) {
    ...
    filteredProducts(year) {
        const prods = buildYearProductSales(year);
        const catsProp = this.props.categories;
        ...
        return prods.filter(p => {
            ...
            if (categoriesSelected && !categoriesSelected.includes(meta.category))
                return false;
            if (brandSelected && String(meta.brand ?? '') !== String(brandSelected))
                return false;
            if (warehouseSelected && String(meta.warehouse ?? '') !== String(warehouseSelected))
                return false;
            return true;
        });
    }
    ...
    // ===== Handlers =====
    onMainChartPointClick = (args) => {
        const cat = String(args?.point?.x ?? '').trim();
        if (cat)
            this.setState({ drillCategory: cat }, () => this.drillRef.current?.refresh());
    };
    onBack = () => this.setState({ drillCategory: null }, () => this.mainRef.current?.refresh());
    // ===== Renderers =====
    renderBadges(over, short) {
        const fmt = (n) => new Intl.NumberFormat('en-US', { maximumFractionDigits: 0 }).format(n);
        return (<div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginBottom: 6 }}>
    ...
    renderMain() {
        ...
        return (<div style={{ height: '100%', width: '100%', borderRadius: 10, padding: 8, display: 'flex', flexDirection: 'column', gap: 8, boxSizing: 'border-box' }}>
        {hasRows && this.renderBadges(overShort.over, overShort.short)}
        <ChartComponent id="currentStockComposition" ref={this.mainRef} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{ labelFormat: '{value}', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, minimum: 0, maximum: yAxisMax, interval: yInterval }} tooltip={{ enable: true, shared: true }} legendSettings={{ visible: true }} pointClick={this.onMainChartPointClick} chartMouseClick={this.onMainChartMouseClick} load={onChartLoad}>
          <Inject services={[StackingColumnSeries, Legend, ChartTooltip, Category, DataLabel]}/>
          <SeriesCollectionDirective>
            <SeriesDirective type="StackingColumn" name="On hand" dataSource={rows} xName="x" yName="onhand" columnWidth={0.5} fill={COLOR_ONHAND} marker={{ dataLabel: { visible: true, position: 'Bottom', font: { fontWeight: 'Bold', size: '9px' } } }} emptyPointSettings={{ mode: 'Zero' }}/>
            <SeriesDirective type="StackingColumn" name="Committed" dataSource={rows} xName="x" yName="committed" columnWidth={0.5} fill={COLOR_COMMITTED} marker={{ dataLabel: { visible: true, position: 'Bottom', font: { fontWeight: 'Bold', size: '9px' } } }} emptyPointSettings={{ mode: 'Zero' }}/>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    }
    renderDrill(category) {
        ...
        return (<div style={{ height: '100%', width: '100%', borderRadius: 10, padding: 8, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button className="e-btn e-outline" onClick={this.onBack}>Back</button>
          <div style={{ fontWeight: 600 }}>{category} • Cover vs Stockouts</div>
        </div>
        <ChartComponent key={`drill-${category}`} id="stockCoverDrill" ref={this.drillRef} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} 
        ...
        primaryYAxis={{ title: 'Stockouts (units)', labelFormat: 'n0', minimum: 0, rangePadding: 'None', lineStyle: { width: 0 }, majorTickLines: { width: 0 } }} axes={[{ name: 'coverAxis', opposedPosition: true, title: 'Cover (days)', minimum: 0, rangePadding: 'None', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }]} tooltip={{ enable: true, shared: true }} legendSettings={{ visible: true }} load={onChartLoad}>
          ...
          <SeriesCollectionDirective>
            <SeriesDirective type="Column" name="Stockouts" dataSource={data} xName="m" yName="stockouts" columnWidth={0.5} fill={COLOR_STOCKOUTS} marker={{ dataLabel: { visible: true, font: { fontWeight: 'Bold' } } }}/>
            {canSpline && (<SeriesDirective type="Spline" name="Cover (days)" dataSource={data} xName="m" yName="coverDays" yAxisName="coverAxis" width={2} marker={{ visible: true, width: 7, height: 7 }} fill={COLOR_COVER}/>)}
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    }
...
class ProductClass extends React.Component {
    ...
    totalSalesSparkRef = React.createRef();
    lowStockSparkRef = React.createRef();
    turnoverSparkRef = React.createRef();
    categoryRevenueChartRef = React.createRef();
    productMixApi;
    currentStockCompositionApi;
    topItemsGridRef = React.createRef();
    lowItemsGridRef = React.createRef();
    state = {
        year: this.props.year ?? 2025,
        monthIndex: this.props.monthIndex ?? ALL_MONTH,
        categories: Array.isArray(this.props.categories) ? this.props.categories : [],
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        const changed = prevState.year !== this.state.year ||
            prevState.monthIndex !== this.state.monthIndex ||
    ...
    TOP_ITEMS_COUNT = 10;
    LOW_PCT = 0.20; // bottom 20% by revenue/units
    LOW_MARGIN_PCT = 0.10; // <=10% margin considered weak
    ...
    aggregateProductMetricsForSelection() {
        const isSingleMonth = this.props.monthIndex !== ALL_MONTH;
    ...
    rangeMetrics(year, idxs) {
        let totalUnits = 0;
        let totalSales = 0;
        let onhandSum = 0;
        const lowProducts = new Set();
        const list = this.filteredProducts();
        for (const i of idxs) {
    ...
    categorySalesTrendContent = () => {
        ...
        const maxY = (data) => Math.max(0, ...data.map(p => Number(p.y) || 0));
        const roundUp = (v) => {
        ...
        if (useSplitAxis) {
            ...
            leftMax = roundUp(Math.max(0, ...series
        ...
        <div style={{ height: "calc(100% - 30px)", width: '100%' }}>
          <ChartComponent ref={this.categoryRevenueChartRef} id="categorySalesTrendChart" chartArea={{ border: { width: 0 } }} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 } }} primaryYAxis={{
                ...formatCurrencyAxis,
                ...
                : []} tooltip={{ enable: true }} legendSettings={{ visible: true, toggleVisibility: true }} highlightMode="Series" axisLabelRender={this.categorySalesTrendAxisLabelRender} tooltipRender={onCurrencyTooltip} load={onChartLoad}>
            <Inject services={[LineSeries, Legend, ChartTooltip, Category, Highlight]}/>
            <SeriesCollectionDirective>
              {series.map(s => {
                const isDom = useSplitAxis && s.name === DOMINANT;
                return (<SeriesDirective key={s.name} type="Line" name={s.name} xName="x" yName="y" dataSource={s.data} width={isDom ? 3 : 2} marker={{
                        visible: true,
                        width: isDom ? 9 : 7,
                        height: isDom ? 9 : 7,
    ...
    topPerformingItemsContent = () => {
        ...
        <div style={{ minHeight: 0, flex: 1 }}>
          <GridComponent id="productTopItemsGrid" ref={this.topItemsGridRef} dataSource={parentRows} allowSorting={true} allowPaging={false} allowResizing={true} gridLines="Horizontal" height="100%" childGrid={childGridModel} sortSettings={{ columns: [{ field: 'shortage', direction: 'Descending' }] }}>
            <ColumnsDirective>
    ...
    lowPerformingItemsContent = () => {
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ flex: 1, minHeight: 0 }}>
          <GridComponent id="lowPerformingItemsGrid" ref={this.lowItemsGridRef} dataSource={rows} allowPaging={false} allowSorting={true} allowResizing={true} height="100%" sortSettings={{ columns: [{ field: 'overstock', direction: 'Descending' }] }}>
            <ColumnsDirective>
    ...
    render() {
        return (<div className="Container">
        <div className="sidebar-content">
          <DashboardLayoutComponent ref={this.ProductRef} id="ecommerce_marketing_product_dashboard" columns={8} cellAspectRatio={1} cellSpacing={[12, 12]} allowResizing={false} allowDragging={false} created={this.ProductDashboardCreated} mediaQuery="(max-width:950px)">
            <PanelsDirective>
              <PanelDirective row={0} col={0} sizeX={4} sizeY={1} content={() => this.totalSalesContent()}/>
              <PanelDirective row={0} col={4} sizeX={2} sizeY={1} content={() => this.unitSoldContent()}/>
              <PanelDirective row={0} col={6} sizeX={2} sizeY={1} content={() => this.aspContent()}/>
              <PanelDirective row={1} col={0} sizeX={8} sizeY={3} header="<div>Category Sales Trend</div>" content={() => this.categorySalesTrendContent()}/>
              <PanelDirective row={4} col={0} sizeX={8} sizeY={3} header="<div>Product Mix Breakdown</div>" content={() => (<ProductMixPanel year={this.props.year} monthIndex={this.props.monthIndex} categories={this.props.categories} brand={this.props.brand} region={this.props.region} warehouse={this.props.warehouse} onReady={(api) => { this.productMixApi = api; }}/>)}/>
              <PanelDirective row={7} col={0} sizeX={8} sizeY={3} header="<div>Current Stock Composition</div>" content={() => (<StockCompositionPanel year={this.props.year} monthIndex={this.props.monthIndex} categories={this.props.categories} brand={this.props.brand} region={this.props.region} warehouse={this.props.warehouse} onReady={(api) => { this.currentStockCompositionApi = api; }}/>)}/>
              <PanelDirective row={10} col={0} sizeX={8} sizeY={5} header="<div>Top Performing Items</div>" content={() => this.topPerformingItemsContent()}/>
              <PanelDirective row={15} col={0} sizeX={8} sizeY={4} header="<div>Low Performing Items</div>" content={() => this.lowPerformingItemsContent()}/>
            </PanelsDirective>
          </DashboardLayoutComponent>
        </div>
      </div>);
...
class ForecastClass extends React.Component {
    ...
    revenueWaterfallRef = React.createRef();
    scenarioSplineRangeAreaRef = React.createRef();
    channelHeatmapRef = React.createRef();
    state = {
    ...
    componentDidUpdate(prevProps) {
        const changed = prevProps.year !== this.props.year ||
            prevProps.monthIndex !== this.props.monthIndex ||
    ...
    /* --------- Formatting helpers --------- */
    clamp = (v, min = -1e15, max = 1e15) => Math.max(min, Math.min(max, v));
    ...
    ForecastGrowth = { 2026: 1.12, 2027: 1.15 };
    ScenarioAbsoluteDelta = {
        Baseline: 0,
        Optimistic: 25000,
        Conservative: -25000
    };
    ...
    static _seasonalityCache;
    static computeSeasonalityIndex() {
    ...
    avgPromoShareForMonth(monthIndex) {
        let promo = 0, total = 0;
        for (const yr of [2022, 2023, 2024, 2025]) {
    ...
    avgPaidShareForMonth(monthIndex) {
        let paid = 0, total = 0;
        for (const yr of [2022, 2023, 2024, 2025]) {
    ...
    getLastYearActual(year, monthIndex) {
        const prev = year - 1;
        const prevRow = (monthlyRevenueByYear[prev] ?? [])[monthIndex];
        if (prevRow?.actual && prevRow.actual > 0)
    ...
    campaignImpact(base, year, monthIndex, scenario) {
        const prev = year - 1;
    ...
    channelImpact(base, year, monthIndex, scenario) {
        const prev = year - 1;
    ...
    buildMonthComponents(year, monthIndex, scenario) {
    ...
    computeSelectionOrderFactor(year, monthIdx) {
        ...
        const includedRegions = (this.props && this.props.region === 'ALL') ? allRegions
            : (isRegionKey(this.props?.region ?? '') ? [this.props.region] : allRegions);
        const selectedChannels = asChannelKeys(this.props?.channels ?? []);
        const channels = selectedChannels.length ? selectedChannels : ALL_CHANNELS;
        let numer = 0;
        for (const r of includedRegions) {
    ...
    projectedTotalForSelection(year, monthIndex, scenario) {
    ...
    projectedRevenueContent = () => {
        ...
        const tone = (deltaPct == null) ? 'neutral' : (deltaPct > 0 ? 'good' : (deltaPct < 0 ? 'bad' : 'neutral'));
        return renderCommonKpiTile({
    ...
    growthPercentageContent = () => {
        ...
        const tone = growthPct == null ? 'neutral' : (growthPct > 0 ? 'good' : 'bad');
    ...
    projectedConversionRateContent = () => {
        // -----------------------------
        ...
        const scenario = (this.props.scenario ?? 'Baseline');
        const selectedChannels = (this.props.channels ?? []);
        const ALL_MONTH = -1; // (matches app constant)
        const monthsArr = months;
        // -----------------------------
        // Region + Channel scope
        // -----------------------------
        const isRegionKeyLocal = (r) => r === 'AsiaPacific' || r === 'Europe' || r === 'NorthAmerica' || r === 'LatinAmerica' || r === 'MiddleEastAfrica';
        const getIncludedRegions = () => {
            if (region === 'ALL')
                return ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica'];
            return isRegionKeyLocal(region) ? [region] : ['AsiaPacific', 'Europe', 'NorthAmerica', 'LatinAmerica', 'MiddleEastAfrica'];
        };
        const asChannelKeysLocal = (arr) => (arr ?? []).filter((x) => x === 'Paid' || x === 'Organic' || x === 'Email' || x === 'Social');
        const getIncludedChannels = () => {
            const ch = asChannelKeysLocal(selectedChannels);
            return ch.length ? ch : ['Paid', 'Organic', 'Email', 'Social'];
        };
        const selectedMonthIdxs = () => {
            if (monthIndex === ALL_MONTH)
                return Array.from({ length: 12 }, (_, i) => i);
            return [Math.max(0, Math.min(11, monthIndex))];
        };
        const scenarioDeltaPP = {
            Baseline: 0,
            Optimistic: 0.25,
            Conservative: -0.25
        };
        const buildProjectedCRTable = (y) => {
        ...
        const projectedCRForMonth = (y, mIdx) => {
        ...
        const badgeTone = deltaPct === null ? 'neutral' : deltaPct > 0 ? 'good' : deltaPct < 0 ? 'bad' : 'neutral';
    ...
    buildAdjustedMonthComponents(year, monthIdx, scenario) {
        // Use base engine
    ...
    axisformatCurrency = (n) => {
        return new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 2, notation: "compact" }).format(n ?? 0);
    };
    revenueWaterfallAxisLabelRender = (args) => {
        if (args.axis && args.axis.name === 'percentAxis')
            return;
        if (args?.axis?.valueType !== 'Category') {
    ...
    revenueWaterfallContent = () => {
        ...
        const currency0 = (v) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(Math.round(v || 0));
        ...
        <div style={{ flex: 1, minHeight: 0, height: '100%' }}>
          <ChartComponent id="revenue-waterfall" ref={this.revenueWaterfallRef} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, labelIntersectAction: 'Trim' }} primaryYAxis={{ labelFormat: 'c0', majorGridLines: { width: 1, color: '#e5e7eb' }, lineStyle: { width: 0 } }} tooltip={{ enable: true }} textRender={onaxisTextRender} tooltipRender={onaxisCurrencyTooltip} axisLabelRender={this.revenueWaterfallAxisLabelRender} load={onChartLoad}>
            <Inject services={[WaterfallSeries, Tooltip, DataLabel, Category]}/>
            <SeriesCollectionDirective>
              <SeriesDirective type='Waterfall' dataSource={data} xName='x' yName='y' intermediateSumIndexes={[]} sumIndexes={[sumIndex]} negativeFillColor={colors.neg} columnWidth={0.6} fill={colors.pos} summaryFillColor={colors.sum} marker={{ dataLabel: { visible: true, position: 'Outer', format: 'c0' } }} animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    ...
    getChannelSharesFromPriorYear(year, monthIndex) {
        ...
        const total = Object.keys(agg).reduce((s, k) => s + (agg[k] ?? 0), 0) || 1;
        return {
            Paid: agg.Paid / total,
    ...
    computeFinalFor(year, monthIdx, scenario) {
        const clampToNonNeg = (v) => Math.max(0, Math.round(v));
        if (year <= 2027) {
    ...
    buildScenarioMonthlyForecast(year, scenario) {
        return months.map((m, idx) => {
            const y = this.computeFinalFor(year, idx, scenario);
            return { x: `${m} ${year}`, m, idx, y, year };
        });
    }
    buildScenarioWindowForecast(year, startIdx, count, scenario) {
        const out = [];
        let y = year;
        let i = startIdx;
    ...
    revenueForecastAxisLabelRender = (args) => {
        if (args.axis && args.axis.name === 'percentAxis')
            return;
        if (args?.axis?.valueType !== 'Category') {
            args.text = formatCurrency(Number(args.value || 0));
    ...
    onScenarioSharedTooltipRender = (args) => {
        const toNumber = (s, abbr) => {
        ...
        const fmtLine = (line) => line.replace(/\$?\s*(-?\d[\d,]*)(\.\d+)?\s*([KMB])?/gi, (_m, intPart, frac = '', abbr) => {
        ...
        if (Array.isArray(args.text)) {
            ...
            const single = fmtLine(String(args.text ?? ''));
            args.text = [single];
        }
    };
    buildScenarioRangeAndLines() {
        const { year, monthIndex } = this.props;
        ...
        const maxVal = Math.max(0, ...range.map(r => r.high || 0), ...baseline.map(d => d.y || 0), ...optimistic.map(d => d.y || 0), ...conservative.map(d => d.y || 0));
    ...
    scenarioForecastContent = () => {
        const { scenario } = this.props;
        const { range, baseline, optimistic, conservative, yMax, windowMode, firstLabel, lastLabel } = this.buildScenarioRangeAndLines();
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box' }}>
        <div style={{ height: "calc(100% - 10px)" }}>
          <ChartComponent id="scenarioSplineRangeArea" ref={this.scenarioSplineRangeAreaRef} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 }, majorTickLines: { width: 0 }, labelIntersectAction: 'Trim' }} primaryYAxis={{
                title: '', labelFormat: 'c0', minimum: 0, maximum: yMax,
                majorGridLines: { width: 1, color: '#e5e7eb' },
                lineStyle: { width: 0 }, majorTickLines: { width: 0 }
            }} legendSettings={{ visible: true }} tooltip={{ enable: true, shared: true }} sharedTooltipRender={this.onScenarioSharedTooltipRender} chartArea={{ border: { width: 0 } }} width="100%" height="100%" axisLabelRender={this.revenueForecastAxisLabelRender} load={onChartLoad}>
            <Inject services={[SplineRangeAreaSeries, SplineSeries, Legend, ChartTooltip, Category]}/>
            <SeriesCollectionDirective>
              {/* Band: Conservative ↔ Optimistic */}
              <SeriesDirective type="SplineRangeArea" name="Range: Conservative ↔ Optimistic" xName="x" high="high" low="low" dataSource={range} opacity={0.35} fill={colorRangeFill} border={{ width: 1, color: colorRangeBorder }} animation={{ enable: false }}/>

              {/* Distinct marker shapes per series */}
              <SeriesDirective type="Spline" name="Optimistic" xName="x" yName="y" dataSource={optimistic} width={2} fill={colorOptimistic} marker={marker(colorOptimistic, 'Triangle')} animation={{ enable: false }}/>
              <SeriesDirective type="Spline" name="Baseline" xName="x" yName="y" dataSource={baseline} width={2} fill={colorBaseline} marker={marker(colorBaseline, 'Circle')} animation={{ enable: false }}/>
              <SeriesDirective type="Spline" name="Conservative" xName="x" yName="y" dataSource={conservative} width={2} fill={colorConservative} marker={marker(colorConservative, 'Diamond')} animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>
      </div>);
    ...
    buildChannelRevenueHeatMapData(year, monthIndex, scenario) {
        if (year !== 2026 && year !== 2027)
            return [];
        const buildForMonth = (mIdx) => {
            ...
            let shareSum = this.HeatMapChannels.reduce((s, ch) => s + sharesRaw[ch], 0);
            if (shareSum <= 0) {
                shareSum = 4; // 4 channels
                this.HeatMapChannels.forEach(ch => (sharesRaw[ch] = 1));
            }
            const alloc = {};
            let allocated = 0;
            for (const ch of this.HeatMapChannels) {
                const frac = sharesRaw[ch] / shareSum;
            ...
            if (delta !== 0) {
                ...
                const ordered = [...this.HeatMapChannels].sort((a, b) => (sharesRaw[b] ?? 0) - (sharesRaw[a] ?? 0));
                let i = 0;
    ...
    renderChannelWeekHeatmap = () => {
        const { year, monthIndex, scenario } = this.props;
        const data = this.buildChannelRevenueHeatMapData(year, monthIndex, scenario);
        return (<div style={{ height: '100%', width: '100%', padding: 8, boxSizing: 'border-box' }}>
        <HeatMapComponent id="channelRevenueHeatmap" ref={this.channelHeatmapRef} dataSource={data} dataSourceSettings={{
                isJsonData: true,
                adaptorType: 'Cell',
                xDataMapping: 'month',
    ...
    render() {
        return (<div className="Container">
        <div className="sidebar-content">
          <DashboardLayoutComponent ref={this.ForecastRef} id="ecommerce_marketing_forecast_dashboard" columns={8} cellAspectRatio={1} cellSpacing={[12, 12]} allowResizing={false} allowDragging={false} created={this.ForecastDashboardCreated} mediaQuery="(max-width:950px)">
            <PanelsDirective>
...
export class EcommerceMarketingDashboard extends SampleBase {
    ...
    handleNavClick = (id) => {
        ...
        if (id === 'promotion') {
            ...
            if (fixedChannels !== this.state.channels)
    ...
    renderDashboard() {
        switch (this.state.selectedId) {
            case 'overview':
                return (<OverviewClass year={this.state.year} monthIndex={this.state.monthIndex} region={this.state.region} channels={Array.isArray(this.state.channels)
                        ? this.state.channels
            ...
            case 'promotion': return <MarketingClass year={this.state.year} monthIndex={this.state.monthIndex} region={this.state.region} channels={Array.isArray(this.state.channels)
                    ? this.state.channels
                    : (this.state.channels === 'ALL' ? [] : [String(this.state.channels)])} categories={Array.isArray(this.state.categories)
                    ? this.state.categories
                    : (this.state.categories === 'ALL' ? [] : [String(this.state.categories)])} campaign={this.state.campaign}/>;
            case 'performance': return <ProductClass year={this.state.year} monthIndex={this.state.monthIndex} region={this.state.region} categories={Array.isArray(this.state.categories)
                    ? this.state.categories
                    : (this.state.categories === 'ALL' ? [] : [String(this.state.categories)])} brand={this.state.brand} warehouse={this.state.warehouse}/>;
            case 'forecast_revenue':
                return <ForecastClass year={this.state.year} monthIndex={this.state.monthIndex} region={this.state.region} channels={Array.isArray(this.state.channels) ? this.state.channels : (this.state.channels === 'ALL' ? [] : [String(this.state.channels)])} scenario={this.state.scenario}/>;
            default: return <OverviewClass year={this.state.year} monthIndex={this.state.monthIndex} region={this.state.region} channels={Array.isArray(this.state.channels)
                    ? this.state.channels
                    : (this.state.channels === 'ALL' ? [] : [String(this.state.channels)])} categories={Array.isArray(this.state.categories)
                    ? this.state.categories
                    : (this.state.categories === 'ALL' ? [] : [String(this.state.categories)])}/>;
    ...
    handleCategoriesChange = (e) => {
        const v = e.value ?? e.target?.value ?? null;
        if (Array.isArray(v)) {
    ...
    handleScenarioChange = (e) => {
        const v = String(e.value ?? 'Baseline');
    ...
    renderGlobalFilters = () => {
        const showCategoryFilter = this.state.selectedId === 'overview' ||
            this.state.selectedId === 'promotion' ||
            this.state.selectedId === 'performance';
        const showChannelFilter = this.state.selectedId === 'overview' ||
            this.state.selectedId === 'promotion' ||
        ...
        const itemTemplateWithTitle = (data) => <div title={data?.text ?? data} className='commerce-commerce-item-template-title'>{data?.text ?? data}</div>;
        return (<div className='commerce-toolbar' style={{ display: 'flex', alignItems: 'center', justifyContent: 'end', flexDirection: 'row', gap: 12, marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div className='commerce-dropdown' style={{ minWidth: 100, width: 100 }}>
            <DropDownListComponent id="globalYear" dataSource={yearOptions} key={`${isForecast ? 'yr-forecast' : 'yr-historic'}-${displayYear}`} value={this.state.year} change={this.handleYearChange} placeholder="Year" popupHeight="220px"/>
          </div>

          <div className='commerce-dropdown' style={{ minWidth: 100, width: 120 }}>
            <DropDownListComponent id="globalMonth" dataSource={[{ text: 'All (Yearly)', value: ALL_MONTH }, ...months.map((m, idx) => ({ text: m, value: idx }))]} fields={{ text: 'text', value: 'value' }} value={this.state.monthIndex} change={(e) => this.setState({ monthIndex: Number(e.value) })} popupHeight="260px"/>
          </div>

          <div className='commerce-dropdown' style={{ minWidth: 100, width: 140 }}>
            <DropDownListComponent id="globalRegion" dataSource={REGION_OPTIONS} fields={{ text: 'text', value: 'value' }} value={this.state.region} change={(e) => this.setState({ region: String(e.value ?? 'ALL') })} popupHeight="260px" itemTemplate={itemTemplateWithTitle}/>
          </div>

          {showChannelFilter && (<div className='commerce-dropdown' style={{ minWidth: 100, width: 130 }}>
          ...
          {showCategoryFilter && (<div className='commerce-dropdown' style={{ minWidth: 100, width: 200 }}>
              <DropDownListComponent id="categoriesTop" dataSource={DROPDOWN_CATEGORY_OPTIONS} fields={{ text: 'text', value: 'value' }} value={this.state.categories} change={this.handleCategoriesChange} popupHeight="260px" itemTemplate={itemTemplateWithTitle}/>
            </div>)}

          {showCampaignFilter && (<div className='commerce-dropdown' style={{ minWidth: 100, width: 190 }}>
              <DropDownListComponent id="campaignFilterTop" dataSource={CAMPAIGN_OPTIONS} fields={{ text: 'text', value: 'value' }} value={this.state.campaign} change={this.handleCampaignChange} popupHeight="260px" itemTemplate={itemTemplateWithTitle}/>
            </div>)}

          {showBrandFilter && (<div className='commerce-dropdown' style={{ minWidth: 100, width: 130 }}>
              <DropDownListComponent id="brandFilter" dataSource={BRAND_OPTIONS} fields={{ text: 'text', value: 'value' }} value={this.state.brand} change={this.handleBrandChange} popupHeight="260px"/>
            </div>)}

          {showWarehouseFilter && (<div className='commerce-dropdown' style={{ minWidth: 100, width: 150 }}>
              <DropDownListComponent id="warehouseFilter" dataSource={WAREHOUSE_OPTIONS} fields={{ text: 'text', value: 'value' }} value={this.state.warehouse} change={this.handleWarehouseChange} popupHeight="260px"/>
            </div>)}

          {showScenarioFilter && (<div className='commerce-dropdown' style={{ minWidth: 100, width: 120 }}>
              <DropDownListComponent id="scenarioFilter" dataSource={scenarioOptions} value={this.state.scenario} change={this.handleScenarioChange} popupHeight="260px"/>
            </div>)}
        </div>
      </div>);
    };
    ...
    onToolbarClicked = (args) => {
        const isMenu = args?.item?.prefixIcon?.includes('e-menu');
        if (isMenu)
            this.onMenuClicked();
    };
        ...
        <div className="control-section" id="target_marketing_dash">
          <div className="col-lg-12 col-sm-12 col-md-12" id="marketing_dashboard_sidebar_section">
            <div id="analytic_head">
              <ToolbarComponent cssClass="ecommerce-dockToolbar" id="ecommerce-dockToolbar" height='65px' clicked={this.onToolbarClicked} key={this.state.selectedId}>
                <ItemsDirective>
            ...
            <div className="commerce-workarea">
              <SidebarComponent id="dockMarketingSideDash" ref={this.sidebarRef} width="240px" enableDock={true} closeOnDocumentClick={false} enableGestures={false} dockSize="60px" type="Push" target=".commerce-content" open={this.onSidebarOpen} close={this.onSidebarClose} created={this.onSidebarCreated}>
                <div className="sidebar-content">
                  {this.withTooltip('Overview', <div className={`commerce-nav-item ${isActive('overview')}`} onClick={() => this.handleNavClick('overview')}>
                      <span className="e-icons e-home" aria-hidden="true"></span>
                      <span className="commerce-nav-text">Overview</span>
                    </div>)}
                  {this.withTooltip('Promotion', <div className={`commerce-nav-item ${isActive('promotion')}`} onClick={() => this.handleNavClick('promotion')}>
                      <span className={this.icon('promotion')} aria-hidden="true"></span>
                      <span className="commerce-nav-text">Promotion</span>
                    </div>)}
                  {this.withTooltip('Product and Sales', <div className={`commerce-nav-item ${isActive('performance')}`} onClick={() => this.handleNavClick('performance')}>
                      <span className={this.icon('performance')} aria-hidden="true"></span>
                      <span className="commerce-nav-text">Product and Sales</span>
                    </div>)}
                  {this.withTooltip('Revenue Forecasting', <div className={`commerce-nav-item ${isActive('forecast_revenue')}`} onClick={() => this.handleNavClick('forecast_revenue')}>
