...
import { AccumulationChartComponent, AccumulationSeriesCollectionDirective, AccumulationSeriesDirective, AccumulationLegend, Legend, PieSeries, AccumulationTooltip, AccumulationDataLabel, ChartComponent, SeriesCollectionDirective, SeriesDirective, ColumnSeries, Category, Tooltip, DataLabel, SplineSeries, StackingColumnSeries, LineSeries, Inject, BarSeries, SparklineComponent, StripLine, WaterfallSeries, AreaSeries, SplineAreaSeries, SparklineTooltip, } from '@syncfusion/ej2-react-charts';
import { useEffect, useMemo, useState } from 'react';
import { AnnotationDirective, Annotations, AnnotationsDirective, AxesDirective, AxisDirective, CircularGaugeComponent, PointerDirective, PointersDirective } from '@syncfusion/ej2-react-circulargauge';
import { SampleBase } from '../common/sample-base';
import { TooltipComponent } from '@syncfusion/ej2-react-popups';
import './dashboard-bold-icon.css';
import './dashboard-light-icon.css';
let financeData = financeDataSource;
...
const companyShareData = financeData?.companyshares?.companyshares_quarterly ?? [];
const profitShareData = financeData?.incomeStatement.series ?? [];
const currentAssetIds = ["cash", "ar", "inventory"];
const currentLiabilityIds = ["ap", "accrued", "shortDebt"];
const debtIds = ["shortDebt", "longDebt"];
const companyShareGroupedByYear = companyShareData.reduce((acc, item) => {
    (acc[item.year] ??= []).push(item);
    return acc;
}, {});
const companyIncomeByYear = profitShareData.reduce((acc, item) => {
    acc[item.year] = item;
    return acc;
}, {});
let quartersData = companyShareGroupedByYear[2025];
const sumItems = (items = []) => items.reduce((s, x) => s + (x.amount ?? 0), 0);
const sumOneQuarter = (q) => sumItems(q);
const sumCurrentAssetItems = (items = []) => items.filter(a => currentAssetIds.includes(a.id)).reduce((s, x) => s + (x.amount ?? 0), 0);
const sumCurrentAssetQuarter = (q) => sumCurrentAssetItems(q);
const sumCurrentLiabilitiesItems = (items = []) => items.filter(a => currentLiabilityIds.includes(a.id)).reduce((s, x) => s + (x.amount ?? 0), 0);
const sumCurrentLiabilitiesQuarter = (q) => sumCurrentLiabilitiesItems(q);
const sumDebtItems = (items = []) => items.filter(a => debtIds.includes(a.id)).reduce((s, x) => s + (x.amount ?? 0), 0);
const sumDebtQuarter = (q) => sumDebtItems(q);
const sumInvesmentItems = (items = []) => items.reduce((s, x) => s + (x.amount ?? 0), 0);
const sumInvesmentQuarter = (q) => sumInvesmentItems(q);
const sumInventoryItems = (items = []) => items.find(a => a.id === "inventory")?.amount ?? 0;
const sumInventoryQuarter = (q) => sumInventoryItems(q);
//stored the total amount based on the each year.
const quarterAllSharesByYear = Object.fromEntries(Object.entries(companyShareGroupedByYear).map(([year, qObj]) => {
    const totals = {
...
const cashInflowData = (financeData.cashInFlows?.series ?? [])
    .flatMap((s) => (s.monthlyseries ?? []).map((m) => ({
...
const cashInFlowGroupCollection = cashInflowData.reduce((acc, item) => {
    const idx = Number(item.year) - 1;
    (acc[idx] ??= []).push(item);
    return acc;
}, []);
const cashInFlowTotals = cashInFlowGroupCollection.map((group) => {
    let sum = 0;
    for (let i = 0; i < 12; i++) {
...
const cashOutFlowGroupCollection = cashOutflowData.reduce((acc, item) => {
    const idx = Number(item.year) - 1;
    (acc[idx] ??= []).push(item);
    return acc;
}, []);
const cashOutFlowTotals = cashOutFlowGroupCollection.map((group) => {
    let sum = 0;
    for (let i = 0; i < 12; i++) {
        sum += group[i]?.monthamount ?? 0;
    }
...
const getAmount = (items, id) => items?.find(x => x.id === id)?.amount ?? 0;
const sum = (o) => Object.values(o).reduce((a, b) => a + b, 0);
const buildChartData = (quarters) => quarters.map((q) => {
    const assets = {
...
function formatCompactCurrencySignificant(n, significantDigits = 3) {
...
const getAxisLabelFromTarget = (targetId) => {
    if (!targetId)
    ...
    if (el) {
        ...
        return { text, year: m ? Number(m[1]) : null };
    }
...
const OverviewDashboard = ({ selectedYear, onYearChange }) => {
    const overviewRef = React.useRef(null);
    const sparkPieRef = React.useRef(null);
    const debtAssetRatioChartRef = React.useRef(null);
    const assetLiabilitiesPieChartRef = React.useRef(null);
    const assetLiabilitiesQuaterlyChartRef = React.useRef(null);
    React.useEffect(() => {
        let timer;
        const refreshAll = () => {
            clearTimeout(timer);
    ...
    const [drillLevel, setDrillLevel] = React.useState('root');
    ...
    const availableQuarters = useMemo(() => Array.from(new Set(chartData.map(d => d.period))), [chartData]);
    const [selectedQuarters, setSelectedQuarters] = useState(availableQuarters);
    const financialFilteredData = useMemo(() => selectedQuarters.length ? chartData.filter(d => selectedQuarters.includes(d.period)) : chartData, [selectedQuarters, chartData]);
    const debtAssetEquityMaxValue = Math.max(...debtRatiosData.flatMap(d => [d.debtToAsset, d.debtToEquity]));
    ...
    let npvPctFormatted = npvPctChange === null ? '—' : (npvPctChange === Infinity ? '∞' : `${npvPctChange.toFixed(1)}%`);
    ...
    const computeIRR = (cashFlows, guess = 0.1, maxIter = 100, tol = 1e-6) => {
        let rate = guess;
    ...
    const initialInvestment = Math.abs(quarterAllSharesByYear[financialAvailableYears[selectedDropdownYear]]?.InitialInvestment
        ?? financeData?.companyshares?.initialInvestment
        ?? 0);
    const initialInvestPrevious = Math.abs(quarterAllSharesByYear[financialAvailableYears[selectedDropdownYear - 1]]?.InitialInvestment
        ?? financeData?.companyshares?.initialInvestment
        ?? 0);
    ...
    let workingPctFormatted = workingPctChange === null ? '—' : (workingPctChange === Infinity ? '∞' : `${workingPctChange.toFixed(1)}%`);
    ...
    const getPctChange = (data, year) => {
        ...
        if (prev === null || cur === null || Math.abs(prev) < 1e-9) {
            return { pct: null, cur, prev, isGreater: null };
        }
        const diff = cur - prev;
    ...
    const gaugeEquity = () => RatioCardEquity("Equity Ratio", formatCurrency(equityRatioValue), "#87A2FB", "#87A2FB", equitySeries, equityComparisonLabel, (equityPctChange?.isGreater ?? false));
    const gaugeQuick = () => RatioCard("Quick Ratio", formatCurrency(quickRatioValue), "#EF9A53", "#EF9A53", quickSeries, quickComparisonLabel, (quickPctChange?.isGreater ?? false));
    const gaugeCurrent = () => RatioCard("Current Ratio", formatCurrency(currentRatioValue), "#7DE5ED", "#7DE5ED", currentSeries, currentComparisonLabel, (currentPctChange?.isGreater ?? false));
    const RatioCardEquity = (title, value, color, bgFill, data, comparisonvalue, isGood) => {
        const chipBg = isGood ? "#dcfce7" : "#fee2e2";
        const chipText = isGood ? "#16a34a" : "#991b1b";
        const arrow = isGood ? '▲' : '▼';
        return (<div id='main-finance-div'>
        <div id='title-finance-card'>{title}</div>
        <div id='card-finance-value'>{value}</div>
      </div>);
    };
    const RatioCard = (title, value, color, bgFill, data, comparisonvalue, isGood) => {
        const chipText = isGood ? "#16a34a" : "#991b1b";
        return (<div id='main-finance-div'>
        <div id='title-finance-card'>{title}</div>
        <div id='card-finance-value'>{value}</div>
    ...
    const MetricInvestmentCard = (title, value, comparisonValue, arrowUp) => {
    ...
    const IRRCard = (title, showtooltip, tooltipValue, value, updownarrow, comparisonvalue, sparkId, previousValue, currentValue, color, pieData, tooltipFormatValue) => {
        ...
        <div className="spark-content" style={{ width: '50%', height: '50%' }}>
          <div style={{ marginTop: 0, width: '100%' }}>
            <SparklineComponent id={`${sparkId}-pie`} ref={sparkPieRef} type="Pie" dataSource={pieData} xName="x" yName="y" width="100%" height="68px" lineWidth={2} palette={sparklinePalette} markerSettings={{ visible: ['All'], size: 2, fill: '#05B3DA' }} tooltipSettings={{ visible: true, format: tooltipFormatValue }}>
    ...
    const assetliabilities = () => {
        ...
        <ChartComponent id="stacked-column-chart" ref={assetLiabilitiesQuaterlyChartRef} load={onChartLoad} textRender={onTextRender} axisLabelRender={onAxisLabelRender} tooltipRender={onCurrencyTooltip} chartArea={{ border: { width: 0 } }} primaryXAxis={{
                valueType: 'Category',
          ...
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={financialFilteredData} xName="period" yName="assets" name="Assets" fill={rootPalette.assets} type="Column" stackingGroup="assets" marker={{ visible: true, dataLabel: { visible: true } }} animation={{ enable: false }}/>
            <SeriesDirective dataSource={financialFilteredData} xName="period" yName="liabilities" name="Liabilities" fill={rootPalette.liabilities} type="Column" stackingGroup="liabilities" marker={{ visible: true, dataLabel: { visible: true } }} animation={{ enable: false }}/>
            <SeriesDirective dataSource={financialFilteredData} xName="period" yName="equity" name="Equity" fill={rootPalette.equity} type="Column" stackingGroup="equity" marker={{ visible: true, dataLabel: { visible: true } }} animation={{ enable: false }}/>
          </SeriesCollectionDirective>
    ...
    const assetliabilitiesPie = () => {
        ...
        const drillPieChart = (key, breakdownLabel) => {
            const quarters = companyShareGroupedByYear[selectedDropdownYear] ?? [];
            const items = quarters.flatMap(q => (q[key] ?? []));
        ...
        const getPieBreakdownFromTarget = (targetId) => {
            if (!targetId)
                return null;
        ...
        return (<div style={{ height: '100%', width: '100%', position: 'relative' }} onClick={() => { if (drillLevel !== 'root')
            buildRootDataWithColors(); }}>
        {drillLevel !== 'root' && (<button onClick={(e) => { e.stopPropagation(); buildRootDataWithColors(); }} style={{ background: 'transparent', position: 'absolute', top: 8, right: 8, zIndex: 5 }}>
            Back
          </button>)}
        <div style={{ paddingTop: 8, textAlign: 'center', fontWeight: 600 }}>{title}</div>
        {drillLevel === 'root' ? (<AccumulationChartComponent ref={assetLiabilitiesPieChartRef} id="asset-liability-equity-pie" load={onAccumulationLoad} legendSettings={{ visible: true, position: 'Bottom' }} tooltip={{ enable: true, format: '${point.x} : ${point.y}' }} height={'100%'} width={'100%'} textRender={onTextRender} chartMouseClick={onChartMouseClick} pointClick={onPointClick} tooltipRender={onCurrencyTooltip}>
            <Inject services={[PieSeries, AccumulationLegend, AccumulationTooltip, AccumulationDataLabel]}/>
            <AccumulationSeriesCollectionDirective>
              <AccumulationSeriesDirective dataSource={pieData} xName="x" yName="y" type="Pie" pointColorMapping="color" innerRadius='40%' dataLabel={{
                    visible: true,
                    format: '${value}',
                    name: 'y',
                    position: 'Outside',
            ...
            <ChartComponent id="drill-column-chart" load={onChartLoad} textRender={onTextRender} axisLabelRender={onAxisLabelRender} tooltipRender={onCurrencyTooltip} height={'85%'} width={'90%'} primaryXAxis={{ valueType: 'Category' }} tooltip={{ enable: true, format: '${point.x} : $${point.y}' }}>
              ...
              <SeriesCollectionDirective>
                <SeriesDirective dataSource={pieData} xName="x" yName="y" type="Bar" pointColorMapping="color" marker={{ dataLabel: { visible: true, format: '${value}', position: 'Outer' } }} animation={{ enable: false }}/>
              </SeriesCollectionDirective>
            </ChartComponent>
          </div>)}
      </div>);
    ...
    const renderAssetHeader = () => (<div className="finance-panel-header">
      <div>Quarterly Comparison of Assets, Liabilities, and Equity</div>
      <MultiSelectComponent id="quarterelement" mode="CheckBox" width='30%' placeholder="Select Quarter" dataSource={availableQuarters} value={selectedQuarters} fields={{ text: 'value', value: 'value' }} change={(args) => setSelectedQuarters(args?.value ?? [])}>
        <Inject services={[CheckBoxSelection]}/>
      </MultiSelectComponent>
    </div>);
    const renderRatioHeader = () => (<div className="finance-panel-header">
      <div>Debt Asset and Equity Ratio</div>
      <MultiSelectComponent id="dbtelement" mode="CheckBox" width='40%' placeholder="Select Ratio" dataSource={debtValues} value={selectedRatio} fields={{ text: 'value', value: 'value' }} change={(args) => {
    ...
    const debtEquityRatio = () => {
        return (<div style={{ height: "100%", width: "100%", padding: 8 }}>
        <ChartComponent id='debtratio' ref={debtAssetRatioChartRef} load={onChartLoad} primaryXAxis={{
          ...
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={debtRatiosData} xName='year' yName='debtToAsset' fill='#87A2FB' name='Debt by Asset Ratio' type='Spline' visible={selectedRatio.length === 0 || selectedRatio.includes('Debt by Asset Ratio')} marker={{ dataLabel: { visible: true, format: 'n3' }, visible: true, width: 8, height: 8, shape: 'Circle' }} tooltipFormat='Debt by Asset Ratio: ${point.y}' animation={{ enable: false }}>
            </SeriesDirective>
            <SeriesDirective dataSource={debtRatiosData} xName='year' yName='debtToEquity' fill='#C539B4' name='Debt by Equity Ratio' type='Spline' visible={selectedRatio.length === 0 || selectedRatio.includes('Debt by Equity Ratio')} marker={{ dataLabel: { visible: true, format: 'n3' }, visible: true, width: 8, height: 8, shape: 'Circle' }} tooltipFormat='Debt by Equity Ratio: ${point.y}' animation={{ enable: false }}>
            </SeriesDirective>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    ...
    return (<div className="Container">
      ...
      <div className="e-card cs-toolbar">
        ...
        <div className="cs-toolbar-right">
          <DropDownListComponent dataSource={financialAvailableYears.filter(y => y !== 2021)} value={selectedDropdownYear} placeholder="Select year" change={onYearChange}/>
        </div>
      </div>

      <DashboardLayoutComponent ref={overviewRef} id="overview_dashboard" style={{ height: '100%', width: '100%' }} columns={8} cellAspectRatio={1} cellSpacing={cellSpacing} allowResizing={false} allowDragging={false} mediaQuery="(max-width:950px)">
        <PanelsDirective>
...
const ProfitLossDashboard = ({ selectedYear, onYearChange }) => {
    const selectedDropdownYear = selectedYear;
    const [detailMode, setDetailMode] = useState(false);
    const [incomePieData, setIncomePieData] = useState([]);
    const [expensePieData, setExpensePieData] = useState([]);
    const [detailTitle, setDetailTitle] = useState('');
    const profitlossRef = React.useRef(null);
    const incomePieRef = React.useRef(null);
    const expensePieRef = React.useRef(null);
    const incomeexpensechartRef = React.useRef(null);
    const operatingProfitRef = React.useRef(null);
    React.useEffect(() => {
        let timer;
        const refreshAll = () => {
            clearTimeout(timer);
    ...
    const buildDetailPies = (year) => {
        ...
        const inc = companyIncomeByYear[year] ?? {};
        // find matching cash flow series for the year (period is 1..n)
        const inflowSeries = (financeData.cashInFlows?.series ?? []).find((s) => Number(s.period) === (idx + 1)) ?? null;
        const outflowSeries = (financeData.cashOutFlows?.series ?? []).find((s) => Number(s.period) === (idx + 1)) ?? null;
        const paletteIn = ['#F675A8', '#87A2FB', '#C539B4', '#73467D', '#D989B5'];
        const paletteOut = ['#E94560', '#FFADBC', '#C539B4', '#87A2FB', '#73467D'];
        const mapBreakdown = (arr, palettes) => (arr ?? []).map((b, ii) => ({
        ...
        if (inc && inc.incomeBreakdown && inc.incomeBreakdown.length) {
            incomePie = inc.incomeBreakdown.map((it, ii) => ({
                x: it.name || `Item ${ii + 1}`,
        ...
        else if (Array.isArray(inflowSeries?.monthlyseries) && inflowSeries.monthlyseries.some((m) => Array.isArray(m.breakdown) && m.breakdown.length)) {
            const agg = {};
        ...
        if (companyIncomeByYear[year] && companyIncomeByYear[year].expenseBreakdown && companyIncomeByYear[year].expenseBreakdown.length) {
            expensePie = companyIncomeByYear[year].expenseBreakdown.map((it, ii) => ({
                x: it.name || `Item ${ii + 1}`,
                y: Math.round(Number(it.amount ?? it.value ?? 0) || 0),
        ...
        else if (Array.isArray(outflowSeries?.monthlyseries) && outflowSeries.monthlyseries.some((m) => Array.isArray(m.breakdown) && m.breakdown.length)) {
            const agg = {};
    ...
    const incomeExpenseProfit = () => {
        ...
        const drillDownPie = (yearStr) => {
            const year = Number(yearStr);
            if (!year || !financialAvailableYears.includes(year))
                return;
            const idx = financialAvailableYears.indexOf(year);
            const inflowSeries = (financeData.cashInFlows?.series ?? []).find((s) => Number(s.period) === (idx + 1)) ?? {};
            const outflowSeries = (financeData.cashOutFlows?.series ?? []).find((s) => Number(s.period) === (idx + 1)) ?? {};
            ...
            const incomePie = Array.isArray(inflowSeries.breakdown) && inflowSeries.breakdown.length
                ? inflowSeries.breakdown.map((b, ii) => ({ x: b.name ?? b.id ?? `Item ${ii + 1}`, y: Math.round(Number(b.amount ?? b.value ?? 0)), color: ['#F675A8', '#87A2FB', '#C539B4'][ii % 3] }))
                : (() => {
            ...
            const expensePie = Array.isArray(outflowSeries.breakdown) && outflowSeries.breakdown.length
                ? outflowSeries.breakdown.map((b, ii) => ({ x: b.name ?? b.id ?? `Item ${ii + 1}`, y: Math.round(Math.abs(Number(b.amount ?? b.value ?? 0))), color: ['#E94560', '#FFADBC', '#C539B4'][ii % 3] }))
                : (() => {
        ...
        if (detailMode) {
            ...
            return (<div style={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ height: '100%', width: '100%' }}>
                <AccumulationChartComponent id="income-breakdown" ref={incomePieRef} load={onAccumulationLoad} textRender={onTextRender} tooltipRender={onCurrencyTooltip} legendSettings={{ visible: true, position: 'Bottom' }} tooltip={{ enable: true }}>
                  <Inject services={[PieSeries, AccumulationLegend, AccumulationTooltip, AccumulationDataLabel]}/>
                  <AccumulationSeriesCollectionDirective>
                    <AccumulationSeriesDirective dataSource={incomePieData} xName="x" yName="y" type="Pie" pointColorMapping="color" dataLabel={{ visible: true, format: '${value}', name: 'y', position: 'Outside' }} animation={{ enable: false }}/>
                  </AccumulationSeriesCollectionDirective>
                </AccumulationChartComponent>
              </div>
            </div>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ height: '100%', width: '100%' }}>
                <AccumulationChartComponent id="expense-breakdown" ref={expensePieRef} load={onAccumulationLoad} textRender={onTextRender} tooltipRender={onCurrencyTooltip} legendSettings={{ visible: true, position: 'Bottom' }} tooltip={{ enable: true }}>
                  <Inject services={[PieSeries, AccumulationLegend, AccumulationTooltip, AccumulationDataLabel]}/>
                  <AccumulationSeriesCollectionDirective>
                    <AccumulationSeriesDirective dataSource={expensePieData} xName="x" yName="y" type="Pie" pointColorMapping="color" dataLabel={{ visible: true, format: '${value}', name: 'y', position: 'Outside' }} animation={{ enable: false }}/>
                  </AccumulationSeriesCollectionDirective>
                </AccumulationChartComponent>
              </div>
            </div>
        ...
        return (<div className='finance-chart-wrap' style={{ padding: 8 }}>
        <ChartComponent ref={incomeexpensechartRef} id="income-expense-year-column" load={onChartLoad} width="100%" height="100%" primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 } }} chartArea={{ border: { width: 0 } }} tooltip={{ enable: true, shared: false, format: '${series.name} : ${point.y}' }} axisLabelRender={onAxisLabelRenderLocal} pointClick={onColumnPointClick} chartMouseClick={onChartMouseClick} textRender={onTextRender} tooltipRender={onCurrencyTooltip}>
          <Inject services={[ColumnSeries, Legend, Tooltip, DataLabel, Category]}/>
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={columnData} xName="year" yName="income" name="Income" type="Column" fill="#F675A8" marker={{ dataLabel: { visible: true, position: 'Outer' } }} animation={{ enable: false }}/>
            <SeriesDirective dataSource={columnData} xName="year" yName="expenses" name="Expenses" type="Column" fill="#FFADBC" marker={{ dataLabel: { visible: true, position: 'Outer' } }} animation={{ enable: false }}/>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    };
    const operatingProfitBreakdown = () => {
        const income = companyIncomeByYear[selectedDropdownYear] ?? {};
        ...
        <ChartComponent id='waterfall' ref={operatingProfitRef} load={onChartLoad} chartArea={{ border: { width: 0 } }} axisLabelRender={onAxisLabelRender} textRender={onTextRender} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 } }}>
          <Inject services={[WaterfallSeries, DataLabel, Category]}/>
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={waterfallData} type='Waterfall' xName="category" yName="change" intermediateSumIndexes={[4]} sumIndexes={[3]} columnWidth={0.7} cornerRadius={cornerRadius} connector={{ color: '#5F6A6A', width: 0.8, dashArray: '1,2' }} pointColorMapping="color" border={{ width: 0.2, color: '#000000' }} negativeFillColor='#E94560' marker={{
                dataLabel: {
                    visible: true,
                    font: { color: '#FFFFFF' },
                },
    ...
    function calculateNetProfit(year) {
        const idx = financialAvailableYears.indexOf(year);
        const income = companyIncomeByYear[year] ?? {}; // <- cast to CompanyIncome
    ...
    function calculateGrossProfit(year) {
        const idx = financialAvailableYears.indexOf(year);
        const income = companyIncomeByYear[year] ?? {};
        const revenue = Number(income.revenue ?? cashInFlowTotals[idx] ?? 0);
        const cogs = Number(income.cogs ?? 0);
    ...
    const grossProfitPrevious = selectedDropdownYear && financialAvailableYears.indexOf(selectedDropdownYear) > 0
        ? calculateGrossProfit(financialAvailableYears[financialAvailableYears.indexOf(selectedDropdownYear) - 1])
        : null;
    const grossProfitPctChange = grossProfitPrevious !== null && Math.abs(grossProfitPrevious) > 0
    ...
    const calcPctChangeLabel = (cur, prev) => {
    ...
    const operatingExpensesPrev = financialAvailableYears.indexOf(selectedDropdownYear) > 0
        ? calculateOperatingExpenses(financialAvailableYears[financialAvailableYears.indexOf(selectedDropdownYear) - 1])
        : null;
    const operatingExpensesPct = operatingExpensesPrev !== null && Math.abs(operatingExpensesPrev) > 0
    ...
    const operatingProfitPrev = financialAvailableYears.indexOf(selectedDropdownYear) > 0
        ? calculateOperatingProfit(financialAvailableYears[financialAvailableYears.indexOf(selectedDropdownYear) - 1])
        : null;
    const operatingProfitPct = operatingProfitPrev !== null && Math.abs(operatingProfitPrev) > 0
    ...
    const operatingExpenses = () => MetricCard("Operating Expenses", formatCurrency(operatingExpensesValue), operatingExpensesFormatted, operatingExpensesIsUp);
    const operatingProfit = () => MetricCard("Operating Profit", formatCurrency(operatingProfitValue), operatingProfitFormatted, operatingProfitIsUp);
    ...
    'profitmargin', // id
    profitIsUp);
    const grossMargin = () => gaugeProfitRatio(Math.min(Math.max(Math.round(grossMarginCurrent), 0), 100), 'Gross Margin', `${grossMarginCurrent.toFixed(1)}%`, Math.round(grossMarginCurrent), grossChangeLabel, '#863A6F', 'grossmargin', grossIsUp);
    const MetricGoodsCard = (title, value, showProgress, progressValue, id, context, text, color) => (<div id='main-finance-div'>
      <div id='title-finance-card'>{title}</div>
      <div id='card-finance-value'>{value}</div>
    </div>);
    const MetricCard = (title, value, comparisonValue, arrowUp) => {
        const chipBg = arrowUp ? '#dcfce7' : '#fee2e2';
    ...
    const gaugeProfitRatio = (pointerValue, title, value, percentageValue, yearComparisonText, pointerColor, id, arrowUp) => {
        // Light track color from the same hue
        const toRGBA = (hex, a = 0.20) => {
        ...
        <CircularGaugeComponent id={id} background="transparent" load={onGaugeLoad} title={title} titleStyle={{ fontWeight: '400', size: '14px' }} height="100%" width="100%" allowMargin={false}>
          <Inject services={[Annotations]}/>
          <AxesDirective>
            <AxisDirective radius='82%' startAngle={270} endAngle={90} minimum={0} maximum={100} majorTicks={{ interval: 10, height: 0, width: 0 }} minorTicks={{ interval: 1, height: 0, width: 0 }} labelStyle={{ format: 'n1', font: { size: '0px' } }} lineStyle={{ width: 0 }}>
              <PointersDirective>
                {/* Track shows the not-filled remainder in a light tint */}
                <PointerDirective value={100} radius="100%" type="RangeBar" color={trackColor} pointerWidth={35} roundedCornerRadius={16} animation={{ enable: false }}/>
                {/* Filled value on top */}
                <PointerDirective value={percentageValue} radius="100%" type="RangeBar" color={pointerColor} pointerWidth={35} roundedCornerRadius={16} animation={{ enable: false }}/>
              </PointersDirective>
              <AnnotationsDirective>
                <AnnotationDirective content={`<div><div><span style="font-size:28px;font-weight:500;"><b> ${percentageValue}% </b></span></div></div>`} zIndex="1" angle={0} radius="20%"/>
                <AnnotationDirective content={`
                  <div><div>
    ...
    return (<div className="Container">
      ...
      <DashboardLayoutComponent ref={profitlossRef} id="profitloss_dashboard" style={{ height: '100%', width: '100%' }} columns={8} cellAspectRatio={1} cellSpacing={cellSpacing} allowResizing={false} allowDragging={false} mediaQuery="(max-width:950px)">
        <PanelsDirective>
...
const CashFlowDashboard = ({ selectedYear, onYearChange }) => {
    const selectedDropdownYear = selectedYear;
    const [cashDrill, setCashDrill] = useState({ type: null });
    const MONTHS = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const cashflowRef = React.useRef(null);
    const debtChartRef = React.useRef(null);
    const burnchartRef = React.useRef(null);
    const inventoryChartRef = React.useRef(null);
    const turnoverChartRef = React.useRef(null);
    const monthlyCashRef = React.useRef(null);
    const overallcashRef = React.useRef(null);
    React.useEffect(() => {
        let timer;
        const refreshAll = () => {
            clearTimeout(timer);
    ...
    const inventoryTurnOverData = useMemo(() => {
        return financialAvailableYears.map((y) => {
            ...
            const inc = companyIncomeByYear[y] ?? {};
    ...
    const availableYears = useMemo(() => Array.from(new Set(overallCashData.map(d => d.year))), []);
    ...
    const filteredTurnOverData = useMemo(() => selectedYears.length
        ? turnoverData.filter(d => selectedYears.includes(d.year))
        : turnoverData, [selectedYears, turnoverData]);
    const filteredInventoryData = useMemo(() => selectedYears.length
        ? inventoryTurnOverData.filter(d => selectedYears.includes(d.year))
        : inventoryTurnOverData, [selectedYears]);
    const buildBurnSeriesFromData = (year) => {
        const periodIndex = financialAvailableYears.indexOf(Number(year)) + 1;
        const outflowSeries = (financeData.cashOutFlows?.series ?? []).find((s) => s.period === periodIndex) ?? null;
        const months = MONTHS;
        // monthly raw amounts (fallback to cashOutFlowGroupCollection monthamounts)
        const monthlyRaw = (outflowSeries?.monthlyseries ?? cashOutFlowGroupCollection[financialAvailableYears.indexOf(year)] ?? []).map((m, i) => ({
            monthIndex: i,
            month: months[i],
        ...
        const categoriesFromAnnual = Array.isArray(outflowSeries?.breakdown) ? outflowSeries.breakdown : [];
        const defaultCats = [
        ...
        const cats = categoriesFromAnnual.length ? categoriesFromAnnual.map((c) => ({ id: String(c.id ?? c.name).toLowerCase(), name: c.name ?? c.id, annual: Number(c.amount ?? c.value ?? 0) })) : defaultCats.map(c => ({ id: c.id, name: c.name, annual: 0 }));
        // if we have annual breakdown but no monthly breakdown, distribute proportionally to monthlyRaw amounts
        const totalMonthlySum = monthlyRaw.reduce((s, m) => s + m.amount, 0) || 0;
        return cats.map((cat, ci) => {
            const monthly = monthlyRaw.map((m) => {
                // priority: per-month breakdown if present
                const md = Array.isArray(m.breakdown) ? m.breakdown.find((b) => String(b.id ?? b.name).toLowerCase() === cat.id) : null;
                if (md)
                    return { month: m.month, value: Number(md.amount ?? md.value ?? 0) };
    ...
    const buildMonthlyDebtFromCompanyShares = (year) => {
        const quarters = companyShareGroupedByYear[year] ?? [];
    ...
    const [yearDrill, setYearDrill] = useState(null); // <-- new state for overallCashByYear drill
    const overallCashByYear = () => {
        ...
        const buildMonthlyForYear = (year) => {
            const idx = financialAvailableYears.indexOf(year);
        ...
        if (yearDrill !== null) {
          ...
          <ChartComponent id='monthly-cash-drill' ref={monthlyCashRef} load={onChartLoad} chartArea={{ border: { width: 0 } }} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 } }} tooltip={{ enable: true }} legendSettings={{ visible: false }} axisLabelRender={onAxisLabelRender} textRender={onTextRender} tooltipRender={onCurrencyTooltip}>
            <Inject services={[ColumnSeries, SplineSeries, Legend, Tooltip, DataLabel, Category]}/>
            <SeriesCollectionDirective>
              <SeriesDirective dataSource={monthlyData} xName='month' yName='balance' name='Cash' type='Column' fill='#F675A8' marker={{ dataLabel: { visible: true, position: 'Outer', format: '${value}' } }} animation={{ enable: false }}/>
              <SeriesDirective dataSource={monthlyData} xName='month' yName='cumulativeNet' name='Net Cash Flow' type='Spline' fill='#87A2FB' marker={{ visible: true }} animation={{ enable: false }}/>
            </SeriesCollectionDirective>
          </ChartComponent>
        </div>);
        }
    ...
    const inventoryTurnover = () => {
        ...
        return (<div style={{ height: "100%", width: "100%", padding: 8 }}>
        <ChartComponent id="inventory-turnover" ref={inventoryChartRef} load={onChartLoad} primaryXAxis={{
                valueType: 'Category',
                majorGridLines: { width: 0 },
          ...
          <SeriesCollectionDirective>
            {/* Turnover = Column */}
            <SeriesDirective type="Column" dataSource={filteredInventoryData} xName="year" yName="turnover" name="Higher turnover - faster inventory movement" columnSpacing={0.25} marker={{ visible: true, dataLabel: { visible: true } }} fill="#C539B4" // reddish as in your image
         border={{ width: 0 }} animation={{ enable: false }}/>

            {/* Target = dashed line across categories */}
            <SeriesDirective type="Line" dataSource={targetData} xName="year" yName="target" name={targetLabel} width={3} dashArray="8 6" fill="#73467D" marker={{ visible: false }} animation={{ enable: false }}/>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    };
    const turnoverChart = () => {
        ...
        return (<div style={{ height: "100%", width: "100%", padding: 8 }}>
        <ChartComponent id="turnover-chart" load={onChartLoad} ref={turnoverChartRef} chartArea={{ border: { width: 0 } }} primaryXAxis={{
                valueType: 'Category',
                majorGridLines: { width: 0 },
            }} primaryYAxis={{
                minimum: 0,
          ...
          <SeriesCollectionDirective>
            ...
            <SeriesDirective dataSource={filteredTurnOverData} xName="year" yName="arTurnover" name="Higher AR Turnover - faster customer collections" type="Column" marker={{ dataLabel: { visible: true, position: 'Middle', format: '{value}' } }} fill="#87A2FB" animation={{ enable: false }}/>

            {/* Accounts Payable Turnover - Column */}
            <SeriesDirective dataSource={filteredTurnOverData} xName="year" yName="apTurnover" name="Lower AP Turnover - slower supplier payments" type="Column" marker={{ dataLabel: { visible: true, position: 'Middle', format: '{value}' } }} fill="#F675A8" animation={{ enable: false }}/>

            {/* AR Target - dashed line with legend label */}
            <SeriesDirective type="Line" dataSource={arTargetData} xName="year" yName="target" width={2} dashArray="6 6" fill="#888" marker={{ visible: false }} animation={{ enable: false }}/>

            {/* AP Target - dashed line with legend label */}
            <SeriesDirective type="Line" dataSource={apTargetData} xName="year" yName="target" width={2} dashArray="4 4" fill="#888" marker={{ visible: false }} animation={{ enable: false }}/>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    };
    const burnLineChart = () => {
        ...
        const monthlyTotals = MONTHS.map((m, mi) => burnSeries.reduce((s, bs) => s + (bs.monthly[mi]?.value ?? 0), 0));
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8 }}>
        <ChartComponent id='burnline' load={onChartLoad} ref={burnchartRef} chartArea={{ border: { width: 0 } }} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 } }} primaryYAxis={{
          ...
          <SeriesCollectionDirective>
            {burnSeries.map((bs, i) => (<SeriesDirective key={bs.id} dataSource={bs.monthly} xName='month' yName='value' type='StackingColumn' name={bs.name} fill={palette[i % palette.length]} columnWidth={0.6} marker={{ dataLabel: { visible: false } }} animation={{ enable: false }}/>))}
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>);
    };
    const debtHeader = () => {
        return (<div className="finance-panel-header">
    ...
    const debtLineChart = () => {
        ...
        return (<div style={{ height: '100%', width: '100%', padding: 8 }}>
        <ChartComponent id='debtline' ref={debtChartRef} load={onChartLoad} chartArea={{ border: { width: 0 } }} primaryXAxis={{ valueType: 'Category', majorGridLines: { width: 0 } }} primaryYAxis={{
                majorGridLines: { width: 0.5 }
          ...
          <SeriesCollectionDirective>
            ...
            <SeriesDirective dataSource={monthlyDebt} xName='month' yName='longDebt' type='SplineArea' name="Long-term Debt" fill={longColor} opacity={0.35} marker={{
                visible: true,
                shape: 'Circle',
                width: 10,
                height: 10,
            ...
            {/* short-term area */}
            <SeriesDirective dataSource={monthlyDebt} xName='month' yName='shortDebt' type='SplineArea' name="Short-term Debt" fill={shortColor} opacity={0.45} marker={{
                visible: true,
                shape: 'Circle',
                width: 10,
                height: 10,
            ...
            <SeriesDirective dataSource={monthlyDebt} xName='month' yName='longDebt' type='Spline' fill="transparent" width={2} marker={{
                visible: false
            }} border={{ width: 2, color: longColor }} animation={{ enable: false }}/>
            <SeriesDirective dataSource={monthlyDebt} xName='month' yName='shortDebt' type='Spline' fill="transparent" width={2} marker={{
                visible: false,
    ...
    const runwaymonthCard = () => {
        ...
        const getMonthlyBurnForIndex = (i) => {
        ...
        const computeRunwayMonthsForIndex = (i) => {
        ...
        const formatRunwayLabel = (months) => {
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'center', width: '100%' }}>
          ...
          <div style={{ flex: '0 0 100px', minWidth: 180, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: '100%', maxWidth: 260 }}>
              <SparklineComponent id="operating-cash-year-spark" height="56px" width="100%" type="Area" axisSettings={{
         ...
         valueType="Category" markerSettings={{
                size: 6,
                fill: '#ffffff',
    ...
    return (<div className="Container">
      ...
      <DashboardLayoutComponent ref={cashflowRef} id="cashflow_dashboard" style={{ height: '100%', width: '100%' }} columns={8} cellAspectRatio={1} cellSpacing={cellSpacing} allowResizing={false} allowDragging={false} mediaQuery="(max-width:950px)">
        <PanelsDirective>
...
export class FinanceDashboard extends SampleBase {
    sidebarRef;
    ...
    toolbarTitleTemplate = () => (<span className="finance-header-title">Finance Dashboard</span>);
    ...
    onSidebarOpen = () => {
        ...
        setTimeout(() => {
            const el = document.getElementById('overview_dashboard');
        ...
        <div className="control-section" id="target_finance_dash">
          <div id='finance-dashboard_sidebar_section'>
            ...
            <div className="cs-workarea">
              <SidebarComponent id="dockFinanceSideDash" ref={this.sidebarRef} width="240px" enableDock={true} closeOnDocumentClick={false} enableGestures={false} dockSize="60px" type="Push" target=".cs-content" open={this.onSidebarOpen} close={this.onSidebarClose} created={this.onSidebarCreated}>
                <div className="sidebar-content">
                  {this.withTooltip('Overview', <div className={`finance-nav-item ${isActive('overview')}`} onClick={() => this.setState({ selectedId: 'overview' })}>
                      <span className="e-icons e-home" aria-hidden="true"></span>
                      <span className="finance-nav-text">Overview</span>
                    </div>)}
                  {this.withTooltip('Financial Performance', <div className={`finance-nav-item ${isActive('profitloss')}`} onClick={() => this.setState({ selectedId: 'profitloss' })}>
                      <span className={this.icon('profit-loss')} aria-hidden="true"></span>
                      <span className="finance-nav-text">Financial Performance</span>
                    </div>)}
                  {this.withTooltip('Cash Flow and Liquidity', <div className={`finance-nav-item ${isActive('cashflow')}`} onClick={() => this.setState({ selectedId: 'cashflow' })}>
