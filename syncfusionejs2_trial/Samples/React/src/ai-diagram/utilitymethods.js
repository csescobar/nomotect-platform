...
function pushWorkingData(diagram) {
    ...
    for (var i = 0; i < diagram.nodes.length; i++) {
        ...
        var nodeData = {
            ...
            level: (_a = node.addInfo) === null || _a === void 0 ? void 0 : _a.level,
            orientation: (_b = node.addInfo) === null || _b === void 0 ? void 0 : _b.orientation,
            hasChild: false,
        };
...
function changeToolbarSelection(tool) {
    var items = ai_text_to_mindmap_1.toolbarObj.items;
...
function onClickDisable(args, node) {
    if (args === false) {
        ai_text_to_mindmap_1.toolbarObj.items[4].disabled = false;
        if (node.addInfo.level !== 0) {
            ai_text_to_mindmap_1.toolbarObj.items[5].disabled = false;
        }
        else {
            ai_text_to_mindmap_1.toolbarObj.items[5].disabled = true;
        }
    }
    else if (args === true) {
        ai_text_to_mindmap_1.toolbarObj.items[4].disabled = true;
        ai_text_to_mindmap_1.toolbarObj.items[5].disabled = true;
    }
}
...
function zoomChange(args) {
    ...
    switch (args.item.text) {
        ...
        case 'Zoom Out':
            ...
            break;
        case 'Zoom to Fit':
            ai_text_to_mindmap_1.diagram.fitToPage();
            break;
        case 'Zoom to 50%':
            if (currentZoom === 0.5) {
                currentZoom = 0;
