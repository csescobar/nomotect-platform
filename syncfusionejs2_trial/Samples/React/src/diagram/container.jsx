...
import { DiagramComponent, DiagramConstraints, Inject, UndoRedo, Snapping, ConnectorBridging } from "@syncfusion/ej2-react-diagrams";
...
let fontColor;
let selectedItems;
function createNode(id, offsetX, offsetY, height, width, content, marginX = 0, marginY = 0) {
    var ports = [];
    if (id == "node5") {
        ports = [
...
function createConnector(id, sourceID, targetID, sourcePortID = '', targetPortID = '', sourceDecorator = null) {
    return {
        id: id,
        type: 'Orthogonal',
        sourceID: sourceID,
...
export class ContainerSample extends SampleBase {
    ...
    render() {
        ...
        <div style={{ width: "100%" }}>
          ...
          <DiagramComponent id="diagram" ref={(diagram) => (diagramInstance = diagram)} width={'100%'} height={'700px'} nodes={nodes} connectors={connectors} selectionChange={selectionChange} constraints={DiagramConstraints.Default | DiagramConstraints.Bridging} rulerSettings={{ showRulers: true, dynamicGrid: true }}>
...
function selectionChange(args) {
    if (args.state === 'Changed') {
        ...
        toolbarItems.forEach(id => {
            const item = toolbarEditor.items.find((item) => item.id === id);
            if (item) {
...
function updateAnnotationValue(value, fontSize, fontFamily) {
    ...
    for (let i = 0; i < selectedItems.length; i++) {
        ...
        const annotations = (object.shape.type === 'Container') ? [object.shape.header.annotation] : object.annotations || [];
        // Iterate through annotations of each node
        for (let j = 0; j < annotations.length; j++) {
            let annotationStyle = annotations[j].style;
