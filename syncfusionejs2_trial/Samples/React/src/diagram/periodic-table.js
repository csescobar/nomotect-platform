...
function createElementNode(element) {
    ...
    var id = element.atomicNumber ? element.atomicNumber.toString() : "element_".concat(element.symbol);
    var annotations = [
    ...
    var node = {
        ...
        constraints: (ej2_react_diagrams_1.NodeConstraints.Default | ej2_react_diagrams_1.NodeConstraints.ReadOnly) & ~ej2_react_diagrams_1.NodeConstraints.Select,
        style: { fill: color },
        addInfo: element
    };
    return node;
...
function createRowsColumns() {
    ...
    nodes.push(group);
    // Period numbers
    for (var period_1 = 0; period_1 < 7; period_1++) {
        var node = {
            id: 'PERIOD_' + period_1,
            offsetX: startX - (cellWidth + cellSpacing) / 3,
            offsetY: (startY + period_1 * (cellHeight + cellSpacing)) + cellHeight / 2,
            width: cellWidth / 3,
            height: cellHeight,
            shape: {
                type: 'Text',
                content: (period_1 + 1).toString()
            },
            style: {
                fontSize: 14,
                color: '#424242',
                fill: period_1 % 2 ? '#fbfcff' : '#dfe6f3',
                fontFamily: 'Roboto',
            },
            borderWidth: 1,
            borderColor: '#d0d7e2',
            constraints: ej2_react_diagrams_1.NodeConstraints.Select | ej2_react_diagrams_1.NodeConstraints.PointerEvents | ej2_react_diagrams_1.NodeConstraints.ReadOnly,
            addInfo: { label: 'PERIOD', cellValue: period_1 + 1 }
        };
        nodes.push(node);
    }
    // Group numbers
    for (var group_1 = 0; group_1 < 18; group_1++) {
        var node = {
            id: 'GROUP_' + group_1,
            offsetX: startX + group_1 * (cellWidth + cellSpacing) + cellWidth / 2,
            offsetY: startY - (cellHeight + cellSpacing) / 2 - 10,
            width: cellWidth,
            height: cellHeight / 3,
            shape: {
                type: 'Text',
                content: (group_1 + 1).toString()
            },
            style: {
                fontSize: 14,
                color: '#424242',
                fill: group_1 % 2 ? '#fbfcff' : '#dfe6f3',
                fontFamily: 'Roboto',
            },
            borderWidth: 1,
            borderColor: '#d0d7e2',
            constraints: ej2_react_diagrams_1.NodeConstraints.Select | ej2_react_diagrams_1.NodeConstraints.PointerEvents | ej2_react_diagrams_1.NodeConstraints.ReadOnly,
            addInfo: { label: 'GROUP', cellValue: group_1 + 1 }
        };
        nodes.push(node);
    }
    return nodes;
...
function initPeriodicTable() {
    ...
    periodicTableData.forEach(function (element) {
        nodes.push(createElementNode(element));
    });
    // Add legend nodes
    nodes.push.apply(nodes, createLegendNodes());
    // Add row and column numbers
    nodes.push.apply(nodes, createRowsColumns());
    // Add block labels and get connectors
    var blockResult = createBlock();
    nodes.push.apply(nodes, blockResult.nodes);
    // Add title
    var titleNode = {
        id: 'title',
    ...
    return { nodes: nodes, connectors: blockResult.connectors };
}
// PeriodicTable component renders the periodic table using React Diagram.
var PeriodicTable = /** @class */ (function (_super) {
    __extends(PeriodicTable, _super);
    function PeriodicTable() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.periodicTableModel = initPeriodicTable();
        // Handle mouse enter event
        _this.handleMouseEnter = function (args) {
            var _a;
            var element = args.actualObject;
            if (((_a = element.shape) === null || _a === void 0 ? void 0 : _a.type) !== 'Text') {
        ...
        _this.handleMouseLeave = function (args) {
            var _a;
        ...
        _this.handleSelectionChange = function (args) {
            var _a;
            if (args.state === 'Changed') {
                var selectednode = args.newValue[0];
                if ((_a = selectednode === null || selectednode === void 0 ? void 0 : selectednode.addInfo) === null || _a === void 0 ? void 0 : _a.label) {
                    var label_1 = selectednode.addInfo.label;
                    var cellValue_1 = selectednode.addInfo.cellValue;
                    // Highlight Selected Group/Period Elements
                    diagramInstance.nodes.forEach(function (node) {
                        var element = node.addInfo;
                        if (element === null || element === void 0 ? void 0 : element.name) {
                            node.style.opacity =
                                ((label_1 === 'PERIOD' && element.period === cellValue_1) ||
                                    (label_1 === 'GROUP' && element.group === cellValue_1 && element.category !== 'lanthanides' && element.category !== 'actinides'))
                                    ? 1 : 0.3;
                        }
                    });
                }
                else {
                    diagramInstance.nodes.forEach(function (node) { return node.style.opacity = 1; });
                }
            }
        };
    ...
    PeriodicTable.prototype.render = function () {
        return (React.createElement("div", { className: "control-pane diagram-control-pane" },
            React.createElement("div", { className: "control-section periodic-table-container", style: { width: "100%", opacity: 0 } },
                React.createElement(ej2_react_diagrams_1.DiagramComponent, { id: "diagram", ref: function (diagram) { return (diagramInstance = diagram); }, width: '100%', height: '700px', nodes: this.periodicTableModel.nodes, connectors: this.periodicTableModel.connectors, constraints: ej2_react_diagrams_1.DiagramConstraints.Default & ~ej2_react_diagrams_1.DiagramConstraints.UndoRedo, snapSettings: { constraints: ej2_react_diagrams_1.SnapConstraints.None }, tool: ej2_react_diagrams_1.DiagramTools.SingleSelect, getConnectorDefaults: this.getConnectorDefaults, selectedItems: { constraints: ej2_react_diagrams_1.SelectorConstraints.None }, mouseEnter: this.handleMouseEnter, mouseLeave: this.handleMouseLeave, selectionChange: this.handleSelectionChange, created: function () {
                        if (diagramInstance) {
                            diagramCreated = true;
                            ...
                            // show diagram
                            setTimeout(function () {
                                var container = document.querySelector('.periodic-table-container');
                                if (container) {
            ...
            React.createElement("div", { id: "action-description" },
                React.createElement("p", null,
                    "This sample demonstrates an interactive Periodic Table of Elements built using the ",
                    React.createElement("a", { href: "https://www.syncfusion.com/react-components/react-diagram", target: "_blank" }, "React Diagram"),
                    ", displaying all known elements with categorization by color, detailed hover information, and highlighting by period or group.")),
