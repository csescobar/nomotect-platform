...
function bindTimingClicks(): void {
  ...
  timingItems.forEach(function (item: HTMLElement) {
    let timingId: number = parseInt(item.getAttribute("data-timing") || '0');
...
function updateBookingSummaryUI(): void {
  ...
  if (seatSelection.TicketCount > 0) {
    ...
    if (totalAmountElem) totalAmountElem.innerText = seatSelection.Amount === null ? '₹0' : "₹" + (seatSelection.Amount + 50);
    ...
    if (feesAmountElem) feesAmountElem.innerText = "+ Fees: ₹" + 50;
  } else {
...
function handleProceed() {
  ...
  let totalAmount: string = (seatSelection.Amount !== null ? seatSelection.Amount + 50 : 50).toFixed(2);
