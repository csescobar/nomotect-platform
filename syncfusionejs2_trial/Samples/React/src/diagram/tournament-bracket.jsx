...
import { DiagramComponent, SnapConstraints, NodeConstraints, DiagramTools } from '@syncfusion/ej2-react-diagrams';
...
function findData(id) {
    const found = tournamentData.find((item) => item.id === id);
    if (found)
        return found;
    return {
        id: id,
...
const angleTiltAmountForRound16ToQuarter = 60;
const angleTiltAmountForQuarterToSemi = 130;
// Node Size configuration
const championNodeSize = { w: 280, h: 200 };
const tournamentNodeSize = { w: 180, h: 100 };
// X-offset positions for tournament nodes
const offsetXIncreaseAmount = 280;
const leftRound16NodesOffsetX = offsetXIncreaseAmount;
const leftQuarterFinalNodesOffsetX = leftRound16NodesOffsetX + offsetXIncreaseAmount;
const leftSemiFinalNodesOffsetX = leftQuarterFinalNodesOffsetX + offsetXIncreaseAmount;
const finalNodeOffsetX = leftSemiFinalNodesOffsetX + offsetXIncreaseAmount;
const rightSemiFinalNodesOffsetX = finalNodeOffsetX + offsetXIncreaseAmount;
const rightQuarterFinalNodesOffsetX = rightSemiFinalNodesOffsetX + offsetXIncreaseAmount;
const rightRound16NodesOffsetX = rightQuarterFinalNodesOffsetX + offsetXIncreaseAmount;
// Y-offset positions for tournament nodes
const offsetYIncreaseAmount = 190;
const round16TopOffsetY = offsetYIncreaseAmount;
const round16UpperMiddleOffsetY = round16TopOffsetY + offsetYIncreaseAmount;
const round16LowerMiddleOffsetY = round16UpperMiddleOffsetY + offsetYIncreaseAmount;
const round16BottomOffsetY = round16LowerMiddleOffsetY + offsetYIncreaseAmount;
const quarterFinalTopOffsetY = (round16TopOffsetY + round16UpperMiddleOffsetY) / 2;
const quarterFinalBottomOffsetY = (round16LowerMiddleOffsetY + round16BottomOffsetY) / 2;
const semiFinalOffsetY = (quarterFinalTopOffsetY + quarterFinalBottomOffsetY) / 2;
const finalNodeOffsetY = semiFinalOffsetY;
const championNodeOffsetY = finalNodeOffsetY - 350;
...
export class TournamentBracket extends SampleBase {
    ...
    render() {
        return (<div className="control-pane">
                ...
                : (<DiagramComponent id="footballResultDiagram" ref={diagram => (diagramInstance = diagram)} width={"100%"} height={"100%"} nodes={nodes} connectors={connectors} getConnectorDefaults={(connector) => {
                        connector.type = 'Straight';
                        connector.sourcePadding = 10;
                        connector.targetPadding = 10;
                        return connector;
                    }} snapSettings={{ constraints: SnapConstraints.None }} scrollSettings={{ canAutoScroll: false, scrollLimit: "Infinity" }} tool={DiagramTools.ZoomPan} load={() => {
                        setTimeout(() => {
                            if (diagramInstance) {
                                diagramInstance.fitToPage({
                                    canZoomIn: true,
                ...
                <div id="action-description">
                    <p>
                        This sample interactively visualizes a UEFA Champions League tournament bracket using the <a href="https://www.syncfusion.com/react-components/react-diagram" target="_blank">React Diagram</a>, showcasing team progression through match results, winning teams, flip-card animations, and informative tooltip.
                    </p>
                </div>
                <div id="description">
                    <p>
