...
let editTimeLineIndex;
let selectedNode;
let selectedUserHandle;
const eventColors = [
...
const baseLine = 300;
let nodes = createTimelineNodes();
let connectors = createTimelineConnectors();
...
function updateNodePositions(fromIndex) {
    ...
    for (let i = fromIndex; i < indexTable.length; i++) {
        const isOdd = (i + 1) % 2 !== 0;
...
const onUserHandleMouseDown = function (args) {
    formValidator.reset();
                ...
                <div id="timeline-container">
                    ...
                    <div className="control-section">
                        <div className="content-wrapper" style={{ width: "100%" }}>
                            <DiagramComponent id="timeline-diagram" ref={(diagramref) => (diagram = diagramref)} width={"100%"} height={"600"} nodes={nodes} connectors={connectors} tool={DiagramTools.ZoomPan | DiagramTools.SingleSelect} constraints={DiagramConstraints.Default & ~DiagramConstraints.PanY} snapSettings={{ constraints: SnapConstraints.None }} selectedItems={selectedItems} selectionChange={selectionChange} onUserHandleMouseDown={onUserHandleMouseDown} click={click} created={created} load={load} commandManager={{
                commands: [
                    {
                        name: 'copy',
                        canExecute: () => false,
                    },
                    {
                        ...
                        canExecute: () => false,
                            ...
                            <DialogComponent ref={(dialogRef) => (dialog = dialogRef)} visible={false} width="300px" isModal={true} target="#timeline-container" animationSettings={{ effect: 'Zoom' }} buttons={dialogButtons} overlayClick={() => { dialog.hide(); }}>
                                <div style={{ padding: '0px 15px 15px 15px' }}>
                                    <form id="form-element" className="form-horizontal">
                                        <div className="form-group">
                                            <label htmlFor="yearInput">Year:</label>
                                            <TextBoxComponent id="yearInput" name="yearInput" ref={(yearInputRef) => (yearTextBox = yearInputRef)} floatLabelType="Never" placeholder="e.g., 2025" cssClass="e-outline"/>
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="titleInput">Title:</label>
                                            <TextBoxComponent id="titleInput" name="titleInput" ref={(titleInputRef) => (titleTextBox = titleInputRef)} floatLabelType="Never" placeholder="Event Title" cssClass="e-outline"/>
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="descriptionInput">Description:</label>
                                            <TextAreaComponent id="descriptionInput" name="descriptionInput" ref={(desRef) => (describeTextBox = desRef)} floatLabelType="Never" cssClass="e-outline" placeholder="Brief description of the event" resizeMode="Vertical" rows={2}/>
                                        </div>
                                        <div className="form-group">
                                            <label htmlFor="imageUrl">Image Url:</label>
                                            <TextBoxComponent id="imageUrl" name="imageUrl" ref={(imgRef) => (imageTextBox = imgRef)} floatLabelType="Never" placeholder="https://example.com/image.jpg" cssClass="e-outline"/>
                                        </div>
                                    </form>
                                </div>
                            </DialogComponent>
