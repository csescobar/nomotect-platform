...
import { DiagramComponent, SnapConstraints, DiagramTools, NodeConstraints, DiagramConstraints, ConnectorConstraints, SelectorConstraints } from '@syncfusion/ej2-react-diagrams';
...
function calculatePosition(period, group, fblock) {
    let x = startX + (group - 1) * (cellWidth + cellSpacing);
    let y = startY + (period - 1) * (cellHeight + cellSpacing);
    if (fblock) {
...
function createElementNode(element) {
    const position = calculatePosition(element.period, element.group, element.block === 'f');
    const color = categoryColors[element.category];
    const id = element.atomicNumber ? element.atomicNumber.toString() : `element_${element.symbol}`;
    const annotations = [
...
function createLegendNodes() {
    ...
    const legendStartY = startY + 10 * (cellHeight + cellSpacing) + 20;
...
export class PeriodicTable extends SampleBase {
    periodicTableModel = initPeriodicTable();
    ...
    render() {
        return (<div className="control-pane diagram-control-pane">
                <div className="control-section periodic-table-container" style={{ width: "100%", opacity: 0 }}>
                    ...
                    <DiagramComponent id="diagram" ref={(diagram) => (diagramInstance = diagram)} width={'100%'} height={'700px'} nodes={this.periodicTableModel.nodes} connectors={this.periodicTableModel.connectors} constraints={DiagramConstraints.Default & ~DiagramConstraints.UndoRedo} snapSettings={{ constraints: SnapConstraints.None }} tool={DiagramTools.SingleSelect} getConnectorDefaults={this.getConnectorDefaults} selectedItems={{ constraints: SelectorConstraints.None }} mouseEnter={this.handleMouseEnter} mouseLeave={this.handleMouseLeave} selectionChange={this.handleSelectionChange} created={() => {
                if (diagramInstance) {
                    diagramCreated = true;
