...
import { DiagramComponent, ConnectorConstraints, NodeConstraints, PortVisibility, FlipDirection, SnapConstraints, DiagramTools } from '@syncfusion/ej2-react-diagrams';
import { updateSampleSection } from "../common/sample-base";
import { DropDownListComponent } from '@syncfusion/ej2-react-dropdowns';
const createWallConnector = (connectors, id, startX, startY, endX, endY, color, thickness) => {
...
const createItem = (nodes, id, x, y, width, height, pathData, options) => {
    const item = {
        id: id,
        width: width,
        height: height,
...
const createBedroomItem = (nodes, id, x, y, width, height, pathData) => {
    const options = { fill: '#E8F4FD', strokeColor: '#2E86AB' };
    if (id === 'BedItem2') {
...
const createKitchenItem = (nodes, id, x, y, width, height, pathData, color) => {
    const options = { fill: color, strokeColor: '#333' };
    if (id === 'KitchenSink') {
        options.fill = '#FFF2E8';
...
const createLivingRoomItem = (nodes, id, x, y, width, height, pathData) => {
    const options = { fill: '#FFF8DC', strokeColor: '#B8860B' };
    if (id === 'Bathroom') {
        options.rotateAngle = 90;
        options.fill = '#E6E6FA';
...
const createBathroomDoor = (nodes, id, x, y, width, height, pathData) => {
    const options = { fill: '#E6E6FA', strokeColor: '#9370DB', rotateAngle: 270, flip: FlipDirection.Vertical };
    createItem(nodes, id, x, y, width, height, pathData, options);
};
const createPortConnections = (connectors) => {
...
const createRestroomSeparator = (connectors) => {
...
const createHallSeparator = (connectors) => {
...
const formatMeasurement = (value, unit) => `${value.toFixed(1)} ${unit}`;
const convertPixelsToSelectedUnit = (pixelValue, unit) => {
    switch (unit) {
...
const createExtensionLine = (connectors, id, startX, startY, endX, endY) => {
    const extensionLine = { id: id, sourcePoint: { x: startX, y: startY }, targetPoint: { x: endX, y: endY }, type: 'Straight', constraints: ConnectorConstraints.None, sourceDecorator: { shape: 'None' }, targetDecorator: { shape: 'None' }, style: { strokeColor: '#0066CC', strokeWidth: 1 }, };
    connectors.push(extensionLine);
};
const createDimensionConnector = (connectors, id, startX, startY, endX, endY, dimensionText, isHorizontal) => {
    const dimensionLine = {
...
const getInitialLayout = () => {
    ...
    const formatAndConvert = (val) => formatMeasurement(convertPixelsToSelectedUnit(val, selectedUnitForInit), selectedUnitForInit);
...
function DistanceDiagram() {
    ...
    const [selectedUnit, setSelectedUnit] = React.useState('in');
    ...
    const updateConnectorAnnotations = React.useCallback((newUnit) => {
        ...
        diagram.connectors.forEach((connector) => {
            if (connector.id?.includes('Measurement') && connector.annotations?.[0]) {
                const content = connector.annotations[0].content;
    ...
    const onUnitChanged = React.useCallback((args) => {
        const newUnit = args.value;
        setSelectedUnit(newUnit);
        updateConnectorAnnotations(newUnit);
    }, [updateConnectorAnnotations]);
    const updateDimensionConnectors = (movedConnector, diagram) => {
        ...
        if (id.startsWith('Separator') || id === 'RestroomSeparator' || id === 'HallSeparator') {
            ...
            const dimConnector = diagram.getObject(measurementId);
            const extStartConnector = diagram.getObject(measurementId + '_ExtStart');
            const extEndConnector = diagram.getObject(measurementId + '_ExtEnd');
            if (!dimConnector || !extStartConnector || !extEndConnector)
                return;
            const wallOuterTop = diagram.getObject('WallOuterTop');
            const wallOuterBottom = diagram.getObject('WallOuterBottom');
            const wallOuterRight = diagram.getObject('WallOuterRight');
    ...
    const onPointChanged = React.useCallback((args) => {
    ...
    return (<div className="control-pane">
            ...
            <div className="control-section diagram-distance-measurement" style={{ opacity: diagramCreated ? 1 : 0 }}>
                <div className="content-wrapper" style={{ width: '100%' }}>
                    <DiagramComponent id="distanceDiagram" ref={diagramRef} width={'100%'} height={'750px'} nodes={diagramData.nodes} connectors={diagramData.connectors} snapSettings={{ constraints: SnapConstraints.ShowLines, gridType: 'Dots', horizontalGridlines: { dotIntervals: [1, 30, 1, 30, 1, 30] }, verticalGridlines: { dotIntervals: [1, 30, 1, 30, 1, 30] } }} rulerSettings={{ showRulers: true }} tool={DiagramTools.SingleSelect} getNodeDefaults={(node) => {
                ...
                if (diagramCreated) {
                    ...
                    <div className="distance-units">
                        ...
                        <div id="unit-dropdown">
                            <DropDownListComponent dataSource={unitOptions} fields={{ text: 'text', value: 'value' }} value={selectedUnit} change={onUnitChanged} width={'155px'}/>
                        </div>
                    </div>
                </div>
            </div>
