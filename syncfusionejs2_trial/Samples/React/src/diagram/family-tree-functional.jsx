...
import { DiagramComponent, Inject, DataBinding, ComplexHierarchicalTree, DiagramTools, SnapConstraints } from '@syncfusion/ej2-react-diagrams';
...
function FamilyTreeDiagram() {
    ...
    const nodeState = useMemo(() => new Map(), []);
    const originalSize = useMemo(() => new Map(), []);
    const hoveredIdRef = useRef(null);
    const DATA_SOURCE = familyTreeData;
    const layout = {
    ...
    const RELATIONS = useMemo(() => buildRelations(DATA_SOURCE), [DATA_SOURCE]);
    const relatedSet = useCallback((personId) => {
    ...
    const getUpdatedTemplate = useCallback((data, UI) => {
...
<div class="${containerCls}">
  <div class="person-card">
    ...
    // HTML node template
    const setNodeTemplate = useCallback((node) => {
    ...
    // Connector Defaults
    const getConnectorDefaults = useCallback((connector) => {
    ...
    const expandNodeWidth = useCallback((id) => {
    ...
    const restoreNodeSize = useCallback((id) => {
        const diagram = diagramRef.current;
        if (!diagram)
            return;
        const node = diagram.getObject(id);
    ...
    const renderNode = useCallback((id) => {
        ...
        if (!diagram)
            return;
        const node = diagram.getObject(id);
        if (!node || node.data?.Type === 'Union')
            return;
        const data = node.data;
        const UI = nodeState.get(id) ?? {};
        node.shape = { type: 'HTML', content: getUpdatedTemplate(data, UI) };
    }, [getUpdatedTemplate, nodeState]);
    const paintConnectors = useCallback((nodeSet) => {
        const diagram = diagramRef.current;
        if (!diagram)
            return;
    ...
    const focusHover = useCallback((id) => {
        const diagram = diagramRef.current;
        if (!diagram)
            return;
    ...
    return (<div className="control-pane" style={{ width: "100%", opacity: diagramCreated ? 1 : 0 }}>
            <style>{familytreeCss}</style>
            <div className="control-section family-tree">
                <DiagramComponent id="diagram" ref={diagramRef} width={'100%'} height={'600px'} snapSettings={{ constraints: SnapConstraints.None }} tool={DiagramTools.ZoomPan} layout={layout} dataSourceSettings={dataSourceSettings} setNodeTemplate={setNodeTemplate} getConnectorDefaults={getConnectorDefaults} mouseEnter={onMouseEnter} mouseLeave={onMouseLeave} dataLoaded={ondataLoaded}>
