...
import { DiagramComponent, SymbolPaletteComponent, Node, DiagramTools, ImportAndExportVisio, BpmnDiagrams, UndoRedo, Inject } from "@syncfusion/ej2-react-diagrams";
...
function createNode(id, content, offsetY, shape = 'Process', offsetX = 400, width = 100, height = 50, ports) {
    const node = {
        id,
...
export class VisioImportExport extends SampleBase {
    rendereComplete() {
        ...
        setTimeout(() => {
            // show diagram
            const container = document.querySelector('.visio-import-container');
            if (container) {
    ...
    render() {
        ...
        <div className="control-section visio-import-container" style={{ opacity: 0 }}>
          <div style={{ width: "100%" }}>
            <div style={{ display: 'none' }}>
              <UploaderComponent id="fileUpload" type="file" showFileList={false} asyncSettings={asyncSettings} success={onUploadSuccess} multiple={false} allowedExtensions={".vsdx"} ref={(uploaderRef) => (uploaderInstance = uploaderRef)}></UploaderComponent>
            </div>
            <div className="db-toolbar-container">
              <ToolbarComponent ref={(toolbar) => (toolbarEditor = toolbar)} id="toolbar_diagram" clicked={toolbarClick} overflowMode={'Scrollable'} width={'100%'}>
                <ItemsDirective>
                  ...
                  <ItemDirective type='Separator'/>
                  <ItemDirective disabled={true} prefixIcon='e-icons e-undo' tooltipText='Undo' align="Left" id="Undo"/>
                  <ItemDirective disabled={true} prefixIcon='e-icons e-redo' tooltipText='Redo' align="Left" id="Redo"/>
                  <ItemDirective type='Separator'/>
            ...
            ]} width={"100%"} height={"700px"} symbolHeight={50} symbolWidth={50} getSymbolInfo={(symbol) => {
                return { fit: true };
            ...
            <div id="diagram-space" className="sb-mobile-diagram">
              <DiagramComponent id="diagram" ref={diagram => (diagramInstance = diagram)} width={"100%"} height={"700px"} nodes={nodes} connectors={connectors} diagramImporting={diagramImporting} diagramExporting={diagramExporting} historyChange={historyChange} selectionChange={(args) => {
        ...
        dragEnter={(args) => {
            ...
            <ToastComponent ref={toast => toastInstance = toast} id="toast" position={{ X: "Right", Y: "Bottom" }}>
            </ToastComponent>
          </div>
        </div>
