...
// Interface for unit dropdown options
interface UnitOption {
    text: string;
    value: string;
}

export class DistanceDiagram extends SampleBase<{}, {}> {
    private diagramInstance = React.createRef<DiagramComponent>();
    private nodes: NodeModel[] = [];
    private connectors: ConnectorModel[] = [];
    private selectedUnit: string = 'in';
    private diagramCreated: boolean;
    ...
    private createBedroomItem = (id: string, x: number, y: number, width: number, height: number, pathData: string): void => {
        let options: ItemOptions = { fill: '#E8F4FD', strokeColor: '#2E86AB' };
    ...
    private createKitchenItem = (id: string, x: number, y: number, width: number, height: number, pathData: string, color: string): void => {
        let options: ItemOptions = { fill: color, strokeColor: '#333' };
    ...
    private createLivingRoomItem = (id: string, x: number, y: number, width: number, height: number, pathData: string): void => {
        let options: ItemOptions = { fill: '#FFF8DC', strokeColor: '#B8860B' };
    ...
    private createBathroomDoor = (id: string, x: number, y: number, width: number, height: number, pathData: string): void => {
        let options: ItemOptions = { fill: '#E6E6FA', strokeColor: '#9370DB', rotateAngle: 270, flip: FlipDirection.Vertical };
        this.createItem(id, x, y, width, height, pathData, options);
    }
    ...
    private createDimensionConnector = (id: string, startX: number, startY: number, endX: number, endY: number, dimensionText: string, isHorizontal: boolean): void => {
    ...
    private addSpecificSeparatorMeasurement = (separator: ConnectorModel): void => {
        ...
        switch (separator.id) {
            case 'Separator1': distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x); yOffset = wallOuterBottom.sourcePoint.y + 40; this.createDimensionConnector(separator.id + 'Measurement', separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset, this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), true); break;
            case 'Separator3': distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x); yOffset = wallOuterBottom.sourcePoint.y + 20; this.createDimensionConnector(separator.id + 'Measurement', separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset, this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), true); break;
            case 'HallSeparator': distance = Math.abs(separator.targetPoint.y - separator.sourcePoint.y); xOffset = 270; this.createDimensionConnector(separator.id + 'Measurement', xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y, this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), false); break;
            case 'Separator4': distance = Math.abs(separator.targetPoint.x - separator.sourcePoint.x); yOffset = wallOuterTop.sourcePoint.y - 20; this.createDimensionConnector(separator.id + 'Measurement', separator.sourcePoint.x, yOffset, separator.targetPoint.x, yOffset, this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), true); break;
            case 'Separator2': distance = Math.abs(separator.targetPoint.y - separator.sourcePoint.y); xOffset = wallOuterRight.sourcePoint.x + 30; this.createDimensionConnector(separator.id + 'Measurement', xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y, this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), false); break;
            case 'RestroomSeparator': distance = Math.abs(separator.targetPoint.y - separator.sourcePoint.y); xOffset = wallOuterRight.sourcePoint.x + 60; this.createDimensionConnector(separator.id + 'Measurement', xOffset, separator.sourcePoint.y, xOffset, separator.targetPoint.y, this.formatMeasurement(this.convertPixelsToSelectedUnit(distance)), false); break;
        }
    }

    private addMeasurementConnectorsRuntime = (): void => {
        let allSeparators = this.connectors.filter((c) => c.id.startsWith('Separator') || c.id === 'RestroomSeparator' || c.id === 'HallSeparator');
        allSeparators.forEach((separator) => { this.addSpecificSeparatorMeasurement(separator); });
    }
    ...
    private addOverallMeasurementConnectors = (): void => {
        ...
        if (wallOuterTop && wallOuterLeft && wallOuterBottom && wallOuterRight) {
            let totalWidth = Math.abs(wallOuterRight.targetPoint.x - wallOuterLeft.sourcePoint.x);
            ...
            let totalHeight = Math.abs(wallOuterBottom.targetPoint.y - wallOuterTop.sourcePoint.y);
            let heightText = this.formatMeasurement(this.convertPixelsToSelectedUnit(totalHeight));
    ...
    private createInitialLayout = (): void => {
        ...
        this.createLivingRoomItem('MainDoor',630,586,90,40,'M 76 56.54 L 76 63 L 0 63 L 0 56.54 M 76 56.54 L 0 56.54 M 76 56.54 L 76 0 L 75.46 0 C 54.77 0 38 25.31 38 56.54 M 76 56.54 L 38 56.54 M 0 56.54 L 0 0 L 0.54 0 C 21.23 0 38 25.31 38 56.54 M 0 56.54 L 38 56.54 M 38 62.6 L 38 56.54');
        this.createRestroomSeparator();
        this.createBathroomDoor('bathDoor1',802,456,45,45,'M 0 26.92 L 0 30 L 28 30 L 28 26.92 M 0 26.92 L 28 26.92 M 0 26.92 C 0 12.05 12.36 0 27.61 0 L 28 0 L 28 26.92');
        this.createHallSeparator();
        this.addMeasurementConnectorsRuntime();
        this.addOverallMeasurementConnectors();
    }
    
    private onUnitChanged = (args: ChangeEventArgs): void => {
    ...
    private updateConnectorAnnotationsFromStoredDistances = (): void => {
        if (!this.diagramInstance.current.connectors) return;
        this.diagramInstance.current.connectors.forEach((connector: ConnectorModel) => {
            if (connector.id.includes('Measurement') && connector.annotations && connector.annotations.length > 0) {
                let content = connector.annotations[0].content as string;
                let parts = content.split(' ');
                if (parts.length > 1) {
                    let value = parseFloat(parts[0]);
                    let currentUnit = parts[1];
                    let convertedValue = this.convertBetweenUnits(value, currentUnit, this.selectedUnit);
                    connector.annotations[0].content = this.formatMeasurement(convertedValue);
                }
            }
        });
        this.diagramInstance.current.dataBind();
    }

    private formatMeasurement = (value: number): string => { return value.toFixed(1) + ' ' + this.selectedUnit; }
    ...
    private convertBetweenUnits = (value: number, fromUnit: string, toUnit: string): number => {
        ...
        switch (toUnit) {
            ...
            default: return inchValue;
        }
    }
    
    private onPointChanged = (args: IConnectionChangeEventArgs): void => {
        this.updateDimensionConnectors(args.connector as Connector);
    }
    
    private updateDimensionConnectors = (movedConnector: Connector): void => {
        ...
        if (id.startsWith('Separator') || id === 'RestroomSeparator' || id === 'HallSeparator') {
            ...
            let dimConnector = this.diagramInstance.current.getObject(measurementId) as Connector;
            let extStartConnector = this.diagramInstance.current.getObject(measurementId + '_ExtStart') as Connector;
            let extEndConnector = this.diagramInstance.current.getObject(measurementId + '_ExtEnd') as Connector;

            if (!dimConnector || !extStartConnector || !extEndConnector) return;

            let wallOuterTop = this.diagramInstance.current.getObject('WallOuterTop') as Connector;
            let wallOuterBottom = this.diagramInstance.current.getObject('WallOuterBottom') as Connector;
            let wallOuterRight = this.diagramInstance.current.getObject('WallOuterRight') as Connector;
            ...
            dimConnector.annotations![0].content = this.formatMeasurement(this.convertPixelsToSelectedUnit(distance));
    ...
    render() {
        const unitOptions: UnitOption[] = [{ text: 'Feet (ft)', value: 'ft' }, { text: 'Inches (in)', value: 'in' }, { text: 'Centimeters (cm)', value: 'cm' }];

        return (
            <div className="control-pane">
                <style>{sampleCss}</style>
                <div className="control-section diagram-distance-measurement" style={{ width: '100%', opacity: 0 }}>
                    <div className="content-wrapper">
                        <DiagramComponent
                            id="distanceDiagram"
                            ref={this.diagramInstance}
                            width={'100%'}
                            height={'750px'}
                            nodes={this.nodes}
                            ...
                            getNodeDefaults={(node: NodeModel): NodeModel => { node.constraints = NodeConstraints.None; return node; }}
                            created={() => {
                                this.diagramCreated = true;
                                this.diagramInstance.current.fitToPage();
                                setTimeout(() => {
                                const container = document.querySelector('.diagram-distance-measurement') as HTMLElement;
                            ...
                            sourcePointChange={this.onPointChanged}
                            targetPointChange={this.onPointChanged}
                        >
                        </DiagramComponent>
