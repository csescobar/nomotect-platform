...
import { DiagramComponent, SnapConstraints, DiagramTools, NodeConstraints, PortVisibility, Rect, ConnectorConstraints } from '@syncfusion/ej2-react-diagrams';
...
const SCROLL_PADDING = { right: 50, bottom: 50 };
...
export class SerpentineDiagram extends SampleBase {
    ...
    renderSerpentineLayout = () => {
        ...
        const effectiveWidth = diagram.element.clientWidth / zoom;
        const nodes = [];
        const connectors = [];
        ...
        for (let i = 0; i < nodes.length - 1; i++) {
            ...
            if (isRowChange) {
                ...
                const controlX = sourceNode.offsetX + sign * ((NODE_SIZE / 2) + CURVE_RADIUS + (2 * CURVE_BOW_OFFSET));
                connector.segments = [{
                        type: 'Bezier',
    ...
    render() {
        return (<div className="control-pane serpentine-diagram-container">
                <style>{`
                ...
                return (<ButtonComponent key={id} id={id} isPrimary={this.state.currentZoom === level} onClick={() => this.handleZoom(level)}>
                                        {level}x
                                    </ButtonComponent>);
            })}
                        </div>
                        <div>
                           <DiagramComponent id="serpentineDiagram" ref={d => (this.diagramInstance = d)} width="100%" height="600px" className="diagram-container" snapSettings={{ constraints: SnapConstraints.None }} scrollSettings={{ scrollableArea: SCROLL_AREA, padding: SCROLL_PADDING }} tool={DiagramTools.ZoomPan} nodes={[]} connectors={[]}/>
                        </div>
                    </div>
                </div>
