...
export class DistanceDiagram extends SampleBase {
    diagramInstance = React.createRef();
    nodes = [];
    connectors = [];
    selectedUnit = 'in';
    diagramCreated;
    ...
    createBedroomItem = (id, x, y, width, height, pathData) => {
        let options = { fill: '#E8F4FD', strokeColor: '#2E86AB' };
        if (id === 'BedItem2') {
    ...
    createKitchenItem = (id, x, y, width, height, pathData, color) => {
        let options = { fill: color, strokeColor: '#333' };
        if (id === 'KitchenSink') {
            options.fill = '#FFF2E8';
    ...
    createLivingRoomItem = (id, x, y, width, height, pathData) => {
        let options = { fill: '#FFF8DC', strokeColor: '#B8860B' };
        if (id === 'Bathroom') {
            options.rotateAngle = 90;
            options.fill = '#E6E6FA';
    ...
    createBathroomDoor = (id, x, y, width, height, pathData) => {
        let options = { fill: '#E6E6FA', strokeColor: '#9370DB', rotateAngle: 270, flip: FlipDirection.Vertical };
        this.createItem(id, x, y, width, height, pathData, options);
    };
    ...
    updateConnectorAnnotationsFromStoredDistances = () => {
        if (!this.diagramInstance.current.connectors)
            return;
        this.diagramInstance.current.connectors.forEach((connector) => {
    ...
    formatMeasurement = (value) => { return value.toFixed(1) + ' ' + this.selectedUnit; };
    ...
    updateDimensionConnectors = (movedConnector) => {
        ...
        if (id.startsWith('Separator') || id === 'RestroomSeparator' || id === 'HallSeparator') {
            ...
            let dimConnector = this.diagramInstance.current.getObject(measurementId);
            let extStartConnector = this.diagramInstance.current.getObject(measurementId + '_ExtStart');
            let extEndConnector = this.diagramInstance.current.getObject(measurementId + '_ExtEnd');
            if (!dimConnector || !extStartConnector || !extEndConnector)
                return;
            let wallOuterTop = this.diagramInstance.current.getObject('WallOuterTop');
            let wallOuterBottom = this.diagramInstance.current.getObject('WallOuterBottom');
            let wallOuterRight = this.diagramInstance.current.getObject('WallOuterRight');
    ...
    render() {
        const unitOptions = [{ text: 'Feet (ft)', value: 'ft' }, { text: 'Inches (in)', value: 'in' }, { text: 'Centimeters (cm)', value: 'cm' }];
        return (<div className="control-pane">
                <style>{sampleCss}</style>
                <div className="control-section diagram-distance-measurement" style={{ width: '100%', opacity: 0 }}>
                    <div className="content-wrapper">
                        <DiagramComponent id="distanceDiagram" ref={this.diagramInstance} width={'100%'} height={'750px'} nodes={this.nodes} connectors={this.connectors} snapSettings={{ constraints: SnapConstraints.ShowLines, gridType: 'Dots', horizontalGridlines: { dotIntervals: [1, 30, 1, 30, 1, 30] }, verticalGridlines: { dotIntervals: [1, 30, 1, 30, 1, 30] } }} rulerSettings={{ showRulers: true }} tool={DiagramTools.SingleSelect} getNodeDefaults={(node) => { node.constraints = NodeConstraints.None; return node; }} created={() => {
                this.diagramCreated = true;
                this.diagramInstance.current.fitToPage();
                setTimeout(() => {
                    const container = document.querySelector('.diagram-distance-measurement');
                    if (container)
                    ...
                    if (this.diagramCreated) {
                        ...
                        <div className="distance-units">
                            ...
                            <div id="unit-dropdown">
                                <DropDownListComponent dataSource={unitOptions} fields={{ text: 'text', value: 'value' }} value={this.selectedUnit} change={this.onUnitChanged} width={'155px'}/>
                            </div>
                        </div>
                    </div>
                </div>
