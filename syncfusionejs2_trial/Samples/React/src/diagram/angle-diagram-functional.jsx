...
import { DiagramComponent, NodeConstraints, ConnectorConstraints, SnapConstraints, SelectorConstraints, } from '@syncfusion/ej2-react-diagrams';
...
function createTextNode(id, width, height, offsetX, offsetY, content, style, constraints = NodeConstraints.None) {
    return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'Text', content }, style };
}
function createNativeNode(id, width, height, offsetX, offsetY, svgContent, constraints = NodeConstraints.None) {
    return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'Native', content: svgContent } };
}
function createImageNode(id, width, height, offsetX, offsetY, source, rotateAngle, constraints, pivotX, pivotY) {
    const node = { id, width, height, offsetX, offsetY, shape: { type: 'Image', source }, style: { fill: 'transparent', strokeColor: 'transparent' } };
...
function createEllipseNode(id, width, height, offsetX, offsetY, fill, strokeColor, strokeWidth, constraints = NodeConstraints.None) {
    return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'Basic', shape: 'Ellipse' }, style: { fill, strokeColor, strokeWidth } };
}
function createHtmlNode(id, width, height, offsetX, offsetY, html, constraints = NodeConstraints.None) {
    return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'HTML', content: html } };
}
function createRectNode(id, width, height, offsetX, offsetY, fill, strokeColor, strokeWidth, constraints = NodeConstraints.None) {
    return { id, width, height, offsetX, offsetY, constraints, style: { fill, strokeColor, strokeWidth } };
}
function createConnectorBezier(id, spx, spy, tpx, tpy, c1x, c1y, c2x, c2y, strokeColor, strokeWidth, dash, opacity) {
    return { id, zIndex: 1, type: 'Bezier', constraints: ConnectorConstraints.None, sourcePoint: { x: spx, y: spy }, targetPoint: { x: tpx, y: tpy }, segments: [{ type: 'Bezier', point1: { x: c1x, y: c1y }, point2: { x: c2x, y: c2y } }], style: { strokeColor, strokeWidth, strokeDashArray: dash, opacity }, sourceDecorator: { shape: 'None' }, targetDecorator: { shape: 'None' } };
}
...
function initializeAngleControls() {
    ...
    if (inputEl) {
        ['pointerdown', 'mousedown', 'touchstart', 'click'].forEach((evt) => inputEl.addEventListener(evt, (e) => e.stopPropagation()));
...
function calculateSolarPosition() {
    ...
    const dayOfYear = Math.ceil((selectedDateForCalculation.getTime() - startOfYearForCalculation.getTime()) / (1000 * 60 * 60 * 24)) + 1;
...
function updateDiagram() {
    if (!diagram)
        return;
    const solarPanelNode = diagram.getObject('solarPanelFrame');
    if (solarPanelNode) {
...
function AngleDiagram() {
    ...
    return (<div className="control-pane diagram-control-pane" style={{ opacity: diagramCreated ? 1 : 0 }}>
      <style>{sampleCss}</style>
      <div className="control-section angle-diagram-container" style={{ width: '100%' }}>
        <DiagramComponent id="diagram" ref={(diagramref) => (diagram = diagramref)} width={'100%'} height={'750px'} snapSettings={{ constraints: SnapConstraints.None }} selectedItems={{ constraints: SelectorConstraints.Rotate }} nodes={nodes} connectors={connectors} scrollChange={() => {
            if (locationDropdown?.isPopupOpen)
        ...
        }} rotateChange={onRotationChange} created={() => {
