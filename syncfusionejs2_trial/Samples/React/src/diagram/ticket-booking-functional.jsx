...
import { DiagramComponent, DiagramTools, SnapConstraints, NodeConstraints, AnnotationConstraints, DiagramConstraints, } from "@syncfusion/ej2-react-diagrams";
...
const handleProceed = () => {
    ...
    let totalAmount = (seatSelection.amount !== null ? seatSelection.amount + 6 : 6).toFixed(2);
...
function TicketBookingDiagram() {
    ...
    return (<div className="control-pane diagram-seat-booking">
      ...
      <div className="control-section">
        ...
        <div className="show-timing-section" style={{ margin: "20px 0" }}>
          <ul className="timing-list" style={{ display: "flex", gap: 16, listStyle: "none", justifyContent: "center" }}>
            ...
            {showTimings.map((timing) => (<li key={timing.id} onClick={() => onTimingClick(timing)} className={`timing-item ${timing.status} ${selectedTimingId === timing.id ? "selected" : ""}`} data-timing={timing.id} style={{ pointerEvents: timing.status === "sold-out" ? "none" : "auto", cursor: "pointer" }}>
        ...
        <div style={{ width: "100%", minHeight: 400, margin: "10px auto" }}>
          <DiagramComponent id="diagram" width={"100%"} height={"800px"} nodes={initializeLayout()} constraints={DiagramConstraints.Default & ~DiagramConstraints.UndoRedo} snapSettings={{ constraints: SnapConstraints.None }} tool={DiagramTools.ZoomPan | DiagramTools.SingleSelect} click={seatClicked} ref={(diagram) => (diagramInstance = diagram)} created={() => {
            if (diagramInstance) {
                diagramCreated = true;
        ...
        <div className="booking-summary-container">
          ...
          <div className="booking-section">
            ...
            <button className="btn-proceed" disabled={seatSelection.ticketCount === 0} id="proceedButton">
              Proceed
            </button>
          </div>
        </div>

        <DialogComponent id="seatBookingDialog" ref={(dialog) => (seatDialogRef = dialog)} visible={false} width="400px" animationSettings={{ effect: "Zoom" }} isModal={true} target={'.control-section'} buttons={[
            {
      ...
      <div id="action-description">
        <p>
          This sample implements a functional movie ticket booking interface using the <a href="https://www.syncfusion.com/react-components/react-diagram" target="_blank">React Diagram</a>, providing an intuitive visual representation of a cinema hall's seating arrangement with real-time price updates and a complete booking workflow.
        </p>
      </div>
      <div id="description">
        <p>
