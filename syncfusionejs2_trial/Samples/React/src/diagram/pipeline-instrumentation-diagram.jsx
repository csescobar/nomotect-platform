...
import { DiagramComponent, Inject, UndoRedo, SnapConstraints, ConnectorBridging, NodeConstraints, DiagramTools, Diagram, PortVisibility, FlipDirection } from '@syncfusion/ej2-react-diagrams';
...
let gauge;
let checkbox;
let checkValve1CloseBtn;
let checkValve2CloseBtn;
let checkValve3CloseBtn;
...
let inputTankOn = false;
const animationIntervals = {};
const gradualStopIntervals = {};
...
function getPorts(nodeId) {
    ...
    if (nodeId !== "controlvalveBox1") {
        ...
        ports = stacticPorts;
        if (nodeId === "tank1") {
            ports.push({ id: 'tankPort1', offset: { x: 1, y: 0.2 }, visibility: PortVisibility.Visible }, { id: 'tankPort2', offset: { x: 1, y: 0.8 }, visibility: PortVisibility.Visible });
        }
            ...
            ports.push({ id: 'pumpPort1', offset: { x: 0.94, y: 0.1 }, visibility: PortVisibility.Hidden }, { id: 'pumpPort2', offset: { x: 0.97, y: 0.95 }, visibility: PortVisibility.Hidden });
        }
    }
    return ports;
}
...
function animatePathFlow(pathId, animate, color, slow) {
    ...
    if (animate) {
        ...
        if (slow) {
            ...
            const endColor = hexToRgb(currentColor);
...
export class PipelineInstrumentationDiagram extends SampleBase {
    pipelineModel = initPipelineDiagram();
    ...
    render() {
        return (<div className="control-pane">
                <div className="control-section diagram-pipeline" style={{ width: "100%", opacity: 0 }}>
                    ...
                    <DiagramComponent id="diagram" ref={(diagram) => (diagramInstance = diagram)} width={'100%'} height={'700px'} nodes={this.pipelineModel.nodes} connectors={this.pipelineModel.connectors} tool={DiagramTools.ZoomPan} snapSettings={{ constraints: SnapConstraints.None }} getNodeDefaults={this.getNodeDefaults} getConnectorDefaults={this.getConnectorDefaults} created={() => {
                diagramCreated = true;
                appendHTMLElements();
                diagramInstance.fitToPage({ canZoomOut: true });
                // show diagram
                setTimeout(() => {
                    const container = document.querySelector('.diagram-pipeline');
                    if (container) {
