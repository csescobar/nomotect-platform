...
function SerpentineDiagram() {
    ...
    const createDecorator = (color, pivotX) => ({
        shape: 'Custom',
        width: 20,
        height: 30,
        pivot: { x: pivotX },
    ...
    return (<div className="control-pane serpentine-diagram-container">
            ...
            return (<ButtonComponent key={id} id={id} isPrimary={currentZoom === level} onClick={() => handleZoom(level)}>
                                    {level}x
                                </ButtonComponent>);
        })}
                    </div>
                    <div>
                        <DiagramComponent id="serpentineDiagram" ref={diagramInstance} width="100%" height="600px" className="diagram-container" snapSettings={{ constraints: SnapConstraints.None }} scrollSettings={{ scrollableArea: SCROLL_AREA, padding: SCROLL_PADDING }} tool={DiagramTools.ZoomPan} nodes={[]} connectors={[]}/>
                    </div>
                </div>
            </div>
