...
import { DiagramComponent, Diagram, UndoRedo, BpmnDiagrams, DiagramTools, NodeConstraints, SymbolPalette, SymbolPaletteComponent, } from '@syncfusion/ej2-react-diagrams';
...
let flowTimeOut1;
let flowTimeOut2;
let switchTooltipRef;
let switchRef;
let switchContainerRef;
...
const paletteTemplate = () => (<aside id="symbolPalette" style={{
        display: isEditMode ? "flex" : "none",
        height: "100%",
        alignItems: "center",
        border: "none",
    }}>
    <SymbolPaletteComponent id="symbolPaletteComponent" ref={(symbolPaletteComponent) => (symbolPaletteRef = symbolPaletteComponent)} width="100%" height="62px" symbolHeight={45} symbolWidth={45} enableAnimation={false} 
// showHeader={false}
palettes={[
        {
            id: "BPMN",
    ...
    ]} getSymbolInfo={() => ({ fit: true })} paletteExpanding={(args) => { args.cancel = true; }}/>
  </aside>);
// React component for the BPMN workflow editor
export class WorkFlowEditor extends SampleBase {
    render() {
        return (<div className="control-pane diagram-workflow">
        <style>{sample_css}</style>
        <div className="col-lg-12 control-section">
          <div style={{ width: "100%" }}>
            <div id='toolbarContainer'>
              <div id='toolbar'>
                <ToolbarComponent ref={(toolbar) => (toolbarEditor = toolbar)} id="toolbar_diagram" clicked={tooledit} overflowMode="Popup">
                  <ItemsDirective>
              ...
              <div id='toggleSwitchContainer'>
                <div id="switch-container" ref={(switchContainer) => (switchContainerRef = switchContainer)}>
                  <span id="editLabel" style={{ fontSize: '14px', marginRight: '6px' }}>Edit</span>
                  <TooltipComponent id="switch_tooltip_diagram" ref={(switchTooltip) => (switchTooltipRef = switchTooltip)} content="Enable Editing" position="TopCenter">
                    <SwitchComponent id="DiagramToggleSwitch" ref={(DiagramToggleSwitch) => (switchRef = DiagramToggleSwitch)} checked={isEditMode} change={(args) => switchChange(args.checked)}/>
                  </TooltipComponent>
                </div>
              </div>
            </div>
            <div>
              <DiagramComponent id="diagram" ref={(diagram) => (diagramInstance = diagram)} width={"100%"} height={"645px"} nodes={nodes} connectors={connectors} tool={DiagramTools.ZoomPan} scrollSettings={{ scrollLimit: "Diagram", canAutoScroll: true }} selectedItems={{ userHandles: userHandles }} getNodeDefaults={(node, diagram) => {
