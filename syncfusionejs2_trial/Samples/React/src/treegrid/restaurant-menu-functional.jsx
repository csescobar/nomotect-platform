...
const RestaurantMenu = () => {
    ...
    const foodCountChangeFn = (args, rowData) => {
        ...
        const updatedCart = [...foodOrderDetailsRef.current];
    ...
    const getFoodCount = (foodName) => {
        const item = foodOrderDetailsRef.current.find(order => order.foodName === foodName);
        ...
        <div className="menu-right">
          <div className="resmenu-image-wrap">
            ...
            <div className="resmenu-count">
              <NumericTextBoxComponent min={0} max={10} step={1} format="N0" width="110px" value={getFoodCount(props.FoodName)} change={(args) => {
    ...
    const onCartClick = () => {
        const cartItems = foodOrderDetailsRef.current.filter(item => item.count > 0);
        if (dialogRef.current) {
    ...
    const beforeOpen = (args) => {
        const itemsInCart = foodOrderDetailsRef.current.filter(item => item.count > 0);
      ...
      <div className="control-section">
        <div className="treegrid-center-container">
          <div className="resmenu-toolbar-search">
            <AutoCompleteComponent dataSource={Array.from(new Set(foodMenu.map(i => i.FoodName)))} placeholder="Search for dishes" width='800px' highlight={true} filterType='Contains' change={onSearchChange} id='search-autocomplete'/>
          </div>
          ...
          <div id="res-menu">
            ...
            <TreeGridComponent id='RestaurantTreeGrid' ref={treegridRef} allowKeyboard={false} dataSource={treeData} dataBound={databound} idMapping='FoodId' parentIdMapping='CategoryId' treeColumnIndex={0} rowTemplate={rowTemplate} toolbar={toolbarOptions} toolbarClick={toolbarClick} height={400} width={800}>
              <ColumnsDirective>
