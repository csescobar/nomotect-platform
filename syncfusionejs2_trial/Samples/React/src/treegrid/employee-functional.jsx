...
const EmployeeTreeGrid = () => {
    useEffect(() => {
        updateSampleSection();
    }, []);
    const treeGridRef = useRef(null);
    const [viewer, setViewer] = useState('hr');
    const getBarHTML = (value) => {
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                <div className="viewer-container" style={{ margin: '10px' }}>
                    ...
                    <DropDownListComponent id="viewer" dataSource={viewerRoles} fields={{ text: 'role', value: 'id' }} value={viewer} change={handleViewerChange} placeholder="Select Viewer Role"/>
                </div>

                <TreeGridComponent key={viewer} id="sample" ref={treeGridRef} dataSource={employeeData} childMapping="Children" treeColumnIndex={0} height={400} allowSorting={true} allowFiltering={true} filterSettings={{
            type: 'Menu',
            hierarchyMode: 'None',
            mode: 'Immediate',
        }} pageSettings={{ pageSize: 10 }} enableHover={true} gridLines="Both">
                    <ColumnsDirective>
                        {getColumns(viewer).map((col, idx) => (<ColumnDirective key={idx} {...col}/>))}
                    </ColumnsDirective>
