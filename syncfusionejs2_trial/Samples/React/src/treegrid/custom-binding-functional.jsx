...
function CustomBinding() {
    let treegrid;
    let data;
    const BASE_URL = 'https://services.syncfusion.com/react/production/api/SupportTicketData';
    const pageSettings = { pageSize: 10, pageCount: 4 };
    const daterules = { date: true, required: true };
    const dateeditparam = { params: { format: 'M/d/yyyy' } };
    React.useEffect(() => {
        updateSampleSection();
        rendereComplete();
    }, []);
    ...
    function dataStateChange(state) {
        if (state.requestType === 'expand') {
            execute(state).then((treegridData) => {
                state.childData = treegridData.result;
                state.childDataBind();
            });
        }
        else {
            execute(state).then((treegridData) => { treegrid.dataSource = treegridData; });
        }
    }
    const ajax = new Ajax({
        type: 'GET', mode: true,
    ...
    function buildFilterQuery(where) {
        ...
        for (const cond of where) {
            if (cond.predicates?.length) {
                const groupFilters = cond.predicates.map((pred) => predicateToString(pred));
    ...
    return (<div className="control-pane">
            <div className="control-section">
                ...
                <TreeGridComponent dataSource={data} ref={(g) => treegrid = g} height="350" allowPaging={true} allowSorting={true} allowFiltering={true} pageSettings={pageSettings} treeColumnIndex={1} idMapping="TicketID" parentIdMapping="ParentTicketID" hasChildMapping="isParent" dataStateChange={dataStateChange.bind((this))}>
                     <ColumnsDirective>
