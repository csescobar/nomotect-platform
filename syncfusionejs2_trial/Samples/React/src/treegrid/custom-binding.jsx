import { TreeGridComponent, ColumnsDirective, ColumnDirective, Page, Filter, Inject, Sort, } from '@syncfusion/ej2-react-treegrid';
...
export class CustomBinding extends SampleBase {
    ...
    daterules = { date: true, required: true };
    dateeditparam = { params: { format: 'M/d/yyyy' } };
    rendereComplete() {
    ...
    dataStateChange = (state) => {
        if (state.requestType === 'expand') {
            this.taskService.execute(state).then((treegridData) => {
    ...
    render() {
        return (<div className='control-pane'>
                <div className='control-section'>
                    ...
                    <TreeGridComponent dataSource={this.data} ref={(g) => this.treegrid = g} height={350} allowPaging={true} allowSorting={true} allowFiltering={true} pageSettings={this.pageSettings} treeColumnIndex={1} idMapping="TicketID" parentIdMapping="ParentTicketID" hasChildMapping="isParent" dataStateChange={this.dataStateChange.bind((this))}>
                        <ColumnsDirective>
...
export class TaskService {
    ...
    BASE_URL = 'https://services.syncfusion.com/react/production/api/SupportTicketData';
    ...
    getData(state) {
        ...
        if (state.sorted && state.sorted.length > 0) {
            sortQuery = `&$orderby=${state.sorted
    ...
    buildSearchQuery = (search) => {
        if (!search || !search.length)
            return "";
        const s = search[0];
        const searchStr = s.key.toLowerCase();
        const orConds = (s.fields || []).map((field) => `substringof('${searchStr}',tolower(cast(${field}, 'Edm.String')))`);
