...
class RestaurantMenu extends SampleBase {
    ...
    foodCountChangeFn = (args) => {
        ...
        const updatedCart = [...this.state.foodOrderDetails];
    ...
    getFoodCount = (foodName) => {
        const item = this.state.foodOrderDetails.find(order => order.foodName === foodName);
        return item ? item.count : 0;
    };
    onSearchChange = (args) => {
        const value = args.value ? args.value.toLowerCase() : '';
        let searchedData = foodMenu.filter(item => item.FoodName.toLowerCase().includes(value) ||
                    ...
                    <div className="menu-right">
                        <div className="resmenu-image-wrap">
                            ...
                            <div className="resmenu-count">
                                <NumericTextBoxComponent min={0} max={10} step={1} format='N0' width='110px' value={this.getFoodCount(props.FoodName)} change={(args) => this.foodCountChangeFn(args)}/>
                            </div>
                        </div>
                    </div>
                </td>
    ...
    onCartClick = () => {
        const cartItems = this.state.foodOrderDetails.filter(item => item.count > 0);
        if (this.dialogRef.current) {
    ...
    ;
    databound = () => {
        if (this.treegridRef.initialRender) {
            this.treegridRef.grid.emptyRecordTemplate = this.emptyTemplate;
        }
    };
    beforeOpen = (args) => {
        const items = this.state.foodOrderDetails.filter((item) => item.count > 0);
        if (items.length < 4) {
    ...
    render() {
        return (<div className="control-pane">+
                <div className="control-section">
                    <div className="treegrid-center-container">
                        <div className="resmenu-toolbar-search">
                            <AutoCompleteComponent dataSource={Array.from(new Set(foodMenu.map(i => i.FoodName)))} placeholder="Search for dishes" width='800px' highlight={true} filterType='Contains' change={this.onSearchChange} id='search-autocomplete'/>
                        </div>
                        <div id="res-menu">
                            <DialogComponent id="cartDialog" ref={this.dialogRef} header='Bill Summary' width='400px' showCloseIcon={true} isModal={true} visible={false} animationSettings={{ effect: 'Zoom' }} beforeOpen={this.beforeOpen} buttons={[
            ...
            ]} target='#RestaurantTreeGrid'></DialogComponent>
                            <TreeGridComponent id='RestaurantTreeGrid' ref={this.treegridRef} dataSource={this.state.treeData} dataBound={this.databound} allowKeyboard={false} idMapping='FoodId' parentIdMapping='CategoryId' treeColumnIndex={0} rowTemplate={this.rowTemplate} toolbar={this.toolbarOptions} toolbarClick={this.toolbarClick} height={400}>
                                <ColumnsDirective>
