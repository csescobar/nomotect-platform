...
import { ScheduleComponent, TimelineMonth, Resize, DragAndDrop, ResourcesDirective, ResourceDirective, Inject, } from '@syncfusion/ej2-react-schedule';
import { ChartComponent, SeriesCollectionDirective, SeriesDirective, ColumnSeries, Category, Legend, Tooltip, Highlight } from '@syncfusion/ej2-react-charts';
...
const SchedulerWithChart = () => {
    ...
    const [currentMode, setCurrentMode] = useState('tripcount');
    const generateChartData = useCallback((mode) => {
        if (mode === 'capacity') {
    ...
    const updateChart = useCallback((mode) => {
    ...
    const resourceTemplate = useCallback((props) => {
    ...
    const resourceTooltipTemplate = useCallback((props) => {
        const d = props.resourceData;
        return (<div className="template-wrap">
    ...
    const headerIndentTemplate = useCallback(() => (<div className="template-wrap header-indent">
    ...
    return (<div className='schedule-control-section'>
                <div className="col-lg-12 schedule-with-chart control-section">
                    <div className="schedule-content-wrapper">
                        <div className="modern-layout" style={{ display: 'flex', gap: '10px' }}>
                            {/* Scheduler */}
                            <div id="scheduler" className="scheduler-wrapper">
                                <ScheduleComponent ref={scheduleRef} height="650px" width="100%" selectedDate={new Date(2026, 0, 12)} views={['TimelineMonth']} allowOverlap={false} resourceHeaderTemplate={resourceTemplate} headerIndentTemplate={headerIndentTemplate} group={{ resources: ['TruckDetails'], headerTooltipTemplate: resourceTooltipTemplate }} eventSettings={{ dataSource: events }} actionComplete={(args) => {
            if (args.requestType === 'eventCreated' ||
                ...
                updateChart(currentMode);
                                    ...
                                    <ResourcesDirective>
                                        <ResourceDirective name="TruckDetails" field="DriverID" dataSource={drivers} textField="driver" idField="id" colorField="color"/>
                                    </ResourcesDirective>
                            ...
                            <div id="chart-container" className="chart-section">
                                <div className="chart-header">
                                    ...
                                    <div id="chart-ddl" className="dropdown-wrapper">
                                        <DropDownListComponent width="100%" value={currentMode} placeholder="Select Metric" fields={{ text: 'text', value: 'value' }} dataSource={[
            ...
            setCurrentMode(e.value);
                                ...
                                <div id="chart" className="chart-area">
                                    <ChartComponent ref={chartRef} primaryXAxis={{
            valueType: 'Category',
            labelRotation: Browser.isDevice ? -45 : 315
        }} legendSettings={{ visible: false }} load={load.bind(this)} tooltip={{ enable: true, enableHighlight: true }} pointRender={(args) => {
            let driver;
            if (currentMode === 'capacity') {
                driver = drivers.find((d) => d.truck === args.point?.x);
            }
            else {
                driver = drivers.find((d) => d.driver === args.point?.x);
            }
            if (driver) {
            ...
            Highlight
                                        ...
                                        <SeriesCollectionDirective>
                                            <SeriesDirective type="Column" xName="Driver" yName="Value"/>
                                        </SeriesCollectionDirective>
                                    </ChartComponent>
                                </div>
                            </div>
