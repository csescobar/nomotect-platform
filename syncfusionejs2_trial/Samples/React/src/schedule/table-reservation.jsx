...
import { ScheduleComponent, TimelineViews, Agenda, Inject, ResourceDirective, ResourcesDirective, ViewsDirective, ViewDirective, Week, Day, ToolbarItemsDirective, ToolbarItemDirective } from '@syncfusion/ej2-react-schedule';
...
export class TableReservation extends SampleBase {
    ...
    slotTabRef = React.createRef();
    allWaitingDataTreeViewRef = React.createRef();
    breakfastWaitingDataTreeViewRef = React.createRef();
    lunchWaitingDataTreeViewRef = React.createRef();
    dinnerWaitingDataTreeViewRef = React.createRef();
    alertDialogRef = React.createRef();
    ...
    draggedItemId;
    startDateTimeValue;
    selectedDate = new Date(2025, 2, 16);
    animationSettings = { effect: 'None' };
    eventsData = extend([], dataSource.TableReservationData, null, true);
    breakfastWaitingData = extend([], dataSource.BreakfastWaitingData, null, true);
    lunchWaitingData = extend([], dataSource.LunchWaitingData, null, true);
    dinnerWaitingData = extend([], dataSource.DinnerWaitingData, null, true);
    allWaitingData = this.breakfastWaitingData.concat(this.lunchWaitingData, this.dinnerWaitingData);
    tableCategory = [
    ...
    allWaitingDataTreeFields = { dataSource: this.allWaitingData, id: 'Id', text: 'CustomerName' };
    breakfastWaitingDataTreeFields = { dataSource: this.breakfastWaitingData, id: 'Id', text: 'CustomerName' };
    lunchWaitingDataTreeFields = { dataSource: this.lunchWaitingData, id: 'Id', text: 'CustomerName' };
    dinnerWaitingDataTreeFields = { dataSource: this.dinnerWaitingData, id: 'Id', text: 'CustomerName' };
    scheduleFields = {
        id: 'Id',
    ...
    slotData = ['Breakfast', 'Lunch', 'Dinner'];
    alertDialogButtons = [
        {
            click: () => {
    ...
    formatTimeRange = (startHour, endHour, date) => {
    ...
    getMealPeriodTimeRange = (date) => {
        const hours = date.getHours();
        if (hours >= 7 && hours < 12)
            return this.formatTimeRange(7, 12, date);
        if (hours >= 12 && hours < 17)
            return this.formatTimeRange(12, 17, date);
        if (hours >= 17 && hours <= 22)
            return this.formatTimeRange(17, 22, date);
        return '';
    };
    majorSlotTemplate = (props) => {
        const hours = props.date.getHours();
        const minutes = props.date.getMinutes();
        const mealPeriod = this.getMealPeriod(props.date);
        const showTimeRange = (hours === 7 && minutes === 0) || (hours === 12 && minutes === 0) || (hours === 17 && minutes === 0);
        const timeRange = showTimeRange ? this.getMealPeriodTimeRange(props.date) : '';
        return (<div className="custom-slot-template">
    ...
    resourceHeaderTemplate = (props) => {
        if (props.resource.name === 'Category') {
            return (<div className="e-resource-text">{props.resourceData.category}</div>);
        }
        else {
    ...
    toggleWaitingListElement = () => {
        let settingsPanel = document.querySelector('.waiting-list-container');
        let toggleButton = this.scheduleRef.current?.element.querySelector('.e-show-waiting-list') || this.scheduleRef.current?.element.querySelector('.e-hide-waiting-list');
    ...
    agendaTemplate = (props) => {
        ...
        <div className="agenda-item">
          {props.Status === 'Cancelled' && (<div className="status-badge">Cancelled</div>)}
          ...
          {props.Status === 'Blocked' || props.Status === 'Not Available' ? (<div className="time-size-details">
              ...
              {props.Notes && (<div className="notes-details">
    ...
    getBackgroundColorByTime = (startHour) => {
        let backgroundColor = '';
    ...
    getQuickInfoHeaderStyle = (data) => {
        ...
        let backgroundColor = this.getBackgroundColorByTime(startHour);
    ...
    quickInfoContent = (props) => {
        const tableName = this.tables.filter((table) => table.id === props.TableId)[0].name;
        const tableType = this.tableCategory.filter((table) => table.id === props.CategoryId)[0].category;
        ...
        <div className="event-content">
          ...
          {props.Notes && props.Notes.length > 0 && (<div className="e-notes e-content-item e-notes-row">
    ...
    onEventRendered = (args) => {
        ...
        if (eventData) {
            let status = eventData.Status.toLowerCase();
    ...
    onActionBegin = (args) => {
        if (args.requestType === 'eventCreate') {
            let formData = args.addedRecords[0];
    ...
    editorHeaderTemplate = (props) => {
        return (<div id="event-editor-header">New Reservation</div>);
    };
    stratDateTimeChange = (args) => {
        const previousStartDateTime = this.startDateTimeValue;
        const startDateTime = new Date(this.startDateTimePickerRef.current.value);
        let difference = 0;
        ...
        const endDateTime = new Date(this.endDateTimePickerRef.current.value);
        const newEndDateTime = new Date(endDateTime.getTime() + difference);
    ...
    editorTemplate = (props) => {
        ...
        return (<div className="custom-event-editor">
        <div className="form-row ready-only-item">
          <div className="form-group first-item">
            ...
            <div className="e-field-wrapper">
              <input id="Subject" className="e-field e-input" type="text" name="Subject" data-name="Subject" defaultValue={props.Subject || ''}/>
            </div>
          </div>
          <div className="form-group half-width">
            <label className="e-textlabel">Table category</label>
            <div className="e-field-wrapper">
              <DropDownListComponent id="CategoryId" dataSource={this.tableCategory} fields={{ text: 'category', value: 'id' }} value={props.CategoryId} className="e-field" data-name="CategoryId"/>
            </div>
          </div>
        </div>
        <div className="form-row ready-only-item">
          <div className="form-group first-item">
            ...
            <div className="e-field-wrapper">
              <NumericTextBoxComponent id="GuestCount" value={props.GuestCount || 4} min={1} max={20} format="n0" showSpinButton={true} className="e-field" data-name="GuestCount"/>
            </div>
          </div>
          <div className="form-group half-width">
            <label className="e-textlabel">Table</label>
            <div className="e-field-wrapper">
              <DropDownListComponent id="TableId" dataSource={this.tables} fields={{ text: 'name', value: 'id' }} value={props.TableId} className="e-field" data-name="TableId"/>
            </div>
          </div>
        </div>
        <div className="form-row e-start-end-time">
          <div className="form-group first-item">
            ...
            <div className="e-field-wrapper">
              <DateTimePickerComponent ref={this.startDateTimePickerRef} id="StartTime" format="M/dd/yy h:mm a" data-name="StartTime" value={this.startDateTimeValue} className="e-field" change={this.stratDateTimeChange}/>
            </div>
          </div>
          <div className="form-group half-width ready-only-item">
            <label className="e-textlabel">End</label>
            <div className="e-field-wrapper">
              <DateTimePickerComponent ref={this.endDateTimePickerRef} id="EndTime" format="M/dd/yy h:mm a" data-name="EndTime" value={new Date(props.EndTime || props.endTime)} className="e-field"/>
            </div>
          </div>
        </div>
        <div className="form-row ready-only-item">
          <div className="form-group first-item">
            ...
            <div className="e-field-wrapper">
              <DropDownListComponent id="Duration" dataSource={['1 hour', '1.5 hours', '2 hours', '2.5 hours', '3 hours']} value={props.Duration || '2 hours'} className="e-field" data-name="Duration"/>
            </div>
          </div>
          <div className="form-group half-width">
            <label className="e-textlabel">Meal slot</label>
            <div className="e-field-wrapper">
              <DropDownListComponent id="slot" dataSource={['Breakfast', 'Lunch', 'Dinner']} value={props.slot || 'Breakfast'} className="e-field" data-name="slot"/>
            </div>
          </div>
        </div>
        <div className="form-row ready-only-item">
          <div className="form-group e-contact-number">
            ...
            <div className="e-field-wrapper">
              <TextBoxComponent id="contactNumber" placeholder="Enter contact number" value={props.contactNumber || ''} className="e-field" data-name="contactNumber"/>
            </div>
          </div>
        </div>
        <div className="form-row ready-only-item">
          <div className="form-group e-note">
            ...
            <div className="e-field-wrapper">
              <textarea id="Notes" className="e-field e-input" name="Notes" data-name="Notes" rows={3} defaultValue={props.Notes || ''} style={{ width: '100%', height: '80px', resize: 'vertical' }}></textarea>
            </div>
          </div>
        </div>
      </div>);
    ...
    handleEmptyDataSourceDisplay = (treeViewRef, dataSource) => {
        const noEventsElement = document.querySelector('.no-waiting-list-message');
        if (noEventsElement) {
    ...
    onPopupClose = (args) => {
        if (args.type === 'Editor') {
            ...
            if (isSaveAction) {
                ...
                const endtHour = endTime.getHours();
                const slot = args.data.slot;
                const isCorrectTimeSlot = startTime.getDate() === endTime.getDate() && ((slot === 'Breakfast' && startHour >= 7 && endtHour < 12) || (slot === 'Lunch' && startHour >= 12 && endtHour < 17) || (slot === 'Dinner' && startHour >= 17 && endtHour <= 22));
                if (!isCorrectTimeSlot) {
                    let timeElement = args.element.querySelector('.e-start-end-time');
                ...
                let waitingListTreeViewRefs = [this.allWaitingDataTreeViewRef, this.breakfastWaitingDataTreeViewRef, this.lunchWaitingDataTreeViewRef, this.dinnerWaitingDataTreeViewRef];
                let currentTreeViewRef = waitingListTreeViewRefs[this.slotTabRef.current.selectedItem].current;
                let currentTreeViewData = currentTreeViewRef.fields.dataSource;
                const updatedData = currentTreeViewData.filter((item) => item.Id !== this.draggedItemId);
                currentTreeViewRef.fields.dataSource = updatedData;
                this.allWaitingData = this.allWaitingData.filter(item => item.Id !== this.draggedItemId);
    ...
    eventTemplate = (props) => {
        ...
        return (<div className="template-wrap reservation-card">
        {props.Status === 'Cancelled' && (<div className="status-badge">Cancelled</div>)}
    ...
    treeTemplate = (props) => {
        return (<div className="template-wrap waiting-list-card">
    ...
    unplannedEventsUpdatedData = (dataSource, selectedTabIndex) => {
        return dataSource.filter((data) => data.Slot === this.slotData[selectedTabIndex - 1]);
    };
    onTabSelecting = (args) => {
        let waitingListTreeViewRefs = [this.allWaitingDataTreeViewRef, this.breakfastWaitingDataTreeViewRef, this.lunchWaitingDataTreeViewRef, this.dinnerWaitingDataTreeViewRef];
        const previousTabIndex = args.selectedIndex;
        const selectedTabIndex = args.selectingIndex;
        waitingListTreeViewRefs[selectedTabIndex].current.fields.dataSource =
            selectedTabIndex === 0 ? this.breakfastWaitingDataTreeViewRef.current.fields.dataSource.concat(this.lunchWaitingDataTreeViewRef.current.fields.dataSource, this.dinnerWaitingDataTreeViewRef.current.fields.dataSource) : this.unplannedEventsUpdatedData(this.allWaitingData, selectedTabIndex);
        ...
        this.handleEmptyDataSourceDisplay(waitingListTreeViewRefs[selectedTabIndex].current, waitingListTreeViewRefs[selectedTabIndex].current.fields.dataSource);
    };
    ...
    onTreeDragging = (event) => {
        ...
        const cancelledAppointment = closest(targetElement, '.e-appointment.e-cancelled');
        const allElements = document.querySelectorAll('.not-allowed-cursor');
        ...
        if ((targetElement.classList.contains('e-work-cells') && !targetElement.classList.contains('e-resource-group-cells')) ||
            ...
            if (cancelledAppointment) {
                const allChildren = cancelledAppointment.querySelectorAll('*');
    ...
    onTreeDragStop = (event) => {
        ...
        let classElement = this.scheduleRef.current?.element.querySelector('.e-device-hover');
        if (classElement) {
        ...
        if (!treeElement) {
            ...
            if (scheduleElement) {
                ...
                const filteredData = this.allWaitingData.filter((item) => item.Id === parseInt(event.draggedNodeData.id, 10));
                const targetIsCell = targetElement.classList.contains('e-work-cells') && !targetElement.classList.contains('e-resource-group-cells');
                const targetIsCancelledEvent = !isNullOrUndefined(closest(event.target, '.e-appointment.e-cancelled'));
                let eventData;
                let tableId;
                let StartTime;
                let groupIndex;
                let EndTime;
                if (targetIsCell) {
                    let cellData = this.scheduleRef.current?.getCellDetails(event.target);
                ...
                const tableData = this.tables.filter((table) => table.id === tableId)[0];
                const tableType = this.tableCategory.filter((tableGroup) => tableGroup.id === tableData.groupId)[0].category.split(' ')[0];
                if (tableType !== Table) {
                ...
                const isCorrectTimeSlot = (Slot === 'Breakfast' && hour >= 7 && hour < 12) || (Slot === 'Lunch' && hour >= 12 && hour < 17) || (Slot === 'Dinner' && hour >= 17 && hour <= 22);
                if (!isCorrectTimeSlot) {
                ...
                const isSlotAvailable = this.scheduleRef.current?.isSlotAvailable(StartTime, EndTime, groupIndex) && startDate === endDate && endHour <= 22;
                ...
                let updatedEventData = targetIsCancelledEvent ? eventData : {};
                ...
                this.draggedItemId = parseInt(event.draggedNodeData.id, 10);
            }
        }
        document.body.classList.remove('e-disble-not-allowed');
    };
    render() {
        return (<div className='control-section table-reservation-control-section'>
          <div className='control-wrapper table-reservation-wrapper'>
            <ScheduleComponent ref={this.scheduleRef} cssClass='schedule-table-reservation' width='100%' height='550px' selectedDate={this.selectedDate} group={{ enableCompactView: false, resources: ['Category', 'Tables'] }} resourceHeaderTemplate={this.resourceHeaderTemplate} startHour="07:00" endHour='22:00' timeScale={this.timeScale} allowOverlap={false} eventSettings={{
              ...
              <ResourcesDirective>
                <ResourceDirective field="CategoryId" title="Category" name="Category" cssClassField='table-category' allowMultiple={false} dataSource={this.tableCategory} textField="category" idField="id"/>
                <ResourceDirective field="TableId" title="Tables" name="Tables" cssClassField='table-name' allowMultiple={true} dataSource={this.tables} textField="name" idField="id" groupIDField='groupId'/>
              </ResourcesDirective>
              <ToolbarItemsDirective>
                ...
                <ToolbarItemDirective name='Custom' type='Button' prefixIcon='e-icons e-show-waiting-list' align='Right' showTextOn='Overflow' overflow='Show' id="overview_toolbar_settings_waiting_list" click={this.toggleWaitingListElement} cssClass='toolbar-post-agenda'/>
              </ToolbarItemsDirective>
            ...
            <div className="waiting-list-container">
              ...
              <div id="list-container">
                ...
                <TreeViewComponent ref={this.breakfastWaitingDataTreeViewRef} id='breakfastWaitingDataTreeView' style={{ display: "none" }} cssClass='table-reservation-treeview' dragArea=".table-reservation-wrapper .e-content-wrap" nodeTemplate={this.treeTemplate} fields={this.breakfastWaitingDataTreeFields} nodeDragStart={this.onTreeDragStart} nodeDragging={this.onTreeDragging} nodeDragStop={this.onTreeDragStop} allowDragAndDrop={true}/>
                ...
                <TreeViewComponent ref={this.dinnerWaitingDataTreeViewRef} id='dinnerWaitingDataTreeView' style={{ display: "none" }} cssClass='table-reservation-treeview' dragArea=".table-reservation-wrapper .e-content-wrap" nodeTemplate={this.treeTemplate} fields={this.dinnerWaitingDataTreeFields} nodeDragStart={this.onTreeDragStart} nodeDragging={this.onTreeDragging} nodeDragStop={this.onTreeDragStop} allowDragAndDrop={true}/>
                <div className="no-waiting-list-message hidden">No customers are waiting</div>
              </div>
            </div>
            <div id="target">
              <DialogComponent id="modalDialog" cssClass='alert-dialog' isModal={true} buttons={this.alertDialogButtons} header="Notice" height="240px" width="335px" ref={this.alertDialogRef} visible={false} showCloseIcon={true} animationSettings={this.animationSettings} target="#target"></DialogComponent>
            </div>
          </div>
