...
const TableReservation = () => {
    ...
    const slotTabRef = useRef(null);
    const allWaitingDataTreeViewRef = useRef(null);
    const breakfastWaitingDataTreeViewRef = useRef(null);
    const lunchWaitingDataTreeViewRef = useRef(null);
    const dinnerWaitingDataTreeViewRef = useRef(null);
    ...
    let draggedItemId;
    const selectedDate = new Date(2025, 2, 16);
    const animationSettings = { effect: 'None' };
    let startDateTimeValue;
    const eventsData = extend([], dataSource.TableReservationData, null, true);
    const breakfastWaitingData = extend([], dataSource.BreakfastWaitingData, null, true);
    const lunchWaitingData = extend([], dataSource.LunchWaitingData, null, true);
    const dinnerWaitingData = extend([], dataSource.DinnerWaitingData, null, true);
    const allWaitingData = useRef(breakfastWaitingData.concat(lunchWaitingData, dinnerWaitingData));
    const tableCategory = [
    ...
    const allWaitingDataTreeFields = { dataSource: allWaitingData.current, id: 'Id', text: 'CustomerName' };
    const breakfastWaitingDataTreeFields = { dataSource: breakfastWaitingData, id: 'Id', text: 'CustomerName' };
    const lunchWaitingDataTreeFields = { dataSource: lunchWaitingData, id: 'Id', text: 'CustomerName' };
    ...
    const slotData = ['Breakfast', 'Lunch', 'Dinner'];
    ...
    const formatTimeRange = (startHour, endHour, date) => {
    ...
    const getMealPeriodTimeRange = (date) => {
        const hours = date.getHours();
        if (hours >= 7 && hours < 12)
    ...
    const majorSlotTemplate = (props) => {
        ...
        const mealPeriod = getMealPeriod(props.date);
        const showTimeRange = (hours === 7 && minutes === 0) || (hours === 12 && minutes === 0) || (hours === 17 && minutes === 0);
        const timeRange = showTimeRange ? getMealPeriodTimeRange(props.date) : '';
    ...
    const toggleWaitingListElement = () => {
        let settingsPanel = document.querySelector('.waiting-list-container');
        let toggleButton = scheduleRef.current.element.querySelector('.e-show-waiting-list') || scheduleRef.current.element.querySelector('.e-hide-waiting-list');
    ...
    const getBackgroundColorByTime = (startHour) => {
        let backgroundColor = '';
    ...
    const getQuickInfoHeaderStyle = (data) => {
        ...
        let backgroundColor = getBackgroundColorByTime(startHour);
    ...
    const quickInfoContent = (props) => {
        const tableName = tables.filter((table) => table.id === props.TableId)[0].name;
        const tableType = tableCategory.filter((table) => table.id === props.CategoryId)[0].category;
    ...
    const stratDateTimeChange = (args) => {
        const previousStartDateTime = startDateTimeValue;
        const startDateTime = new Date(startDateTimePickerRef.current.value);
        let difference = 0;
        ...
        const endDateTime = new Date(endDateTimePickerRef.current.value);
    ...
    const editorTemplate = (props) => {
        ...
        <div className="form-row ready-only-item">
          ...
          <div className="form-group half-width">
            ...
            <div className="e-field-wrapper">
              <DropDownListComponent id="CategoryId" dataSource={tableCategory} fields={{ text: 'category', value: 'id' }} value={props.CategoryId} className="e-field" data-name="CategoryId"/>
            </div>
          </div>
        </div>
        <div className="form-row ready-only-item">
          ...
          <div className="form-group half-width">
            ...
            <div className="e-field-wrapper">
              <DropDownListComponent id="TableId" dataSource={tables} fields={{ text: 'name', value: 'id' }} value={props.TableId} className="e-field" data-name="TableId"/>
            </div>
          </div>
        </div>
        <div className="form-row e-start-end-time">
          <div className="form-group first-item">
            ...
            <div className="e-field-wrapper">
              <DateTimePickerComponent ref={startDateTimePickerRef} id="StartTime" format="M/dd/yy h:mm a" data-name="StartTime" value={startDateTimeValue} className="e-field" change={stratDateTimeChange}/>
            </div>
          </div>
          <div className="form-group half-width ready-only-item">
            <label className="e-textlabel">End</label>
            <div className="e-field-wrapper">
              <DateTimePickerComponent ref={endDateTimePickerRef} id="EndTime" format="M/dd/yy h:mm a" data-name="EndTime" value={new Date(props.EndTime || props.endTime)} className="e-field"/>
            </div>
          </div>
        </div>
        <div className="form-row ready-only-item">
    ...
    const onPopupClose = (args) => {
        if (args.type === 'Editor') {
            ...
            if (isSaveAction) {
                ...
                let waitingListTreeViewRefs = [allWaitingDataTreeViewRef, breakfastWaitingDataTreeViewRef, lunchWaitingDataTreeViewRef, dinnerWaitingDataTreeViewRef];
                let currentTreeViewRef = waitingListTreeViewRefs[slotTabRef.current.selectedItem].current;
                let currentTreeViewData = currentTreeViewRef.fields.dataSource;
                const updatedData = currentTreeViewData.filter((item) => item.Id !== draggedItemId);
                currentTreeViewRef.fields.dataSource = updatedData;
                allWaitingData.current = allWaitingData.current.filter(item => item.Id !== draggedItemId);
    ...
    const unplannedEventsUpdatedData = (dataSource, selectedTabIndex) => {
    ...
    const onTreeDragStop = (event) => {
        ...
        if (!treeElement) {
            ...
            if (scheduleElement) {
                ...
                const filteredData = allWaitingData.current.filter((item) => item.Id === parseInt(event.draggedNodeData.id, 10));
                ...
                const tableData = tables.filter((table) => table.id === tableId)[0];
                const tableType = tableCategory.filter((tableGroup) => tableGroup.id === tableData.groupId)[0].category.split(' ')[0];
                if (tableType !== Table) {
                ...
                const isSlotAvailable = scheduleRef.current.isSlotAvailable(StartTime, EndTime, groupIndex) && startDate === endDate && endHour <= 22;
    ...
    return (<div className='control-section table-reservation-control-section'>
      <div className='control-wrapper table-reservation-wrapper'>
        <ScheduleComponent ref={scheduleRef} cssClass='schedule-table-reservation' width='100%' height='550px' selectedDate={selectedDate} group={{ enableCompactView: false, resources: ['Category', 'Tables'] }} resourceHeaderTemplate={resourceHeaderTemplate} startHour="07:00" endHour='22:00' timeScale={timeScale} allowOverlap={false} eventSettings={{
          ...
          <ResourcesDirective>
            <ResourceDirective field="CategoryId" title="Category" name="Category" cssClassField='table-category' allowMultiple={false} dataSource={tableCategory} textField="category" idField="id"/>
            <ResourceDirective field="TableId" title="Tables" name="Tables" cssClassField='table-name' allowMultiple={true} dataSource={tables} textField="name" idField="id" groupIDField='groupId'/>
          </ResourcesDirective>
          <ToolbarItemsDirective>
            ...
            <ToolbarItemDirective name='Custom' type='Button' prefixIcon='e-icons e-show-waiting-list' align='Right' showTextOn='Overflow' overflow='Show' id="overview_toolbar_settings_waiting_list" click={toggleWaitingListElement} cssClass='toolbar-post-agenda'/>
          </ToolbarItemsDirective>
        ...
        <div className="waiting-list-container">
          ...
          <div id="list-container">
            ...
            <TreeViewComponent ref={allWaitingDataTreeViewRef} id="allWaitingDataTreeView" style={{ display: "" }} cssClass='table-reservation-treeview' dragArea=".table-reservation-wrapper .e-content-wrap" nodeTemplate={treeTemplate} fields={allWaitingDataTreeFields} nodeDragStart={onTreeDragStart} nodeDragging={onTreeDragging} nodeDragStop={onTreeDragStop} allowDragAndDrop={true}/>
            <TreeViewComponent ref={breakfastWaitingDataTreeViewRef} id='breakfastWaitingDataTreeView' style={{ display: "none" }} cssClass='table-reservation-treeview' dragArea=".table-reservation-wrapper .e-content-wrap" nodeTemplate={treeTemplate} fields={breakfastWaitingDataTreeFields} nodeDragStart={onTreeDragStart} nodeDragging={onTreeDragging} nodeDragStop={onTreeDragStop} allowDragAndDrop={true}/>
            <TreeViewComponent ref={lunchWaitingDataTreeViewRef} id='lunchWaitingDataTreeView' style={{ display: "none" }} cssClass='table-reservation-treeview' dragArea=".table-reservation-wrapper .e-content-wrap" nodeTemplate={treeTemplate} fields={lunchWaitingDataTreeFields} nodeDragStart={onTreeDragStart} nodeDragging={onTreeDragging} nodeDragStop={onTreeDragStop} allowDragAndDrop={true}/>
            <TreeViewComponent ref={dinnerWaitingDataTreeViewRef} id='dinnerWaitingDataTreeView' style={{ display: "none" }} cssClass='table-reservation-treeview' dragArea=".table-reservation-wrapper .e-content-wrap" nodeTemplate={treeTemplate} fields={dinnerWaitingDataTreeFields} nodeDragStart={onTreeDragStart} nodeDragging={onTreeDragging} nodeDragStop={onTreeDragStop} allowDragAndDrop={true}/>
