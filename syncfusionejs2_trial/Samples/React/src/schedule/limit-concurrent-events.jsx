...
export class LimitConcurrentEvents extends SampleBase {
    scheduleObj;
    data = extend([], dataSource.overlappingData, null, true);
    displayMode = 'limited';
    maxEventsLimit = 1;
    getMaxStack = () => {
    ...
    applyMaxStackToAllViews = (value) => {
        if (!this.scheduleObj)
            return;
        const currentViews = this.scheduleObj.views;
    ...
    render() {
        ...
        <div className='col-lg-8 control-section'>
          <div className='control-wrapper'>
            <ScheduleComponent cssClass='schedule-limit-concurrent' width='100%' height='650px' ref={t => this.scheduleObj = t} currentView='Week' selectedDate={new Date(2026, 4, 29)} eventSettings={{ dataSource: this.getEventDataSource() }} navigating={this.onNavigating}>
              <ViewsDirective>
          ...
          <PropertyPane title='Properties'>
            <div style={{ display: 'flex', alignItems: 'center', height: '70px' }}>
              <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                <RadioButtonComponent cssClass={"schedule-radio-button"} name="eventDisplay" value="all" checked={this.displayMode === 'all'} change={() => this.onDisplayModeChange('all')}/>
            ...
            <div style={{ display: 'flex', alignItems: 'center', height: '70px' }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <RadioButtonComponent cssClass={"schedule-radio-button"} name="eventDisplay" value="limited" checked={this.displayMode === 'limited'} change={() => this.onDisplayModeChange('limited')}/>
              </div>
              <div style={{
                display: 'flex',
                alignItems: 'center',
                ...
                <b>Show up to</b>
                <NumericTextBoxComponent value={this.maxEventsLimit} min={1} width="110px" format="n0" enabled={this.displayMode === 'limited'} change={(e) => this.onLimitChange(e.value)}/>
                <b>labels</b>
              </div>
            </div>
          </PropertyPane>
