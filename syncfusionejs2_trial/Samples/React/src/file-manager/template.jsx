...
export class Template extends SampleBase {
    ...
    onActionSelect = (action, item) => {
        ...
        switch (action) {
            case 'Open':
                this.fmRef.openFile(item.name);
                break;
            case 'Download':
                this.fmRef.downloadFiles([item.name]);
                break;
            case 'Refresh':
                this.fmRef.refreshFiles();
                break;
            case 'Delete':
                this.fmRef.deleteFiles([item.name]);
                break;
            default: break;
        }
    };
    renderDDBIntoHost = (hostEl) => {
        ...
        if (!hostEl.hasAttribute('data-ddb-initialized')) {
            ...
            root.render(<DropDownButtonComponent items={items} cssClass="e-caret-hide filemanager-dropdown-button" iconCss="e-icons e-more-vertical-1" select={onSelect}/>);
    ...
    largeIconsTemplate = (item) => {
        ...
        return (<div className="custom-icon-card">
    ...
    navigationPaneTemplate = (item) => {
        const iconClass = this.getIconsForFolders(item);
        return (<div className="e-nav-pane-node" style={{ display: 'inline-flex', alignItems: 'center' }}>
    ...
    detailsViewSettings = {
        columns: [
            ...
            },
            {
                field: 'size',
                headerText: 'Size',
                template: (item) => <div>{item.isFile ? this.formatSize(item.size) : '-'}</div>,
            },
            {
                field: '_fm_modified',
            ...
            {
                headerText: 'Actions',
                template: (item) => (<div className="action-ddb" data-name={item.name} data-isfile={item.isFile}></div>),
            },
        ],
    };
    render() {
        ...
        <div className="sample-container">
          <FileManagerComponent id="template_filemanager" ref={(scope) => { this.fmRef = scope; }} ajaxSettings={{
