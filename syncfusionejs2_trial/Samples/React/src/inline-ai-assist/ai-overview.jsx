...
export class Overview extends SampleBase {
    InlineAssistRef = React.createRef();
    ...
    handleAIAssistantClick = () => {
        if (this.InlineAssistRef.current && this.aiAssistantBtnRef.current) {
            ...
            this.InlineAssistRef.current.relateTo = this.aiAssistantBtnRef.current.element;
            this.InlineAssistRef.current.dataBind();
            this.InlineAssistRef.current.showPopup();
        }
    };
    ...
    handleResponseItemSelect = (args) => {
        if (args.command.label === 'Accept') {
            if (this.state.isGlobalRequest && this.emailContentRef.current) {
                const lastPrompt = this.InlineAssistRef.current?.prompts[(this.InlineAssistRef.current?.prompts?.length || 1) - 1];
                if (lastPrompt) {
    ...
    render() {
        ...
        <div className="control-section">
          <div className="overview-inlineAIAssist">
              ...
              <div className="email-composer e-card">
                ...
                <div className="email-field-vertical e-card-content">
                  <div className="message-header">
                    <ButtonComponent ref={this.aiAssistantBtnRef} id="aiAssistantBtn" iconCss="e-icons e-ai-chat" isPrimary={true} title="AI Assistant">
                      AI Assistant
                    </ButtonComponent>
                  </div>
                  ...
                  <ButtonComponent ref={this.sparkleButtonRef} id="sparkleBtn" iconCss="e-icons e-ai-chat" isPrimary={true} style={{ display: 'none' }} title="AI Assistant"/>
                </div>

                <div className="email-actions e-card-content">
                  <ButtonComponent id="sendEmailBtn" title="Send" cssClass="e-primary" onClick={this.handleSendEmail}>
                    Send Email
                  </ButtonComponent>
                </div>
              </div>
            ...
            <InlineAIAssistComponent ref={this.InlineAssistRef} id="inlineAssist" commandSettings={this.commandSettings} relateTo="#emailContent" promptRequest={this.handlePromptRequest} responseSettings={responseSettings} open={this.handlePopupOpen} close={this.handlePopupClose}/>
          </div>
        </div>
