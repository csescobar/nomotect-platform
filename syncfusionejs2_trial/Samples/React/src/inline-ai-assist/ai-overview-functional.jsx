...
const Overview = () => {
    ...
    const [selectedCommandText, setSelectedCommandText] = React.useState('');
    const [isGlobalRequest, setIsGlobalRequest] = React.useState(false);
    const [isPopupOpen, setIsPopupOpen] = React.useState(false);
    const inlineAssistRef = React.useRef(null);
    const emailContentRef = React.useRef(null);
    const sparkleButtonRef = React.useRef(null);
    const aiAssistantBtnRef = React.useRef(null);
    const currentHoveredParagraphRef = React.useRef(null);
    const abortControllerRef = React.useRef();
    // Synchronous ref to reliably track hovered element without state delays
    const currentHoveredParagraphSyncRef = React.useRef(null);
    const attachHoverEvent = React.useCallback((element) => {
    ...
    const attachHoverEventsToChildren = React.useCallback((container) => {
        Array.from(container.children).forEach((child) => {
            const element = child;
            // Remove old listener first to avoid duplicates
            const existingListener = element.__hoverListener;
            if (existingListener) {
    ...
    const updateSparkleButtonPosition = React.useCallback((element) => {
        const sparkleButton = sparkleButtonRef.current?.element;
        if (!sparkleButton || !emailContentRef.current?.parentElement)
            return;
    ...
    const handleEmailMouseLeave = React.useCallback((e) => {
        const sparkleButton = sparkleButtonRef.current?.element;
    ...
    const handleSparkleClick = React.useCallback(() => {
    ...
    const handleAIAssistantClick = React.useCallback(() => {
    ...
    const handlePromptRequest = React.useCallback((args) => {
    ...
    const handleResponseItemSelect = React.useCallback((args) => {
        if (args.command.label === 'Accept') {
            if (isGlobalRequest && emailContentRef.current) {
                const lastPrompt = inlineAssistRef.current?.prompts[(inlineAssistRef.current?.prompts?.length || 1) - 1];
                if (lastPrompt) {
    ...
    React.useMemo(() => {
        commandSettings.itemSelect = (args) => {
    ...
    return (<div className="control-pane">
      <div className="control-section">
        <div className="overview-inlineAIAssist">
            ...
            <div className="email-composer e-card">
              ...
              <div className="email-field-vertical e-card-content">
                <div className="message-header">
                  <ButtonComponent ref={aiAssistantBtnRef} id="aiAssistantBtn" title="AI Assistant" iconCss="e-icons e-ai-chat" isPrimary={true} onClick={handleAIAssistantClick}>
                      AI Assistant
                    </ButtonComponent>
                </div>
                <div ref={emailContentRef} className="email-body" id="emailContent" contentEditable={true} dangerouslySetInnerHTML={{ __html: initialEmailContent }}/>
                <ButtonComponent ref={sparkleButtonRef} id="sparkleBtn" title="AI Assistant" iconCss="e-icons e-ai-chat" isPrimary={true} style={{ display: 'none' }} onClick={handleSparkleClick}/>
              </div>
          ...
          <InlineAIAssistComponent ref={inlineAssistRef} id="inlineAssist" commandSettings={commandSettings} relateTo="#emailContent" promptRequest={handlePromptRequest} responseSettings={responseSettings} open={() => setIsPopupOpen(true)} close={() => {
