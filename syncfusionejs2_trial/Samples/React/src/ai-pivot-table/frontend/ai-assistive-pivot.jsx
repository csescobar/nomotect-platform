...
import { CalculatedField, GroupingBar, FieldList, Toolbar, PivotViewComponent, Inject, ConditionalFormatting, NumberFormatting } from '@syncfusion/ej2-react-pivotview';
let dialog;
let pivotObj;
let assistView;
...
function AIAssistivePivot() {
    ...
    const toolbarOptions = ['FieldList', 'Grid', 'Chart', { text: 'AI Assist', tooltipText: 'AI Assist', prefixIcon: 'e-assistview-icon', id: 'ai-assist-btn', align: 'Right', click: toolbarClick.bind(this) }];
    ...
    return (<div>
            <div id='assistive-pivot'>
                <DialogComponent ref={(dialogIns) => dialog = dialogIns} id='ai-assist-dialog' width='500px' visible={false} height='500px' footerTemplate={dialogFooterTemplate}>
                    <AIAssistViewComponent id="ai-pivot-aiassistview" ref={(assist) => assistView = assist} toolbarSettings={toolbarSettings} promptRequest={onPromptRequest} promptSuggestionsHeader='Suggestions' responseItemTemplate={responseTemplate}>
                        <ViewsDirective>
                            <ViewDirective type='Assist' name=' Ask AI'></ViewDirective>
                        </ViewsDirective>
                    </AIAssistViewComponent>
                </DialogComponent>
                 <PivotViewComponent id='ai-pivot' ref={(pivot) => pivotObj = pivot} dataSourceSettings={dataSourceSettings} width={'100%'} height={'650'} gridSettings={{ columnWidth: 140 }} displayOption={{ view: 'Both', primary: 'Table' }} enableValueSorting={true} allowCalculatedField={true} showGroupingBar={true} showFieldList={true} showToolbar={true} allowConditionalFormatting={true} allowNumberFormatting={true} toolbar={toolbarOptions} toolbarRender={beforeToolbarRender}>
