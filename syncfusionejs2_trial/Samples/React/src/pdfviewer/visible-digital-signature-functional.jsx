...
const ImageList = ({ imageUrlsProp, selectedIndexProp }, ref) => {
    ...
    return (imageUrls.length > 0 &&
        <div className="e-pv-visible-sign-uploaded-images">
            {imageUrls.map((src, index) => (<div key={index} className={`e-pv-visible-sign-image-wrapper ${selectedIndex === index ? 'selected' : ''}`} onClick={() => handleImageClick(index)}>
                    {index != 0 &&
...
const VisibleDigitalSignature = () => {
    ...
    useEffect(() => {
        ...
        if (!isNullOrUndefined(viewer) && !isNullOrUndefined(viewer.current)) {
            viewer.current.documentLoad = async (args) => {
    ...
    const renderTabHeader = () => (<div>
            ...
            <div className="e-pv-visible-sign-tab-header">
                {tabHeaders.map((header) => (<div key={header.key} onClick={() => setActiveTab(header.key)} className={`e-pv-visible-sign-tab-header-item${activeTab === header.key ? ' active' : ''}`}>
            ...
            <div className='e-pv-visible-sign-signature-image-group'>
                ...
                {showSignatureImages && (<>
                        <div id='e-pv-visible-sign-image-uploader' style={{ display: 'none' }}>
                            <UploaderComponent ref={(scope) => { uploaderObj = scope; }} id="signature-image-uploader" allowedExtensions=".jpg,.jpeg,.png" dropArea=".e-pv-visible-sign-tab-panel" asyncSettings={pdfviewerApiPath} success={onFileSuccess} showFileList={false} selected={onFileSelect} multiple={false}/>
                        </div>
    ...
    const renderSignatureDescription = () => (<div className='e-pv-visible-sign-group'>
            ...
            <table className='e-pv-visible-sign-inner-table'>
                <tbody>
                    {['signer', 'reason', 'location', 'date'].map((field) => (<tr key={field}>
                            ...
                            <td>
                                {field == 'date' ?
                <DatePickerComponent placeholder={`Enter ${field}`} value={new Date(textBoxValues.current[field])} format={'MM-dd-yy'} inputFormats={['dd-MM-yy', 'MM-dd-yy']} enabled={checkboxStates[field]} allowEdit={false} max={new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate())} showClearButton={false} change={(e) => handleTextFieldChange(field, e.value)}/>
                :
    ...
    const renderDropdowns = () => (<div className='e-pv-visible-sign-group'>
            <table className='e-pv-visible-sign-inner-table'>
                <tbody>
                    <tr>
                        <td>
    ...
    const createContent = (index) => (<div className='e-pv-visible-sign-tab'>
    ...
    const existingFieldContent = () => (<div>
            <div className='e-pv-visible-sign-group'>
                <table className='e-pv-visible-sign-inner-table'>
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <div className='e-pv-visible-sign-text-content'>
                                    <DropDownListComponent dataSource={['Signature Field 1']} value={signatureField.current} placeholder="Select existing field"/>
                                </div>
                            </td>
                        </tr>
                    </tbody>
    ...
    const renderSignDocumentButton = () => (<div className="e-pv-visible-sign-tab-content-footer">
    ...
    const signDocument = async () => {
        viewer.current.saveAsBlob()
            ...
            fileReader.onload = (event) => {
                ...
                const jsonObject = getRequestBody(pdfData);
    ...
    const getRequestBody = (pdfData) => {
        ...
        if (showSignatureImages && displayMode.current != displayModes[2]) {
            const selectedImageData = imageListRef.current?.getSelectedImageUrl();
