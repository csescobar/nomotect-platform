...
import { tasksCollectionRisk } from './datasource';
...
function RiskAnalysis() {
    ...
    const rightLabelTemplate = (data) => {
        return (<div style={{ marginTop: '-7px' }}>
                {data.ganttProperties && (<div id="rightLabel">
                        {window.getSummary(data.ganttProperties)}
                    </div>)}
            </div>);
    };
    return (<div className='control-pane'>
            <div className='control-section'>
                <div id='container'>
                    <GanttComponent id="GanttContainer" ref={gantt => ganttInstance = gantt} dataSource={tasksCollectionRisk} allowSorting={true} allowReordering={true} enableContextMenu={true} toolbar={toolbarOptions} taskFields={taskFields} baselineColor='red' editSettings={{
            allowAdding: true,
            allowEditing: true,
            allowDeleting: true,
            allowTaskbarEditing: true,
            showDeleteConfirmDialog: true
        }} allowFiltering={true} gridLines="Both" highlightWeekends={true} queryTaskbarInfo={(args) => {
            if (summary !== '') {
                for (let i = 0; i < summary.length; i++) {
