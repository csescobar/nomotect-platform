...
function PrintExport() {
    ...
    const [fileName, setFileName] = React.useState('Sankey'); // default (Capital S)
    const [isPanelOpen, setIsPanelOpen] = React.useState(false);
    const originalSvgWidthRef = React.useRef(null);
    ...
    const getSankeySvg = React.useCallback(() => {
        ...
        if (byId instanceof SVGSVGElement)
            return byId;
        const root = sankeyRef.current?.element;
        if (root) {
    ...
    const handleExport = React.useCallback(() => {
        const type = (formatRef.current?.value || 'JPEG');
        const safeName = (fileName || 'Sankey').trim();
        const doExport = () => {
            const element = sankeyRef.current?.element;
    ...
    return (<div className="control-pane">
      ...
      <div className="chart-toolbar" role="toolbar" aria-label="Chart actions">
        <button className="toolbar-btn toolbar-btn--print" onClick={handlePrint} onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                handlePrint();
            }
        ...
        <button className="toolbar-btn toolbar-btn--export" onClick={() => setIsPanelOpen(true)} onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                setIsPanelOpen(true);
            }
      ...
      <div id="chartShell" ref={chartShellRef} className={`chart-shell ${isPanelOpen ? 'with-panel' : ''} ${Browser.isDevice && isPanelOpen ? 'mobile-panel-open' : ''}`}>
        <div id="chartHost" ref={chartHostRef} className="chart-host" style={{ display: Browser.isDevice && isPanelOpen ? 'none' : undefined }}>
          <SankeyComponent id="sankey-print-export" ref={sankeyRef} width="95%" height="450" title="Supply Chain Management" subTitle="Source: OECD‑ITF Global Freight Data" enableRtl={false} orientation="Horizontal" background="transparent" margin={{ left: 20, right: 20, top: 20, bottom: 20 }} border={{ color: '#E0E0E0', width: 0 }} linkStyle={{ opacity: 0.4, curvature: 0.5, colorType: 'Source' }} labelSettings={{ visible: Browser.isDevice ? false : true }} tooltip={{
            enable: true,
        ...
        }} legendSettings={{ visible: true }} load={loadSankeyChartTheme} loaded={onLoaded}>
        ...
        <aside id="exportPanel" ref={exportPanelRef} className="export-panel" aria-hidden={!isPanelOpen}>
          <div className="panel-header">
            ...
            <button className="panel-close" aria-label="Close export panel" onClick={() => setIsPanelOpen(false)} onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                setIsPanelOpen(false);
            }
          ...
          <div className="panel-body">
            ...
            <input id="expFileName" ref={fileNameInputRef} className="panel-input" type="text" value={fileName} placeholder="Input Text" onChange={(element) => setFileName(element.target.value)} onKeyDown={(e) => {
            if (e.key === 'Escape') {
                e.preventDefault();
                setIsPanelOpen(false);
            }
            ...
            <DropDownListComponent id="fileTypeDropdown" ref={formatRef} dataSource={['JPEG', 'PNG', 'SVG', 'PDF']} value={'JPEG'} width={'100%'}/>

            <div className="panel-footer" style={{ background: 'transparent' }}>
              <button className="panel-action-btn panel-action-btn--export" onClick={handleExport} onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                handleExport();
            }
              ...
              <button className="panel-action-btn panel-action-btn--cancel" onClick={() => setIsPanelOpen(false)} onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                setIsPanelOpen(false);
            }
