...
import { SankeyComponent, SankeyNodeDirective, SankeyNodesCollectionDirective, SankeyLinkDirective, SankeyLinksCollectionDirective, Inject, SankeyTooltip, SankeyLegend, SankeyHighlight, SankeyExport } from '@syncfusion/ej2-react-charts';
    ...
    render() {
        return (<div className="control-pane">
                ...
                <div className="control-section">
                    <SankeyComponent id="sankey-container" width="90%" height={Browser.isDevice ? "600" : "450"} title='California Energy Consumption in 2023' subTitle='Source: Lawrence Livermore National Laboratory' linkStyle={{ opacity: 0.6, curvature: 0.55, colorType: 'Source' }} labelSettings={{ visible: Browser.isDevice ? false : true }} tooltip={{ enable: true, nodeTemplate: '${name}: ${value} TBtu', linkTemplate: Browser.isDevice ? '${start.name}: ${start.out} TBtu → <br/> ${target.name}: ${target.in} TBtu' : '${start.name}: ${start.out} TBtu → <br/> ${target.name}: ${target.in} TBtu' }} legendSettings={{ visible: true, position: 'Bottom', itemPadding: 8 }} load={this.load} loaded={this.onLoaded}>
                        <Inject services={[SankeyTooltip, SankeyLegend, SankeyHighlight, SankeyExport]}/>
                        <SankeyNodesCollectionDirective>
                            {nodes.map((node) => (<SankeyNodeDirective key={node.id} id={node.id} color={node.color} offset={node.offset}/>))}
                        </SankeyNodesCollectionDirective>
                        <SankeyLinksCollectionDirective>
                            {links.map((link, i) => (<SankeyLinkDirective key={`${link.sourceId}-${link.targetId}-${i}`} sourceId={link.sourceId} targetId={link.targetId} value={link.value}/>))}
                        </SankeyLinksCollectionDirective>
                    </SankeyComponent>
                </div>
