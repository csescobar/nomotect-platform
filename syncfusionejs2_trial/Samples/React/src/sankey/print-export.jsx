...
import { SankeyComponent, SankeyNodesCollectionDirective, SankeyNodeDirective, SankeyLinksCollectionDirective, SankeyLinkDirective, Inject, SankeyTooltip, SankeyLegend, SankeyHighlight, SankeyExport } from '@syncfusion/ej2-react-charts';
...
class PrintExport extends React.Component {
    sankeyRef = React.createRef();
    formatRef = React.createRef();
    fileName = 'Sankey';
    chartShellRef = React.createRef();
    chartHostRef = React.createRef();
    exportPanelRef = React.createRef();
    fileNameInputRef = React.createRef();
    resizeObserver = null;
    originalSvgWidth = null;
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.state.isPanelOpen) {
            ...
            document.removeEventListener('keydown', this.handlePanelKeydown);
            const exportButton = document.querySelector('.toolbar-btn--export');
            exportButton?.focus();
            setTimeout(() => {
                this.resizeSankeySvg();
                this.refreshChart();
    ...
    getSankeySvg = () => {
        ...
        if (byId instanceof SVGSVGElement)
            return byId;
        const root = this.sankeyRef.current?.element;
        if (root) {
    ...
    resizeSankeySvg = () => {
        ...
        const sankeyHost = document.getElementById('sankey-print-export');
        if (sankeyHost)
            sankeyHost.style.width = `${newWidth}px`;
    };
    handleExport = () => {
        const type = (this.formatRef.current?.value || 'JPEG');
        const name = (this.fileName || 'Sankey').trim();
        const doExport = () => {
            const el = this.sankeyRef.current?.element;
    ...
    openPanel = () => this.setState({ isPanelOpen: true });
    ...
    render() {
        ...
        <div className="chart-toolbar" role="toolbar" aria-label="Chart actions">
          <button className="toolbar-btn toolbar-btn--print" onClick={this.handlePrint} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    this.handlePrint();
                }
          ...
          <button className="toolbar-btn toolbar-btn--export" onClick={this.openPanel} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    this.openPanel();
                }
        ...
        <div id="chartShell" ref={this.chartShellRef} className={`chart-shell ${isPanelOpen ? 'with-panel' : ''} ${Browser.isDevice && isPanelOpen ? 'mobile-panel-open' : ''}`}>
          <div id="chartHost" ref={this.chartHostRef} className="chart-host" style={{ display: Browser.isDevice && isPanelOpen ? 'none' : undefined }}>
            <SankeyComponent id="sankey-print-export" ref={this.sankeyRef} width="95%" height="450" title="Supply Chain Management" subTitle="Source: OECD‑ITF Global Freight Data" enableRtl={false} orientation="Horizontal" background="transparent" margin={{ left: 20, right: 20, top: 20, bottom: 20 }} border={{ color: '#E0E0E0', width: 0 }} linkStyle={{ opacity: 0.4, curvature: 0.5, colorType: 'Source' }} labelSettings={{ visible: Browser.isDevice ? false : true }} tooltip={{
                enable: true,
          ...
          <aside id="exportPanel" ref={this.exportPanelRef} className="export-panel" aria-hidden={!isPanelOpen}>
            <div className="panel-header">
              ...
              <button className="panel-close" aria-label="Close export panel" onClick={this.closePanel} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    this.closePanel();
                }
            ...
            <div className="panel-body">
              ...
              <input id="expFileName" ref={this.fileNameInputRef} className="panel-input" type="text" defaultValue={this.fileName} placeholder="Input Text" onChange={(element) => { this.fileName = element.target.value; }} onKeyDown={(e) => {
                if (e.key === 'Escape') {
                    e.preventDefault();
                    this.closePanel();
                }
              ...
              <DropDownListComponent id="fileTypeDropdown" ref={this.formatRef} dataSource={['JPEG', 'PNG', 'SVG', 'PDF']} value={'JPEG'} width={'100%'}/>

              <div className="panel-footer" style={{ background: 'transparent' }}>
                <button className="panel-action-btn panel-action-btn--export" onClick={this.handleExport} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    this.handleExport();
                }
                ...
                <button className="panel-action-btn panel-action-btn--cancel" onClick={this.closePanel} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    this.closePanel();
                }
