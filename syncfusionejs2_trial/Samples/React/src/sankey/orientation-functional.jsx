...
function Orientation() {
    ...
    return (<div className="control-pane">
      ...
      <div className="control-section">
        <SankeyComponent id="sankey-orientation" width="90%" height="650" title={Browser.isDevice ? 'U.S. Greenhouse Gas Emissions' : 'U.S. Greenhouse Gas Emissions by Economic Sector (2022)'} subTitle='Source: EPA 2022 GHG Inventory' orientation="Vertical" linkStyle={{ opacity: 0.5, curvature: 0.55, colorType: 'Source' }} nodeStyle={{ width: 30, padding: 8, opacity: 1 }} labelSettings={{ visible: Browser.isDevice ? false : true }} tooltip={{ enable: true, nodeTemplate: '${name}: ${value} MMT CO₂e', linkTemplate: Browser.isDevice ? '${start.name}: ${start.out} MMT CO₂e → <br/> ${target.name}: ${target.in} MMT CO₂e' : '${start.name}: ${start.out} MMT CO₂e → ${target.name}: ${target.in} MMT CO₂e' }} legendSettings={{ visible: Browser.isDevice ? false : true, position: 'Right', margin: { left: 100 } }} load={loadSankeyChartTheme} loaded={onLoaded}>
