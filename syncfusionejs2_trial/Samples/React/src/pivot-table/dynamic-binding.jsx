...
function DynamicBindingComponent() {
    ...
    const lastRemoteRef = useRef({ kind: null, url: null });
    ...
    const applyReportSettings = async (pivot, reportSettings, isOlapReport, entireReportSettings) => {
        if (isOlapReport) {
            setCurrentData([]);
        ...
        const hasInlineIncoming = Array.isArray(reportSettings.dataSource) &&
            reportSettings.dataSource.length > 0;
        ...
        reader.onload = async (evt) => {
            try {
                if (isCsv) {
                    ...
                    const unwrappedData = parsed && typeof parsed === 'object' && 'record' in parsed
                        ? parsed.record
                        : parsed;
    ...
    function toolbarRender(args) {
        const connectMenu = { template: '<ul id="connect_menu"></ul>', id: 'custom_toolbar' };
        args.customToolbar.splice(0, 0, connectMenu);
        const openMenu = { template: '<ul id="open_menu"></ul>', id: 'open_toolbar' };
        ...
        const separator3 = { type: 'Separator' };
  ...
  <soap:Body>
    ...
    const parseRowset = (xmlText) => {
        ...
        const result = rows.map((r) => {
            const obj = {};
            Array.from(r.children).forEach((c) => { obj[c.localName] = (c.textContent ?? '').trim(); });
            return obj;
        });
    ...
    const discoverCatalogs = async (endpoint) => {
    ...
    const loadRemoteAndBind = async (kind, url) => {
        ...
        if (kind === 'CSV') {
            ...
            if (!res.ok)
                return res.text().then(() => { throw new Error(`HTTP ${res.status}: ${res.statusText}`); });
            const csvString = await res.text();
            ...
            if (!res.ok)
                throw new Error(`HTTP ${res.status}: ${res.statusText}`);
            const jsonData = await res.json();
            const unwrappedData = jsonData && typeof jsonData === 'object' && 'record' in jsonData
                ? jsonData.record
                : jsonData;
    ...
    return (<div className='control-pane'>
            <div className='control-section' id='pivot-table-section' style={{ overflow: 'initial' }}>
                ...
                {isDialogOpen && dialogType !== 'OLAP' && (<DialogComponent visible={isDialogOpen} isModal showCloseIcon width="480px" header={dialogType === 'JSON Report' ? `Load Pivot Report` : (`Connect to ${dialogType}`)} close={() => setDialogOpen(false)} target=".control-pane" closeOnEscape overlayClick={() => setDialogOpen(false)} position={{ X: 'center', Y: 'center' }} animationSettings={{ effect: 'Zoom', duration: 150 }}>
            ...
            handleOpenRemote();
                ...
                {isErrorDialogOpen && (<DialogComponent visible={isErrorDialogOpen} isModal showCloseIcon width="420px" header="Error" close={() => setIsErrorDialogOpen(false)} target=".control-pane" closeOnEscape overlayClick={() => setIsErrorDialogOpen(false)} position={{ X: 'center', Y: 'center' }} animationSettings={{ effect: 'Fade', duration: 120 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {isDialogOpen && dialogType === 'OLAP' && (<DialogComponent visible={isDialogOpen} isModal showCloseIcon width="620px" header="Connect to OLAP(XMLA)" close={() => setDialogOpen(false)} target=".control-pane" closeOnEscape overlayClick={() => setDialogOpen(false)} position={{ X: 'center', Y: 'center' }} animationSettings={{ effect: 'Zoom', duration: 150 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                try {
                    ...
                    setLoadingSources(false);
                            ...
                            <div className="olap-row">
                                ...
                                <DropDownListComponent value={selectedDataSource} dataSource={olapDataSources} fields={{ text: 'value', value: 'value' }} placeholder={loadingSources ? 'Loading…' : 'Select data source'} enabled={olapConnected || loadingSources} change={async (e) => {
                const v = e.value;
                ...
                try {
                    const cats = await discoverCatalogs(olapProxyUrl);
                    ...
                    setLoadingCatalogs(false);
                            ...
                            <div className="olap-row">
                                ...
                                <DropDownListComponent value={selectedCatalog} dataSource={olapCatalogs} fields={{ text: 'value', value: 'value' }} placeholder={loadingCatalogs ? 'Loading…' : 'Select catalog'} enabled={!!selectedDataSource || loadingCatalogs} change={async (e) => {
                const v = e.value;
                ...
                try {
                    ...
                    setLoadingCubes(false);
                            ...
                            <div className="olap-row">
                                ...
                                <DropDownListComponent value={selectedCube} dataSource={olapCubes} fields={{ text: 'value', value: 'value' }} placeholder={loadingCubes ? 'Loading…' : 'Select cube'} enabled={!!selectedCatalog || loadingCubes} change={async (e) => {
                const v = e.value;
                setSelectedCube(v);
            }} style={{ width: '100%' }}/>
                            </div>
                ...
                <div className='pivot-table-control-section'>
                    <PivotViewComponent id='PivotView' ref={pivotObj} dataSourceSettings={dataSourceSettings} width={'100%'} height={500} showFieldList showToolbar allowCalculatedField={true} allowPdfExport={true} allowExcelExport={true} allowNumberFormatting={true} allowConditionalFormatting={true} toolbar={toolbarOptions} toolbarRender={toolbarRender} dataBound={onDataBound} enginePopulated={onEnginePopulated} displayOption={{ view: 'Both' }} gridSettings={{ columnWidth: Browser.isDevice ? 100 : 120 }}>
            ...
            <div id="description">
                ...
                <br />
                <p>More information on the Essential® JS2 Pivot Table can be found in these <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/pivotview/data-binding">Data Binding</a> and
