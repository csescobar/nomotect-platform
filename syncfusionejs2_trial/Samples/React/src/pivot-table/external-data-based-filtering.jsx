 ...
 */
let Pivot_Data = rData.data;
export class ExternalDataBasedFiltering extends SampleBase {
    pivotObj;
    ...
    applyDateFilter() {
        if (this.startDate && this.endDate) {
            ...
            let pivotData = Pivot_Data.map((item) => ({
                ...item,
                OrderDate: new Date(item.OrderDate),
            }));
            new DataManager({ json: pivotData, adaptor: new JsonAdaptor() }).executeQuery(new Query()
        ...
        return (<div className='control-pane'>
                <div className='control-section'>
                    <div style={{ padding: '0px 10px' }}>
                        ...
                        <DatePickerComponent id='start-datepicker' placeholder='Choose a start date' min={new Date(2019, 0, 1)} max={new Date(2024, 10, 31)} value={this.startDate} width={200} format='MMM yyyy' start='Year' depth='Year' change={(args) => this.setStartDate(args)} cssClass='pivot-datepicker'/>
                        <strong style={{ display: 'inline-block', fontSize: '14px', marginRight: '5px' }}>End Date:</strong>
                        <DatePickerComponent id='end-datepicker' placeholder='Choose an end date' min={new Date(2019, 1, 1)} max={new Date(2024, 11, 31)} value={this.endDate} width={200} format='MMM yyyy' start='Year' depth='Year' change={(args) => this.setEndDate(args)} cssClass='pivot-datepicker'/>
                        <ButtonComponent id='apply' cssClass='e-primary' style={{ width: '80px' }} onClick={this.applyDateFilter.bind(this)}>
                            Apply
                        </ButtonComponent>
                    </div>
                    <PivotViewComponent id='PivotView' ref={(pivotview) => { this.pivotObj = pivotview; }} dataSourceSettings={this.state.dataSourceSettings} width={'100%'} height={'500'} gridSettings={{ columnWidth: 140 }}>
                    </PivotViewComponent>
                </div>
