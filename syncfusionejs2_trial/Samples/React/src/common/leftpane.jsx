...
function updateGroupItemAttributes() {
    const groupItems = document.querySelectorAll('#controlList .e-list-group-item.e-level-1');
    groupItems.forEach((groupItem) => {
        ...
        while (sibling && !sibling.classList.contains('e-list-group-item')) {
            if (!groupItem.hasAttribute('group-name')) {
                const groupName = sibling.getAttribute('group-name');
                if (groupName) {
...
export class LeftPane extends React.Component {
    ...
    getDataSource() {
        ...
        for (let temp of tempData) {
            ...
            if (Browser.isDevice && (temp.hideOnDevice == true || temp.hideOnDevice === 'true')) {
                continue;
            }
            let data = new DataManager(temp.samples);
            let query = new Query().where('ignoreOnBuild', 'notEqual', true);
            if (Browser.isDevice) {
     ...
     */
    getSamples(samples, groupPath) {
        let tempSamples = [];
        let groupName = '';
        let sampleNameAttr = '';
        let isAISample = !!groupPath && groupPath.startsWith('ai-') && ['ai-assistview', 'ai-smart-paste', 'ai-smart-textarea'].indexOf(groupPath) === -1;
