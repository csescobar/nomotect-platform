...
import { Animation, L10n, setCulture, setCurrencyCode, loadCldr, Browser, createElement, closest, enableRipple, select, selectAll, registerLicense, getComponent } from '@syncfusion/ej2-base';
...
L10n.load(Locale);
setCulture('en');
registerLicense(window.syncfusion_license);
/**
 * Mobile View.
 */
...
let dropdownsRendered = false;
//events bound flag
let eventsBound = false;
//mount samples flag
let leftPaneRoot = null;
let tabRoot = null;
/**
...
const control = hashParts[2];
const specialControls = ['pdfviewer', 'spreadsheet', 'document-editor'];
if (specialControls.indexOf(control) !== -1) {
    ...
    const folder = folderMap[control]; // Use the already parsed theme
    const newUrl = `https://document.syncfusion.com/demos/${folder}/react/#/${theme}/${control}/default`;
...
let headerProductsSwitch = document.getElementById('header-products-switcher');
let prevAction;
let themeDropDown;
...
function renderSbPopups() {
    ...
    productsPopup = new Popup(document.getElementById('products-popup'), {
        offsetY: 2,
        zIndex: 10012,
        relateTo: select('.sb-header-item.sb-table-cell.sb-products-wrapper'), position: { X: 'left', Y: 'bottom' },
        collision: { X: 'flip', Y: 'flip' }
    });
...
function bindEvents() {
    ...
    headerProductsSwitch.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
...
function onsearchInputChange(e) {
    ...
    let value = [];
    for (let file of val) {
        if ((file.doc.ignoreOnBuild !== true && file.doc.ignoreOnBuild !== 'true') && !(Browser.isDevice && (file.doc.hideOnDevice === true || file.doc.hideOnDevice === 'true'))) {
...
function ThemeChangeButton() {
    ...
    return (isVisible ? (<button className={`sb-themeswitch-btn`} onClick={toggleDarkMode}>
