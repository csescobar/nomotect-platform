...
const AccumulationLegendTemplate = () => {
    ...
    const legendTemplate = '<div class="legend-template" style="display:flex; align-items:flex-start; gap:' + (Browser.isDevice ? '6px' : '8px') + '; opacity:1; max-width:' + (Browser.isDevice ? '160px' : '280px') + '; box-sizing:border-box;">' +
    ...
    const legendRender = (args) => {
        ...
        const matchedPoint = chart && chart.series && chart.series[0]
            ? chart.series[0].points.find((p) => p.x === args.text)
            : null;
    ...
    const load = (args) => { loadAccumulationChartTheme(args); };
    return (<div className='control-pane'>
      <style>{SAMPLE_CSS}</style>
      <div className='control-section' style={{ textAlign: "center" }}>
        <AccumulationChartComponent style={{ textAlign: "center" }} id='accumulationLegendFunctional' ref={(chart) => { chartRef.current = chart; }} title='Top 5 Oil Producing Countries (2023)' subTitle='Source: Wikipedia.org' titleStyle={{
            position: 'Custom',
        ...
        }} enableBorderOnMouseMove={false} legendRender={legendRender} load={load} legendSettings={{
            visible: true,
          ...
          <AccumulationSeriesCollectionDirective>
            <AccumulationSeriesDirective dataSource={data} xName='x' yName='y' tooltipMappingName='tooltip' border={{ color: '#ffffff', width: 1 }} radius={Browser.isDevice ? '65%' : '70%'} innerRadius='0%' animation={{ enable: false }} type='Pie' dataLabel={{
            visible: true,
      ...
      <div id="description">
        <p>
          This sample uses the <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/accumulation-chart/legendSettings/#template" aria-label="Navigate to the documentation for template in legendSettings in the EJ2 Accumulation Chart control"><code>template</code></a> property of
          <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/accumulation-chart/legendSettings" aria-label="Navigate to the documentation for LegendSettings in the EJ2 Accumulation Chart control"> <code>legendSettings</code></a> to
          fully customize legend items.
          
          The <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/accumulation-chart/iacclegendrendereventargs" aria-label="Navigate to the documentation for legendRender event in the EJ2 Accumulation Chart control"><code>legendRender</code></a> event
        ...
        <p>
          More information on legend customization in accumulation charts can be found in the <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/accumulation-chart/legend/" aria-label="Navigate to the documentation for Legend in TypeScript Accumulation Chart control">documentation</a>.
        </p>
      </div>
    </div>);
};
