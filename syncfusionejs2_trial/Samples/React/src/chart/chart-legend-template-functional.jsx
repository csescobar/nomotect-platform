...
const legendTemplate = '<div style="display:flex;align-items:center;gap:' + (Browser.isDevice ? '1px' : '8px') + ';opacity:1;">' +
...
`;
const ChartLegendTemplate = () => {
    const chartRef = useRef(null);
    useEffect(() => {
        updateSampleSection();
    }, []);
    const loaded = (args) => {
        let chart = document.getElementById('legendTemplateFunctional');
        chart.setAttribute('title', '');
    };
    const legendRender = (args) => {
        const chart = chartRef.current;
        const matchedSeries = chart && chart.series
            ? chart.series.find((s) => s.name === args.text)
            : null;
        const opacity = matchedSeries && !matchedSeries.visible ? '0.5' : '1';
    ...
    const load = (args) => { loadChartTheme(args); };
    return (<div className='control-pane'>
      <style>{SAMPLE_CSS}</style>
      <div className='control-section'>
        <ChartComponent id='legendTemplateFunctional' ref={(chart) => { chartRef.current = chart; }} style={{ textAlign: 'center' }} primaryXAxis={{
            valueType: 'Category',
        ...
        }} legendRender={legendRender} load={load} loaded={loaded.bind(this)} width={Browser.isDevice ? '100%' : '75%'}>
      ...
      <div id="description">
        <p>
          ...
          The legend items are customized using the <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/chart/legendSettings/#template" aria-label="Navigate to the documentation for template in LegendSettings in the EJ2 Chart control"><code>template</code></a> property of
          <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/chart/legendSettings/" aria-label="Navigate to the documentation for LegendSettings in the EJ2 Chart control"> <code>legendSettings</code></a>,
          which allows embedding custom HTML content such as medal images and styled text into each legend item.
        </p>
        <p>
          The <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/chart/#legendrender" aria-label="Navigate to the documentation for legendRender event in EJ2 Chart control"><code>legendRender</code></a> event
        ...
        <p>
          More information on chart legends can be found in the <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/chart/legend" aria-label="Navigate to the documentation for Legend in TypeScript Chart control">documentation</a>.
        </p>
      </div>
    </div>);
};
