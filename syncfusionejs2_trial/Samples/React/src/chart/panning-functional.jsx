...
import { ChartComponent, SeriesCollectionDirective, SeriesDirective, Inject, Category, DataLabel, SplineAreaSeries, ChartAnnotation, AnnotationsDirective, AnnotationDirective } from '@syncfusion/ej2-react-charts';
...
const Pagination = () => {
    ...
    let count = 25;
    let day = 'Friday';
    let updateChart = (buttonId, img, tempCount, chartDay, zoomPos, zoomFactor) => {
        image = img;
        count = tempCount;
        day = chartDay;
    ...
    return (<div className='control-pane' style={{ textAlign: "center" }}>
      ...
      <div className='control-section' style={{ textAlign: "center" }}>
        <ChartComponent id='container' style={{ textAlign: "center" }} ref={chartInstance} primaryXAxis={{
            ...
            majorGridLines: { width: 0 },
            visible: false
        }} chartArea={{ border: { width: 0 } }} load={load.bind(this)} height={'60%'} width={Browser.isDevice ? '100%' : '75%'} title='USA, Texas' titleStyle={{ textAlignment: 'Far', size: '20px' }} axisLabelRender={axisLabelRender} annotationRender={annotationRender}>
          <Inject services={[SplineAreaSeries, DataLabel, Category, ChartAnnotation]}/>
          <AnnotationsDirective>
            <AnnotationDirective content={'<div id="chart_image"><img src="src/chart/images/cloudy.png" alt="Cloud Picture" style="width: 41px; height: 41px"/></div>'} coordinateUnits={'Pixel'} region={'Chart'} x={'10%'} y={'15%'}></AnnotationDirective>
            <AnnotationDirective content={'<div id="days" style="font-size: 11px;">Friday, 01:00 am</div>'} coordinateUnits={'Pixel'} region={'Chart'} x={'90%'} y={'15%'}></AnnotationDirective>
          </AnnotationsDirective>
          <SeriesCollectionDirective>
            <SeriesDirective dataSource={chartData} xName={'x'} yName={'y'} opacity={0.5} width={2} border={{ width: 2 }} type={'SplineArea'} marker={{ visible: false, dataLabel: { visible: true } }}/>
          </SeriesCollectionDirective>
        </ChartComponent>
      </div>
      <div id="button-container" style={{ textAlign: "center" }}>
        <button id="friday" className="custom-button" onClick={() => updateChart('friday', 'sunny_image', 24, 'Friday', 0, 0.2)}>
          <div className="day">Fri</div>
          <img src="src/chart/images/sunny_image.png" alt="Friday"/>
          <div className="temp">24°C - 19°C</div>
        </button>
        <button id="saturday" className="custom-button" onClick={() => updateChart('saturday', 'sunny_image', 20, 'Saturday', 0.1, 0.3)}>
          <div className="day">Sat</div>
          <img src="src/chart/images/sunny_image.png" alt="Saturday"/>
          <div className="temp">20°C - 25°C</div>
        </button>
        <button id="sunday" className="custom-button" onClick={() => updateChart('sunday', 'cloudy', 18, 'Sunday', 0.2, 0.4)}>
          <div className="day">Sun</div>
          <img src="src/chart/images/cloudy.png" alt="Sunday"/>
          <div className="temp">18°C - 24°C</div>
        </button>
        <button id="monday" className="custom-button" onClick={() => updateChart('monday', 'cloudy', 14, 'Monday', 0.3, 0.5)}>
          <div className="day">Mon</div>
          <img src="src/chart/images/cloudy.png" alt="Monday"/>
          <div className="temp">14°C - 19°C</div>
        </button>
        <button id="tuesday" className="custom-button" onClick={() => updateChart('tuesday', 'rainy', 14, 'Tuesday', 0.4, 0.6)}>
