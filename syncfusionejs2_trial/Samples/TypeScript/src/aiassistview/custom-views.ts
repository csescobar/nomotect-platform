...
function promptViewContent() {
    let suggestionsElem = '';
    (window as any).defaultSuggestions.forEach((suggestion: any) => {
...
function updateResponseView(prompt: string) {
    ...
    setTimeout( () => {
        let foundPrompt = (window as any).defaultPromptResponseData.find((promptObj: any) => promptObj.prompt === prompt);
