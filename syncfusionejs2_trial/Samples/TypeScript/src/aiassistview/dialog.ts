...
(window as any).default = () => {
    ...
    let dialogAIAssistView: AIAssistView = new AIAssistView({
        promptSuggestions: (window as any).defaultSuggestions,
    ...
    function onPromptRequest(args: PromptRequestEventArgs) {
        setTimeout(function () {
            let foundPrompt = (window as any).defaultPromptResponseData.find((promptObj: any) => promptObj.prompt === args.prompt);
            ...
            dialogAIAssistView.promptSuggestions = foundPrompt?.suggestions || (window as any).defaultSuggestions;
    ...
    function toolbarItemClicked(args: ToolbarItemClickedEventArgs) {
        ...
        if ((args as any).item.iconCss === 'e-icons e-aiassist-copy') {
