...
(window as any).default = () => {
    ...
    let prompts: { [key: string]: string | string[] } [] = [
        {
    ...
    let copiedTextArea: Element = document.querySelector('.copied-content');

    let overviewAiAssist: AIAssistView = new AIAssistView({
        ...
        promptRequest: function (args: PromptRequestEventArgs) {
            setTimeout(function () {
                let foundPrompt = prompts.find((promptObj) => promptObj.prompt === args.prompt);

                if (foundPrompt) {
                    overviewAiAssist.addPromptResponse(foundPrompt.response);
                    overviewAiAssist.promptSuggestions = foundPrompt.suggestionData as string[];
                }
                else { 
                    overviewAiAssist.addPromptResponse("<p>For real-time prompt processing, connect the AIAssistView component to your preferred AI service, such as OpenAI or Azure Cognitive Services. Ensure you obtain the necessary API credentials to authenticate and enable seamless integration.</p>"); 
                    overviewAiAssist.promptSuggestions = suggestion;
                }
            }, 2000);
        }
    });
    overviewAiAssist.appendTo('#aiassist-overview-sample');

    async function updateCopiedContent() {
        copiedTextArea.innerHTML += await window.navigator.clipboard.readText();
    }

    let dialog: Dialog = new Dialog({
        content: document.getElementById('#aiassist-overview-sample'),
        target: '#target',
        width: '80vh',
        height: '70vh',
        visible: false
