    ...
    searchPopup = new AutoComplete(
        {
            ...
            filtering: function (e: any): void {
                ...
                let value: any = [];
                // include only when not ignoreOnBuild and (on mobile) not hideOnDevice
                for (let file of val) {
                    if (file && file.doc && file.doc.ignoreOnBuild !== true && !(Browser.isDevice && file.doc.hideOnDevice === true)) {
...
function onsearchInputChange(e: any) {
    ...
    for (var j = 0; j < val.length; j++) {
        if (val[j] && val[j].doc && val[j].doc.ignoreOnBuild !== true && !(Browser.isDevice && val[j].doc.hideOnDevice === true)) {
...
function getSampleList(): Controls[] | { [key: string]: Object }[] {
    ...
    for (let temp of tempList) {
        ...
        if (Browser.isDevice && temp.hideOnDevice === true) {
            continue;
        }
        let data: DataManager = new DataManager((temp as any).samples);
        let query: Query = new Query().where('ignoreOnBuild', 'notEqual', true);
        if (Browser.isDevice) {
            query = query.where('hideOnDevice', 'notEqual', true);
        }
        temp.samples = <Samples[]>data.executeLocal(query);
        if ((temp as any).samples && (temp as any).samples.length) {
