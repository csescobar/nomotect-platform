...
(window as any).default = (): void => {
    ...
    xhttp.onreadystatechange = function (): void {
        if (xhttp.readyState === 4 && xhttp.status === 200) {
            datasrc = JSON.parse(xhttp.responseText); let j: number = 2100;
            for (let i: number = 0; i < datasrc.length; i++) {
                value = datasrc[i];
                /* tslint:disable:no-string-literal */
                data.push({ High: value['High'], Low: value['Low'],  Close: value['Close'], Open: value['Open'], date: new Date(2010, 6, j)
                });
                j++;
            }
            let chart: Chart = new Chart({
                series: [{ dataSource: data, width: 2, type: 'Candle', animation: { enable: true }, xName: 'date', low: 'Low',
                high: 'High', close: 'Close', volume: 'Volume', open: 'Open', name: 'Bitcoin', bearFillColor: '#2ecd71',
                bullFillColor: '#e74c3d', yName: 'Close'}], width: Browser.isDevice ? '100%' : '80%', crosshair: { enable: true },
            ...
            let rangeTop: RangeNavigator = new RangeNavigator({
                ...
                }, labelPosition: 'Outside', valueType: 'DateTime', dataSource: data, xName: 'date', yName: 'Close', theme: theme,
            ...
            let rangeBottom: RangeNavigator = new RangeNavigator({
                labelPosition: 'Outside', valueType: 'DateTime', series: [{ dataSource: data, xName: 'date', yName: 'Close' }],
    ...
    xhttp.open('GET', location.origin + location.pathname + 'src/rangenavigator/data-source/period-data.json', true);
    xhttp.send();
};