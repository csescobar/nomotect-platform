...
(window as any).default = (): void => {
    ...
    function fileSelected(this: any, args: any) {
        var file = args.fileDetails;
        if (['.txt', '.docx', '.pdf'].indexOf(file.type) !== -1 && this.selectedItems.length == 1) {
    ...
    if (generatebtn.element) {
        generatebtn.element.onclick = (): void => {
            ...
            let aiOutput: any = (window as any).getAzureChatAIRequest({ messages: [{ role: 'user', content: inputData }] });
