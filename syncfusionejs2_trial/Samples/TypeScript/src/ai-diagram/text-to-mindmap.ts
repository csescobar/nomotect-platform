...
(window as any).default = (): void => {
    ...
    function toolbarItems() {
        let items: ItemModel[] = [
            { prefixIcon: 'e-icons e-undo tb-icons', tooltipText: 'Undo', disabled: true },
            { prefixIcon: 'e-icons e-redo tb-icons', tooltipText: 'Redo', disabled: true },
            {
                type: 'Separator'
            },
    ...
    const aiDBElement = document.getElementById('diagram-db-send') as HTMLInputElement;
    if (aiDBElement) {
        aiDBElement.onclick = () => {
            dialog.hide();
            convertTextToMindmap(textBox.value)
        }
    }
    ...
    if (msgBtn1) {
        msgBtn1.onclick = () => {
            dialog.hide();
            convertTextToMindmap(msgBtn1.innerText);
        }
    }
    if (msgBtn2) {
        msgBtn2.onclick = () => {
            dialog.hide();
            convertTextToMindmap(msgBtn2.innerText);
        }
    }
    if (msgBtn3) {
        msgBtn3.onclick = () => {
            dialog.hide();
            convertTextToMindmap(msgBtn3.innerText);
        }
    }


    async function convertTextToMindmap(inputText: string) {
        showLoading();
        const options = {
            messages: [
                {
        ...
        try {
            ...
            convertTextToMindmap(inputText);
