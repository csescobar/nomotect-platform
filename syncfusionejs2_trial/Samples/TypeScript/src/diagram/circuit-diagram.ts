...
import {
    Diagram, ConnectorEditing, ConnectorModel, LineRouting,
    DiagramConstraints, NodeModel, ShapeAnnotationModel, PathAnnotationModel,
    PointPortModel, PortVisibility, Snapping, SnapConstraints, AvoidLineOverlapping
...
(window as any).default = (): void => {
    ...
    function initDiagramModel(): void {
        create1to16Node('node1', 200, 180, 80, 240);
        create1to16Node('node2', 200, 427.5, 80, 240);
        create9to5Node('node3', 400, 127.5, 100, 135);
        create9to5Node('node4', 400, 367.5, 100, 135);
        create9to5Node('node5', 600, 127.5, 100, 135);
        create9to5Node('node6', 600, 367.5, 100, 135);
        create16to1Node('node7', 800, 240, 80, 240);
        createInputNode('node8', 50, 40, 60, 30, 'Cin');
        createInputNode('node9', 50, 180, 60, 30, 'A');
        createInputNode('node10', 50, 427.5, 60, 30, 'B');
        createOutputNode('node11', 950, 240, 60, 30, 'S');
        createOutputNode('node12', 950, 367.5, 60, 30, 'Cout');

        createConnector('connector01', 'node8', 'node3', 0, 8, 'lightGreen');
        createConnector('connector02', 'node9', 'node1', 0, 0);
        createConnector('connector03', 'node10', 'node2', 0, 0);
        createConnector('connector04', 'node7', 'node11', 0, 0);
    ...
    function addPortsLabels(node: NodeModel, count: number, side: string, labels?: string[], factor?: number): void {
        ...
        for (let i = 1; i <= count; i++) {
            const label: ShapeAnnotationModel = {
                ...
                offset: { x: x, y: (i / factor) - (1 / factor) },
                style: { fontSize: 9 }
            };
    ...
    function createNode(id: string, x: number, y: number, width: number, height: number, label?: string): NodeModel {
        ...
        if (label) {
            const annotation: ShapeAnnotationModel = { content: label };
    ...
    function createConnector(id: string, sourceId: string, targetId: string, sourcePortIndex: number, targetPortIndex: number, strokeColor: string | null = null): void {
        const diagramConnector: ConnectorModel = {
            ...
            style: { strokeColor: strokeColor ? strokeColor : "green", strokeWidth: 2 },
