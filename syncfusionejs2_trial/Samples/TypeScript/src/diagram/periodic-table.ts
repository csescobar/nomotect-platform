 ...
 */
import {
  Diagram, NodeModel, ShapeAnnotationModel,
  BasicShapeModel, SnapConstraints, NodeConstraints,
  DiagramTools, PointModel,
  randomId,
  MouseEventArgs,
  LinearGradientModel,
  SelectorConstraints,
...
function createElementNode(element: Element): NodeModel {
  ...
  const annotations: ShapeAnnotationModel[] = [
    ...
    // Atomic number annotation (top-right)
    {
...
function createRowsColoumns(): NodeModel[] {
...
function createBlock(): NodeModel[] {
  const nodes: NodeModel[] = [
    {
      id: 'p_block',
...
function initPeriodicTable(): NodeModel[] {
  const nodes: NodeModel[] = [];
  ...
  // Add row and coloumn numbers
  nodes.push(...createRowsColoumns());

  // Add block labels
  nodes.push(...createBlock());
