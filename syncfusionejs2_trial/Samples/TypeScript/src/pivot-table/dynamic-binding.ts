...
(window as any).default = (): void => {
    loadCultureFiles();
    let dataSource: any;
    function toDialogText(value: any): string {
        if (value == null) return '';
        if (typeof value === 'string') return value;
        if (value instanceof Error) return value.message || String(value);
        try { return JSON.stringify(value, null, 2); } catch (_) { return String(value); }
    }
    function ej2Alert(message: string, title: string) {
        return new Promise<void>(function (resolve) {
            var dlg = DialogUtility.alert({
                title: title || 'Message',
                content: toDialogText(message as any),
    ...
    function ej2Prompt(title: string, defaultValue: string, message?: { message: any; }) {
    ...
    var pivot: PivotView;
    var connectMenu: Menu, openMenu: Menu;
    var shouldAutoConfig = false;
    var currentData = Pivot_Data;
    var lastRemote: { kind: 'CSV' | 'JSON' | null; url: string | null } = { kind: null, url: null };
    ...
    function cleanOlapForRelational() {
        if (!pivot) return;
        pivot.olapEngineModule = null; pivot.dataType = 'pivot'; pivot.engineModule = new PivotEngine();
    ...
    function resetPivot() {
        ...
        (pivot.dataSourceSettings as any).conditionalFormatSettings = [];
    ...
    function setPivotData(type: DataSourceType, data: any) {
        if (!pivot) return;
    ...
    function applyReportSettings(
        ...
        return new Promise<void>(function (resolve) {
            if (isOlapReport) {
                currentData = [];
                pv.olapEngineModule = new OlapEngine();
                pv.dataType = 'olap';
    ...
    function loadRemoteAndBind(kind: string, url: string): Promise<void> {
    ...
    function xmlaSoapEnvelope(requestType: string, restrictions: { [x: string]: any; CATALOG_NAME?: any; }, properties: { [x: string]: any; DataSourceInfo?: any; }) {
    ...
    function postXMLA(endpoint: string, bodyXml: string) {
    ...
    function parseRowset(xmlText: string) {
        ...
        var result = rows.map(function (r: any) { var obj: any = {}; Array.prototype.slice.call(r.children).forEach(function (c: any) { obj[c.localName] = (c.textContent || '').trim(); }); return obj; });
        var fault = xml.getElementsByTagNameNS('*', 'Fault')[0]; if (fault) throw new Error((fault.textContent || 'SOAP Fault').trim());
        return result;
    }
    function discoverDataSources(endpoint: string) { var body = xmlaSoapEnvelope('DISCOVER_DATASOURCES', {}, {}); return postXMLA(endpoint, body).then(function (xml) { return parseRowset(xml).map(function (r: any) { return r.DataSourceName || r.DATASOURCE_NAME || r.Name || r.NAME; }).filter(Boolean); }); }
    function discoverCatalogsWithDS(endpoint: string, dataSource: any) { var body = xmlaSoapEnvelope('DBSCHEMA_CATALOGS', {}, { DataSourceInfo: dataSource }); return postXMLA(endpoint, body).then(function (xml) { return parseRowset(xml).map(function (r: any) { return r.CATALOG_NAME; }).filter(Boolean); }); }
    function discoverCubesWithDS(endpoint: string, catalog: any, dataSource: any) { var body = xmlaSoapEnvelope('MDSCHEMA_CUBES', { CATALOG_NAME: catalog }, { DataSourceInfo: dataSource }); return postXMLA(endpoint, body).then(function (xml) { return parseRowset(xml).filter(function (r: any) { return r.CUBE_SOURCE === '1'; }).map(function (r: any) { return r.CUBE_NAME; }).filter(Boolean); }); }
    function applyOlapBinding(opts: { url: any; catalog: any; cube: any; }) {
        if (!pivot) return;
        var url = (opts && opts.url) || ''; var catalog = (opts && opts.catalog) || ''; var cube = (opts && opts.cube) || '';
        if (!url || !catalog || !cube) return;
        var olapSettings = { url: url, catalog: catalog, providerType: 'SSAS', cube: cube, localeIdentifier: 1033, rows: [], columns: [], values: [] } as DataSourceSettingsModel;
        pivot.engineModule = null; pivot.olapEngineModule = new OlapEngine(); pivot.dataType = 'olap'; pivot.dataSourceSettings = olapSettings;
        if ((pivot.dataSourceSettings as any).type) {
            (pivot.dataSourceSettings as any).type = undefined;
        }
        currentData = []; shouldAutoConfig = true; pivot.refresh();
    }

    function toolbarRender(args: { customToolbar: any[]; }) {
    ...
    function saveReport() {
        ...
        try {
            ...
            const hasInline = Array.isArray(parsed.dataSourceSettings.dataSource) && parsed.dataSourceSettings.dataSource.length > 0;
            if (!hasInline && lastRemote && lastRemote.url && !parsed.dataSourceSettings.dataUrl && !parsed.dataSourceSettings.csvUrl) {
                if (lastRemote.kind === 'JSON') { (parsed.dataSourceSettings as any).dataUrl = lastRemote.url; (parsed.dataSourceSettings as any).type = 'JSON'; }
                if (lastRemote.kind === 'CSV') { (parsed.dataSourceSettings as any).csvUrl = lastRemote.url; (parsed.dataSourceSettings as any).type = 'CSV'; }
            }
    ...
    function openToolbarClicked(args: { item: { id: any; }; }) {
        var id = args && args.item && args.item.id; if (!id) return;
        if (id === 'local_report') {
            var input: any = document.getElementById('connectFile');
            input.onchange = function (e: any) {
                var file = (e as any).target.files && (e as any).target.files[0]; if (!file) return;
        ...
        if (id === 'remote_report') {
            ej2Prompt('Load Pivot Report', defaultUrls.REPORT).then(function (url: string | null) {
                if (url == null) return;
    ...
    function gridToolbarClicked(args: { item: { id: any; }; }) {
        var id = args && args.item && args.item.id; if (!id) return;
                ...
                reader.onload = function (evt) {
                    try {
                        if (ext === 'CSV') {
                            ...
                            if (inconsistent) { throw new Error('Please enter a valid Report CSV '); }
                            resetPivot();
                            setPivotData('CSV', arr as any);
                        } else {
                            var raw = String(evt.target && evt.target.result || '');
                            ...
                            if (looksLikeReport) {
                                ...
                                var isOlapReport = reportSettings && reportSettings.providerType === 'SSAS';
                                resetPivot();
                                applyReportSettings(pivot, reportSettings, isOlapReport, unwrapped).catch(function (err) { ej2Alert('Failed to load file: ' + err.message, 'Error'); });
                            } else {
                    ...
                    } catch (err) { ej2Alert('Failed to load file: ' + (err && err.message ? err.message : String(err)), 'Error'); }
                    finally { (e as any).target.value = ''; }
                };
                reader.readAsText(file);
            };
        ...
        if (id === 'remote_csv' || id === 'remote_json') {
            const kind = (id === 'remote_csv') ? 'CSV' : 'JSON';
            ej2Prompt('Connect to ' + kind, (defaultUrls as any)[kind]).then(function (url: string | null) {
                if (url == null) return;
                const clean = (url || '').trim();
        ...
        if (id === 'olap') {
            (function showOlapDialog() {
                ...
                function addLabel(text: string) { var l = document.createElement('label'); l.textContent = text; l.style.color = '#666'; l.style.fontSize = '14px'; l.style.fontWeight = '500'; return l; }
                ...
                var dsDrop = new DropDownList({ dataSource: [], enabled: false, placeholder: 'Select data source', change: onDsChange }); dsDrop.appendTo(dsDropEl);
                var catDrop = new DropDownList({ dataSource: [], enabled: false, placeholder: 'Select catalog', change: onCatChange }); catDrop.appendTo(catDropEl);
                var cubeDrop = new DropDownList({ dataSource: [], enabled: false, placeholder: 'Select cube' }); cubeDrop.appendTo(cubeDropEl);
                ...
                function onDsChange() {
                    try {
                        ...
                        cubeDrop.placeholder = 'Select cube'; cubeDrop.value = null; cubeDrop.dataSource = []; cubeDrop.dataBind();
                        discoverCatalogsWithDS(urlInput.value.trim(), ds as string).then(function (catalogs) {
                            catDrop.placeholder = 'Select catalog'; catDrop.dataSource = catalogs; catDrop.enabled = catalogs.length > 0; catDrop.dataBind();
                        }).catch(function (e: any) { errorEl.textContent = 'Failed to load catalogs: ' + e.message; });
                    } catch (e: any) { errorEl.textContent = 'Failed to load catalogs: ' + e.message; }
                }
                function onCatChange() {
                    try {
                        ...
                        discoverCubesWithDS(urlInput.value.trim(), cat as string, ds as string).then(function (cubes) {
                            cubeDrop.placeholder = 'Select cube'; cubeDrop.dataSource = cubes; cubeDrop.enabled = cubes.length > 0; cubeDrop.dataBind();
                        }).catch(function (e: any) { errorEl.textContent = 'Failed to load cubes: ' + e.message; });
                    } catch (e: any) { errorEl.textContent = 'Failed to load cubes: ' + e.message; }
                }
                function onOk() {
                    ...
                    try { errorEl.textContent = ''; applyOlapBinding({ url: url, catalog: cat as string, cube: cube as string }); dialog.hide(); }
                    catch (e: any) { errorEl.textContent = 'Failed to apply OLAP: ' + e.message; }
                }
