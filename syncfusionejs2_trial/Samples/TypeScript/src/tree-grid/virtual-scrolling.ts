...
import { virtualScrollData, virtualDataSource } from './data-source';
import { loadCultureFiles } from '../common/culture-loader';

TreeGrid.Inject(VirtualScroll, Toolbar, Edit, RowDD);

let statusTemplate = (args: any) => {
    let status = (args.Status || '').toString().toLowerCase();
    let cssClass: string = 'rg-badge-status-maintenance';
...
let priorityTemplate = (args: any) => {
    let priority: any = (args.Priority || '').toString().toLowerCase();
    let cssClass: string = 'rg-badge-priority-medium';
...
let complianceTemplate = (args: any) => {
    let value: number = Math.max(0, Math.min(100, parseInt(args.ComplianceScore || 0)));
