...
import { InlineAIPrompt, InlinePromptRequestEventArgs, ToolbarItemClickEventArgs } from '@syncfusion/ej2-interactive-chat';
...
(window as any).default = (): void => {
    ...
    const rteEditor: RichTextEditor = new RichTextEditor({
        quickToolbarSettings: {
            text: [{ prefixIcon: 'e-icons e-ai-chat' } as any, 'Bold', 'Italic', 'Underline', 'StrikeThrough', 'Fontcolor', 'BackgroundColor', '|', 'Unorderedlist', 'Orderedlist']
        },
        toolbarClick: (args: any): void => {
            if (args.item.prefixIcon === 'e-icons e-ai-chat') {
                ...
                if (selectedText && selectedText.length > 0) {
                    ...
                    rtePrompt.relateTo = relateToEl ? range.endContainer : wrapper;
                    rtePrompt.dataBind();
                    rtePrompt.showPopup();
                }
            }
    ...
    const rtePrompt: InlineAIPrompt = new InlineAIPrompt({
    ...
    rtePrompt.appendTo('#inline-ai-prompt');
                        ...
                        .then((response: Response) => {
                            ...
                            function processStream(): Promise<void> {
                                return reader.read().then((result: any) => {
                                    ...
                                    if (done) {
                                        rtePrompt.addResponse(fullText, true);
                                    ...
                                    fullText += chunk;
                                    rtePrompt.addResponse(fullText, false);
