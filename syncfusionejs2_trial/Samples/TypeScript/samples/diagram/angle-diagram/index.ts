...
import {
    Diagram, NodeModel, NodeConstraints, ConnectorModel, ConnectorConstraints, SnapConstraints, SelectorConstraints,
    IRotationEventArgs
...
CircularGauge.Inject(Annotations);
    ...
    // NumericTextBox instance for angle
    let angleNumeric: NumericTextBox;
    // Flag to prevent recursive updates when programmatically changing NumericTextBox value
    let suppressAngleChange = false;
    // Angle constants for UI/model mapping
    const RELATIVE_MIN = -57; // relative UI angle min (maps to 303 deg)
    const RELATIVE_MAX = 43;  
    ...
    // Initialize diagram with solar panel setup
    function initializeDiagram(): void {
        ...
        diagram = new Diagram({
            ...
            scrollChange: () => {
                if (locationDropdown && (locationDropdown as any)?.isPopupOpen) {
            ...
            load: () => {
                setTimeout(() => {
                    if (diagramCreated) {
                        ...
                        updateUI(); // Call updateUI after initialization
                        diagram.fitToPage();
                    }
                });
            },
    ...
    // Handles solar panel rotation events.
    function onRotationChange(args: IRotationEventArgs): void {
        if (args.state === 'Completed') {
            if (args.source && args.source.nodes && args.source.nodes[0]) {
                const node: NodeModel = args.source.nodes[0];
                if (node.id === 'solarPanelFrame') {
                    solarData.currentAngle = Math.round(args.newValue.rotateAngle) % 360;
                    angleCalculation();
                    calculateSolarPosition();
                    calculateEfficiency();
                    updateUI(); // Update UI after rotation
                }
            }
        } else if (args.state === 'Progress') {
            if (args.source && args.source.nodes && args.source.nodes[0]) {
                const selectedNode: NodeModel = args.source.nodes[0];
                if (selectedNode.id === 'solarPanelFrame') {
                    const proposedAngle: number = args.newValue.rotateAngle;
                    const normalizedAngle = normalizeAngle(proposedAngle);
    ...
    /**
     * normalizeAngle
     * Normalizes any angle (positive or negative) into the [0, 360) range.
     */
    ...
    /**
     * relativeToDiagramAngle
     * Converts a relative UI angle in [-57..43] into a diagram rotateAngle in [303..360) U [0..43].
     * - Negative values wrap into 303..359 (e.g., -57 => 303).
     * - Non-negative values remain 0..43.
     */
    function relativeToDiagramAngle(relative: number): number {
        const r = Math.round(Math.max(RELATIVE_MIN, Math.min(RELATIVE_MAX, relative)));
        // normalizeAngle will wrap negatives into the upper band [303..360)
    ...
    function onLocationChanged(args: any): void {
        ...
        const location: LocationData = getLocationData(solarData.selectedLocation);
        if (location) {
            // Map location angle to the 303-43 degree range
            const locationAngle = Math.min(100, location.angle);

            // This mapping logic needs to ensure it correctly translates to the Diagram's 0-360 scale
            // and the derived -57 to 43 range for intPanelAngleDeg.
            // Simplified mapping from 0-100 to the allowed 303-43 degree range in diagram.
            // The intPanelAngleDeg will be calculated from currentAngle in angleCalculation().
            if (locationAngle <= 57) { // 0 to 57 maps to 303 to 360 (Diagram's 0-57 degrees from 360 reference)
                solarData.currentAngle = 303 + locationAngle;
            } else { // 58 to 100 maps to 0 to 42 (Diagram's 58-100 degrees are out of bounds, so clamp to 43 max)
                     // Adjusted to map 58-100 from location.angle to 0-43 in currentAngle.
                     // A simple linear mapping might be too aggressive.
                     // The JS code does: locationAngle - 57. This means locationAngle 58 => 1, locationAngle 100 => 43.
    ...
    function initializePerformanceMessage(): void {
        const efficiencyMsg: { message: string, severity: Severity } = getPerformanceMessage();
        if (performanceMessage) {
            performanceMessage.destroy();
        }
        performanceMessage = new Message({
            content: efficiencyMsg.message,
            severity: efficiencyMsg.severity, // Cast to Severity enum
            showIcon: true,
    ...
    // Initialize angle controls with custom validation and event handling
    function initializeAngleControls(): void {
        if (angleNumeric) angleNumeric.destroy();

        const relativeAngle: number = solarData.intPanelAngleDeg - 57;
        ...
        if (inputEl) {
            ...
            inputEl.addEventListener('keydown', (e: KeyboardEvent) => {
                ...
                e.stopPropagation(); // do not let diagram handle keys while editing
            });
        }

        // If the user types numeric keys while diagram has focus, focus the input
        document.addEventListener('keydown', function onDocKeydown(e: KeyboardEvent) {
            ...
            if (isEditKey) {
                ...
                if (input) {
                    ...
                    setTimeout(() => { try { (input as HTMLInputElement).select(); } catch { } }, 0);
                }
            }
        }, { capture: true });
    }
    ...
    // Apply relative angle (-57..43) -> rotateAngle [303..360, 0..43]
    function applyRelativeAngle(val: number | null | undefined) {
        if (typeof val !== 'number') return;
        // Convert the relative angle back to diagram's rotation angle (0-360)
        const rotateAngle = relativeToDiagramAngle(val);
        solarData.currentAngle = rotateAngle;

        updateDiagram();
        angleCalculation();
        calculateSolarPosition();
        calculateEfficiency();
        updateUI(); // Keep this to ensure all other UI elements are updated after angle change
    }

    // Solar calculations
    function angleCalculation(): void {
        const normalizedAngle: number = solarData.currentAngle;

        // For angles from 303 to 360, map to 0-57 degrees (inclusive of 360, which is 0 degrees)
        if (normalizedAngle >= 303 && normalizedAngle <= 360) {
            solarData.intPanelAngleDeg = normalizedAngle - 303; // 303 -> 0, 360 -> 57
        }
        // For angles from 0 to 43, map to 57-100 degrees
        else if (normalizedAngle >= 0 && normalizedAngle <= 43) {
            solarData.intPanelAngleDeg = 57 + normalizedAngle; // 0 -> 57, 43 -> 100
        }
        else {
            // Default or fall-back, ensuring it's within a sensible range if calculation leads to unexpected
    ...
    function calculateSolarPosition() {
        const location: LocationData = getLocationData(solarData.selectedLocation);
        if (!location) return;
        const lat: number = location.latitude * Math.PI / 180.0;
        const selectedDateForCalculation: Date = new Date(solarData.selectedDateTime);
        const startOfYearForCalculation: Date = new Date(selectedDateForCalculation.getFullYear(), 0, 1);

        const dayOfYear: number = Math.floor((selectedDateForCalculation.getTime() - startOfYearForCalculation.getTime()) / (1000 * 60 * 60 * 24)) + 1;
        const declination: number = 23.45 * Math.PI / 180.0 * Math.sin(2 * Math.PI * (284 + dayOfYear) / 365.0);
        const hourAngle: number = 0; // Solar noon
        const elevation: number = Math.asin(Math.sin(declination) * Math.sin(lat) +
        ...
        if (solarData.sunElevation > 0) {
            const elevationRad: number = elevation;
            let airMass: number = 1.0 / Math.sin(elevationRad);
    ...
    function updateDiagram(): void {
        if (diagram) {
            const solarPanelNode: NodeModel = diagram.getObject('solarPanelFrame') as NodeModel;
            if (solarPanelNode) {
    ...
    /**
     * Synchronizes the NumericTextBox value with the current model data.
     * Prevents triggering the change event.
     */
    function syncAngleInputFromModel(): void {
        if (!angleNumeric) return;
        const relativeAngle: number = solarData.intPanelAngleDeg - 57;
        suppressAngleChange = true; // Block change event from firing
    ...
    function updateUI(): void {
        // Synchronize NumericTextBox from the model first
        syncAngleInputFromModel();

        // Update angle description
        const angleDescElement: HTMLElement | null = document.getElementById('angleDescription');
        if (angleDescElement) {
    ...
    <div style="text-align: center;">
        ...
        if (performanceMessage) {
            ...
            performanceMessage.severity = messageData.severity as Severity; // Cast to Severity enum type
    ...
    function getAngleDescription(): string {
        const relativeAngle: number = Math.abs(solarData.intPanelAngleDeg - 57); // This gives 0 for horizontal (intPanelAngleDeg 57)
        if (relativeAngle <= 5) return 'Horizontal'; // Adjust threshold for "horizontal"
    ...
    function getLocationData(locationName: string): LocationData {
        return locationData.find(loc =>
            loc.name.toLowerCase() === locationName.toLowerCase()
        )!;
    }

    // Initial calculations (moved after all functions are defined)
    initializeDiagram();
    initializeLocationDropdown();
    initializeEfficiencyGauge();
    initializePerformanceMessage();
    ...
    // Perform initial calculations and update UI based on default values
    calculateSolarPosition();
    angleCalculation();
    calculateEfficiency();
    updateUI(); // Initial UI update after all components are ready
