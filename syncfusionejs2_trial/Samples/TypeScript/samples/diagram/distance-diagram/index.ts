...
import {
    Diagram, NodeModel, ConnectorModel, ConnectorConstraints, NodeConstraints,
    PortVisibility, FlipDirection, SnapConstraints, IConnectionChangeEventArgs,
    DiagramTools, DiagramConstraints
...
import { DropDownList, ChangeEventArgs } from '@syncfusion/ej2-dropdowns';
    ...
    let selectedUnit: string = 'in';
    let diagramCreated: boolean = false;

    // Helper function to create a wall connector
    function createWallConnector(id: string, startX: number, startY: number, endX: number, endY: number, color: string, thickness: number): void {
    ...
    function createItem(id: string, x: number, y: number, width: number, height: number, pathData: string, options: ItemOptions): void {
        let item: NodeModel = {
            id: id,
            width: width,
            height: height,
    ...
    function createBedroomItem(id: string, x: number, y: number, width: number, height: number, pathData: string, color: string): void {
        ...
        if (id === 'BedItem2') {
            ...
            options.fill = '#E0F6FF'; // Light peach for bed
        ...
        if (id === 'BedItem4') {
            ...
            options.fill = '#F0F8E8'; // Light green for bed
    ...
    function createKitchenItem(id: string, x: number, y: number, width: number, height: number, pathData: string, color: string): void {
        let options: ItemOptions = {
            fill: color,
            strokeColor: '#333',
        };
        if (id === 'KitchenSink') {
            options.fill = '#FFF2E8'; // Light cyan for sink
            options.strokeColor = '#D2691E';
        }
        if (id === 'KitchenCounter') {
            options.rotateAngle = 90;
            options.fill = '#F5F5DC'; // Beige for counter
            options.strokeColor = '#8B4513';
        }
        createItem(id, x, y, width, height, pathData, options);
    }

    function createLivingRoomItem(id: string, x: number, y: number, width: number, height: number, pathData: string, color: string): void {
        let options: ItemOptions = {
            fill: '#FFF8DC', // Cornsilk for living room items
            strokeColor: '#B8860B',
        };
        if (id === 'Bathroom') {
            options.rotateAngle = 90;
    ...
    function createPortConnections(): void {
        let wallOuterTop = connectors.find((c) => c.id === 'WallOuterTop');
        let wallOuterLeft = connectors.find((c) => c.id === 'WallOuterLeft');
        let wallOuterBottom = connectors.find((c) => c.id === 'WallOuterBottom');
        let wallOuterRight = connectors.find((c) => c.id === 'WallOuterRight');
        let separator1 = connectors.find((c) => c.id === 'Separator1');
        let separator2 = connectors.find((c) => c.id === 'Separator2');
        let separator3 = connectors.find((c) => c.id === 'Separator3');
        let separator4 = connectors.find((c) => c.id === 'Separator4');
    ...
    function createRestroomSeparator(): void {
        createWallConnector('RestroomSeparator', 780, 610, 780, 480, '#333333', 5);
        let restroomSeparator = connectors.find((c) => c.id === 'RestroomSeparator');
        let wallOuterBottom = connectors.find((c) => c.id === 'WallOuterBottom');
    ...
    function createHallSeparator(): void {
        createWallConnector('HallSeparator', 540, 610, 540, 410, '#333333', 5);
        let hallSeparator = connectors.find((c) => c.id === 'HallSeparator');
        let wallOuterBottom = connectors.find((c) => c.id === 'WallOuterBottom');
    ...
    function createInitialLayout(): void {
        ...
        // Update all node coordinates (shifted by x-160, y-110)
        createBedroomItem(
    ...
    // Measurement and Annotation Logic (with updated coordinates)
    function createDimensionConnector(id: string, startX: number, startY: number, endX: number, endY: number, dimensionText: string, isHorizontal: boolean): void {
    ...
    function createBathroomDoor(id: string, x: number, y: number, width: number, height: number, pathData: string, color: string): void {
        let options: ItemOptions = {
            fill: '#E6E6FA',
            strokeColor: '#9370DB',
            rotateAngle: 270,
    ...
    function createExtensionLine(id: string, startX: number, startY: number, endX: number, endY: number): void {
    ...
    function addMeasurementConnectorsRuntime(): void {
        let allSeparators = connectors.filter(
    ...
    function addOverallMeasurementConnectors(): void {
        let wallOuterTop = connectors.find((c) => c.id === 'WallOuterTop');
        let wallOuterLeft = connectors.find((c) => c.id === 'WallOuterLeft');
        let wallOuterBottom = connectors.find((c) => c.id === 'WallOuterBottom');
        let wallOuterRight = connectors.find((c) => c.id === 'WallOuterRight');
        ...
        if (wallOuterTop && wallOuterLeft && wallOuterBottom && wallOuterRight) {
            ...
            let widthText = formatMeasurement(convertPixelsToSelectedUnit(totalWidth));
            ...
            let heightText = formatMeasurement(convertPixelsToSelectedUnit(totalHeight));
    ...
    function addSpecificSeparatorMeasurement(separator: ConnectorModel): void {
        let wallOuterTop = connectors.find((c) => c.id === 'WallOuterTop');
        let wallOuterBottom = connectors.find((c) => c.id === 'WallOuterBottom');
        let wallOuterRight = connectors.find((c) => c.id === 'WallOuterRight');
        let distance: number;
    ...
    function convertPixelsToSelectedUnit(pixelValue: number): number {
        switch (selectedUnit) {
    ...
    function convertBetweenUnits(value: number, fromUnit: string, toUnit: string): number {
    ...
    function updateConnectorAnnotationsFromStoredDistances(): void {
        ...
        diagram.connectors.forEach((connector: ConnectorModel) => {
            if (connector.id.includes('Measurement') && connector.annotations && connector.annotations.length > 0) {
                ...
                if (parts.length > 1) {
                    ...
                    let convertedValue = convertBetweenUnits(value, currentUnit, selectedUnit);
    ...
    function onConnectionPointChanged(args: IConnectionChangeEventArgs): void {
        updateDimensionConnectors(args.connector as ConnectorModel);
    }

    function updateDimensionConnectors(movedConnector: ConnectorModel): void {
        ...
        if (id.startsWith('Separator') || id === 'RestroomSeparator' || id === 'HallSeparator') {
            ...
            let dimConnector = diagram.getObject(measurementId) as ConnectorModel;
            let extStartConnector = diagram.getObject(measurementId + '_ExtStart') as ConnectorModel;
            let extEndConnector = diagram.getObject(measurementId + '_ExtEnd') as ConnectorModel;

            if (!dimConnector || !extStartConnector || !extEndConnector) return;

            let wallOuterTop = diagram.getObject('WallOuterTop') as ConnectorModel;
            let wallOuterBottom = diagram.getObject('WallOuterBottom') as ConnectorModel;
            let wallOuterRight = diagram.getObject('WallOuterRight') as ConnectorModel;
    ...
    function onUnitChanged(args: ChangeEventArgs): void {
        selectedUnit = args.value as string;
        updateConnectorAnnotationsFromStoredDistances();
        diagram.dataBind();
    }

    // Populate the dropdown with options
    let unitOptions: { [key: string]: string }[] = [
    ...
    diagram = new Diagram({
        ...
        load: (): void => {
            if (diagramCreated) {
                setTimeout((): void => diagram.fitToPage(), 10);
            }
        }
    });
