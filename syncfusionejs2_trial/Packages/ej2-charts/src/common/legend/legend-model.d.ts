...
export interface LegendSettingsModel {
    ...
    /**
     * Enables synchronized legend behavior across multiple series in AccumulationChart.
     * When set, points with matching `mappingKey` values are grouped into a single legend item.
     * This is useful for multi-series pie/donut charts to create category-based legends.
     *
     * Example: If `mappingKey` is set to 'category', all points with the same category value
     * across different series will be represented by a single legend item.
     *
     * @default null
     */
