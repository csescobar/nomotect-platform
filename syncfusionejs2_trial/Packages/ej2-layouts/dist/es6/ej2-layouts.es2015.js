...
let Splitter = class Splitter extends Component {
    ...
    createSeparator(i) {
        ...
        const toggleLabel = this.getLocaleText('ToggleNavigation');
    ...
    getLocaleText(text) {
        const locale = this.localeObj.getConstant(text);
        return (locale === '') ? text : locale;
    }
    localeSetModelOption(newProp) {
        const newLocale = newProp && newProp.locale && typeof newProp.locale === 'object' ? newProp.locale : this.locale;
...
let DashboardLayout = class DashboardLayout extends Component {
    ...
    hasTemplateExpressions(template) {
        return /\{\{[\s\S]*?\}\}/.test(template) || /\{\d+\}/.test(template);
    }
    createCspSafeTemplate(template) {
        const sanitized = this.enableHtmlSanitizer ? SanitizeHtmlHelper.sanitize(template) : template;
        return (dataContext) => {
            const result = sanitized.replace(/\{\{:(\w+)\}\}/g, (match, prop) => {
                return dataContext[prop] !== undefined ? String(dataContext[prop]) : match;
            });
