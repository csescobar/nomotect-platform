...
export class ChatUI extends InterActiveChatBase implements INotifyPropertyChanged {
    ...
    private renderGroup(chatContentWrapper: HTMLElement, msg: MessageModel, loadOldChat: boolean, index: number,
                        prevIndex: number, isPrependMessages?: boolean): void {
     ...
     * @returns {void} No return value.
     */
    public prependMessages(messages: string[] | MessageModel[]): void {
        if (isNOU(messages) || messages.length === 0) { return; }
        const prevOnChange: boolean = this.isProtectedOnChange;
        this.isProtectedOnChange = true;
        const isEmptyChat: boolean = this.messages.length > 0 ? false : true;
        const newMessageObjs: MessageModel[] = [];
        for (let i: number = 0; i < messages.length; i++) {
            const message: string | MessageModel = messages[parseInt(i.toString(), 10)];
            let newMessageObj: MessageModel;
            if (typeof message === 'string') {
                newMessageObj = {
                ...
                newMessageObj = {
                    ...message,
                    id: message.id || `${this.element.id}-message-${this.messages.length + i + 1}`,
                    author: message.author || this.user,
                    text: message.text || '',
                    timeStamp: message.timeStamp || new Date(),
                    timeStampFormat: message.timeStampFormat || this.timeStampFormat,
        ...
        if (isEmptyChat) {
            for (let i: number = 0; i < newMessageObjs.length; i++) {
