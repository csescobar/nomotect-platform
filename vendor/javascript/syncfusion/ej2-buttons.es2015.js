...
let Button = class Button extends Component {
    ...
    observeDomAttributeChanges() {
        this.mutationObserver = new MutationObserver((mutations) => {
            const isDomDisabled = this.element.hasAttribute('disabled');
     ...
     * @returns {void}
     */
    fireClick(originalEvent, isRepeat) {
        if (this.disabled) {
            return;
        }
     ...
     * @returns {void}
     */
    startRepeat(originalEvent) {
        if (originalEvent.button !== 0) {
            return;
        }
        this.fireClick(originalEvent, false);
        const effectiveInterval = this.repeatInterval > 0 ? this.repeatInterval : 100;
...
const CHECK = 'e-check';
const DISABLED = 'e-checkbox-disabled e-disabled';
const FRAME = 'e-frame';
...
const DISABLED$2 = 'e-switch-disabled e-disabled';
...
let Switch = class Switch extends Component {
            ...
            switch (prop) {
                ...
                case 'disabled':
                    if (newProp.disabled) {
                        ...
                        this.element.disabled = false;
                        wrapper.classList.remove(...DISABLED$2.split(' '));
                        wrapper.setAttribute('aria-disabled', 'false');
    ...
    setDisabled() {
        const wrapper = this.getWrapper();
        this.element.disabled = true;
        wrapper.classList.add(...DISABLED$2.split(' '));
        wrapper.setAttribute('aria-disabled', 'true');
    }
...
let ChipList = ChipList_1 = class ChipList extends Component {
    ...
    updateSelectedChips() {
        ...
        let chip = null;
        let value = null;
        const isValueBased = typeof this.selectedChips === 'string' || this.selectionType === 'value' ||
...
let SpeedDial = class SpeedDial extends Component {
    ...
    focusFirstElement() {
        ...
        let index = 0;
        while (ele[parseInt(index.toString(), 10)].classList.contains(DISABLED$3)) {
            index++;
