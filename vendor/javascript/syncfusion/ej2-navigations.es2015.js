...
const CLS_ACRDN_CTRL = 'e-acrdn-ctrl';
...
let Sidebar = class Sidebar extends Component {
    ...
    destroy() {
        ...
        if (this.target) {
            ...
            if (targetElement.querySelectorAll('.' + ROOT$1 + '.' + SIDEBARABSOLUTE).length === 0) {
        ...
        if (!isNullOrUndefined(sibling)) {
            let anyOtherSidebarOpen = false;
            if (this.target) {
                const openSidebarElements = this.target.querySelectorAll('.' + ROOT$1 + '.' + OPEN);
